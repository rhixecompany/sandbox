            ...
            for d in deps:
                if isinstance(d, str):
                    ...
                    if mm:
                        ...
                        if not exists:
                            dangling.append((base, mm.group(1), tgt))
...
RULES = [
    (
        "linux-triage",
        ["arch-linux-triage", "centos-linux-triage", "debian-linux-triage", "fedora-linux-triage", "linux-triage"],
    ),
    (
        "azure-cloud",
        ["az-cost-optimize", "azure-resource-health-diagnose", "update-avm-modules-in-bicep", "avm", "bicep"],
    ),
    ...
    (
        "data-sql-database",
        ["cosmosdb", "bigquery", "sql-code-review", "sql-optimization", "postgresql", "database", "db-"],
    ),
    (
        "frontend-web",
        [
...
W(
    f"- **{len(dangling)}** dangling `prompt:` references were found"
    + ("." if dangling else " — every referenced prompt exists.")
)
...
W("")
W(
    f"All 211 files were categorized into **{len([d for d in domain_counts if d != 'uncategorized'])}** domains by filename stem,"
)
...
W("---")
W(
    f"*Generated read-only. Library size: {total} prompts · Domains: {len([d for d in domain_counts if d != 'uncategorized'])} · Delegating prompts: {n_del_src} · Dangling refs: {len(dangling)}.*"
)
...
json.dump(
    {"deleg_map": deleg_map, "dangling": dangling, "domain_map": domain_map, "domain_counts": dict(domain_counts)},
    open(os.path.join(DOCS_DIR, "_registry_data.json"), "w"),
    indent=2,
)
