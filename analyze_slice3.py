...
for path in files:
    ...
    has_accept = bool(re.search(r'acceptance criteria|acceptance\b|definition of done|done when|success criteria', body_lower))
    has_verify = bool(re.search(r'verif|check that|confirm that|test that|run .* and confirm|self[- ]?check', body_lower))
    ...
    results.append(dict(
        name=name, size=len(text), fm_keys=list(fm.keys()),
        has_desc=has_desc, has_tags=has_tags, has_plan=has_plan, has_formatter=has_formatter,
        has_do=has_do, has_accept=has_accept, has_verify=has_verify,
        has_goal=has_goal, has_context=has_context, has_workflow=has_workflow,
        has_rules=has_rules, has_verif_sec=has_verif_sec,
        n_headers=len(headers), tmpl_refs=tmpl_refs, skill_refs=skill_refs, tool_refs=tool_refs,
        n_lines=non_empty,
...
print("name\tsize\thas_desc\thas_tags\thas_plan\thas_fmt\thas_do\thas_acc\thas_ver\thas_goal\thas_ctx\thas_wf\thas_rules\thas_versec\tn_hdr\tn_lines\ttag_count")
for r in results:
    tc = len(r['tags']) if isinstance(r['tags'], list) else 0
    print(f"{r['name']}\t{r['size']}\t{int(r['has_desc'])}\t{int(r['has_tags'])}\t{int(r['has_plan'])}\t{int(r['has_formatter'])}\t{int(r['has_do'])}\t{int(r['has_accept'])}\t{int(r['has_verify'])}\t{int(r['has_goal'])}\t{int(r['has_context'])}\t{int(r['has_workflow'])}\t{int(r['has_rules'])}\t{int(r['has_verif_sec'])}\t{r['n_headers']}\t{r['n_lines']}\t{tc}")

# dump references for manual inspection
print("\n=== REFERENCES ===")
for r in results:
    refs = []
    if r['tmpl_refs']: refs.append('TMPL:'+','.join(r['tmpl_refs']))
    if r['skill_refs']: refs.append('SKILL:'+','.join(r['skill_refs']))
    if r['tool_refs']: refs.append('TOOL:'+','.join(r['tool_refs']))
    if refs:
