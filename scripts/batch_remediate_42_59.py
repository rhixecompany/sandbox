...
def add_missing_sections(body, skill_name):
    ...
    if not has_section(modified, "Skills Required"):
        insert_after = re.search(
            r"^#{1,4}\s*(When NOT to Use|When to Use|Description)\s*$", modified, re.MULTILINE | re.IGNORECASE
        )
        if insert_after:
            pos = modified.find("\n", insert_after.end())
            if pos > 0:
    ...
    if not has_section(modified, "Pitfalls"):
        insert_after = re.search(
            r"^#{1,4}\s*(Verification Checklist|Best Practices|Workflow)\s*$", modified, re.MULTILINE | re.IGNORECASE
        )
        if insert_after:
            pos = modified.find("\n", insert_after.end())
            if pos > 0:
...
def remediate_skill_sync(path, score):
    ...
    if not fm.get("name"):
        fm["name"] = skill_name
    if not fm.get("title"):
        fm["title"] = skill_name.replace("-", " ").title()
    if not fm.get("description"):
        fm["description"] = f"Use when working with {skill_name.replace('-', ' ')}."
...
async def main():
    ...
    skills_to_fix = []
    rewrite_text = await loop.run_in_executor(
        None, read_file_sync, r"C:\Users\Alexa\Desktop\SandBox\judge_results\remaining_rewrite_list.txt"
    )
    if rewrite_text:
    ...
    for i, (score, name) in enumerate(skills_to_fix):
        if name in results:
            info = results[name]
            success, msg = await loop.run_in_executor(None, remediate_skill_sync, info["path"], score)
            if success:
                success_count += 1
            else:
                fail_count += 1
