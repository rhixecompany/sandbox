...
for path in files:
    ...
    has_acc = bool(
        re.search(
            r"acceptance criteria|acceptance\b|definition of done|done when|success criteria|definition of done", bl
        )
    )
    ...
    weak = []
    if not has_plan:
        weak.append("missing plan")
    if not has_acc:
        weak.append("no acceptance criteria")
    if not has_goal:
        weak.append("no Goal section")
    if not has_wf:
        weak.append("no Workflow/Phases")
    if not has_rules:
        weak.append("no Rules section")
    if not has_do:
        weak.append("no explicit Do/Don't rules")
    if not has_ver:
        weak.append("no verification step")
    if dead:
        weak.append("dead template refs")
    if n_lines < 20:
        weak.append("very thin (<20 lines)")
    elif n_lines < 30:
        weak.append("thin (<30 lines)")
    score = len(weak)
    rows.append(
        dict(
            name=name,
