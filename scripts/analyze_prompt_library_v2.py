#!/usr/bin/env python3
"""Accurate second-pass audit. Fixes false positives from v1.

Rules:
- tool: deps are VALID if they name a known MCP server OR a known Hermes native tool.
  Native tools: terminal, file, web, browser, vision, code_execution, tts, skills,
  todo, memory, session_search, clarify, delegation, cronjob, search_files, web_search,
  image_gen, context_engine, mcp.
- prompt: deps: strip any trailing .prompt.md -> slug. If slug is a known PROMPT -> OK.
  If slug is a known SKILL (not a prompt) -> MISLABELED (should be skill:).
  If neither -> DANGLING (missing file).
- Structure: heading contains any of a broad keyword set.
- Safety: only genuine override/jailbreak + truly un-gated destructive ops.
"""
...
for slug, d in data.items():
    ...
    for dep in fm.get("dependencies", []) or []:
        ...
        if low.startswith("tool:"):
            ...
            if tgt in prompts:
                pass  # OK (even if it had .prompt.md suffix, it resolves)
            elif tgt in KS:
...
safety = []
INJ = re.compile(
    r"ignore (previous|prior|above|all) instructions|pretend you are|you are (dan|jailbroken)|reveal your (system|prompt)|system prompt:|exfiltrate (credentials|keys|secrets)",
    re.I,
)
for slug, d in data.items():
    body = d["body"]
    ...
    if not any(any(k in h for k in STRUCT_KW) for h in heads):
        # accept if body has any markdown heading at all (structured enough)
        if not heads:
            struct_less.append(slug)
    for line in body.splitlines():
        if INJ.search(line):
            safety.append((slug, "INJECTION", line.strip()[:120]))
        # destructive op only if NOT near an approval/gate word
...
with open(os.path.join(DOCS, "skill-resolution-audit.md"), "w", encoding="utf-8") as f:
    ...
    for s, dep in tool_issues:
        f.write(f"- {s}: {dep}\n")
    f.write(f"\n## prompt: dependency issues: {len(prompt_issues)}\n")
    for s, dep, note in prompt_issues:
        f.write(f"- {s}: `{dep}` — {note}\n")
