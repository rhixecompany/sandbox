...
TEMPLATES = HERMES_HOME / "skills/development/vscode-workspace-configurator/templates"
...
async def main():
    ...
    # load templates
    tpl_next = await loop.run_in_executor(None, _load_json_sync, TEMPLATES / "settings-nextjs.json") or {}
    await loop.run_in_executor(None, _load_json_sync, TEMPLATES / "settings-django.json") or {}
    await loop.run_in_executor(None, _load_json_sync, TEMPLATES / "settings-node.json") or {}
    ...
    build_task_django = {
        ...
        "label": "Build: collectstatic+migrate",
        "type": "shell",
        "group": "build",
    }
    ...
    for repo_name in sorted([p.name for p in PROJECTS.iterdir() if p.is_dir()]):
        ...
        if repo_name in add_launch:
            ...
            if pkg.exists():
                ...
                j = load_json(pkg)
                deps = (
                    " ".join(list(j.get("dependencies", {}).keys()) + list(j.get("devDependencies", {}).keys())).lower()
                    if j
                    else ""
                )
                if "next" in deps:
        ...
        if repo_name in merge_dual:
            ...
            for k, v in tpl_next.items():
                if k.startswith("[") and k not in cur_settings:
                    cur_settings[k] = v
                    modified = True
            if modified:
