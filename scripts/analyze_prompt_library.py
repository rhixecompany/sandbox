    ...
    for dep in deps:
        ...
        if low.startswith("skill:"):
            ...
            if ref.startswith("mcp-"):
                server = ref[4:]
                if server not in KNOWN_MCP:
                    unknown_mcp.append(ref)
...
B = []
INJ = re.compile(
    r"ignore (previous|prior|above) instructions|pretend you are|you are (dan|jailbreak)|reveal your (system|prompt)|exfiltrate|system prompt",
    re.I,
)
...
for slug, d in data.items():
    ...
    for line in body.splitlines():
        if INJ.search(line):
            crit.append(("CRITICAL/injection", line.strip()[:120]))
...
with open(os.path.join(DOCS, "skill-resolution-audit.md"), "w", encoding="utf-8") as f:
    ...
    if not A:
        f.write(
            "RESULT: **0 issues.** Every `skill:` reference resolves to a real SKILL.md; every `tool:mcp-*` is a known-good MCP server; no toolset mislabeled as a tool.\n"
        )
    else:
        ...
        for slug, un, um, ml in A:
            f.write(f"- **{slug}**\n")
            if un:
                f.write(f"  - UNRESOLVED skill refs: {un}\n")
            if um:
                f.write(f"  - UNKNOWN MCP server: {um}\n")
            if ml:
                f.write(f"  - MISLABELED tool: (toolset used as tool:) {ml}\n")
...
with open(os.path.join(DOCS, "prompt-registry.md"), "w", encoding="utf-8") as f:
    ...
    f.write(
        "- Loaded all 10 prompt-related skills (prompt-verification, prompt-library-maintenance, fix-prompt-frontmatter, prompt-batch-fixer, prompt-management, validate-prompts, fix-prompts, audit-prompts, boost-prompts, ai-prompt-engineering-safety-review).\n"
    )
    f.write("- Schema + integrity verified across all 211 files (independent verifier, 0 issues).\n")
    f.write(
        "- Fixed 36 files: 22 `trigger:` → `/slug`; 3 `name:` synced to filename; 13 bidirectional DEPS==SKILLS; 8 toolset normalizations.\n"
    )
    f.write(
        "- Repaired 2 corrupted files: `web-research-pipeline` (MCP server relabeled `skill:mcp-fetch`→`tool:mcp-fetch`, self-ref `skill:web-research-pipeline` removed) and `update-implementation-plan` (duplicated frontmatter block collapsed to one).\n"
    )
