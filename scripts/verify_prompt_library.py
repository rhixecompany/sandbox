...
def verify(path):
    ...
    # duplicate metadata blocks (text-level)
    ...
    if dep_skills != skills:
        ...
        if only_deps:
            issues.append(f"SKILL_DEP_ONLY:{sorted(only_deps)}")
        if only_skills:
            issues.append(f"SKILL_LIST_ONLY:{sorted(only_skills)}")
    # mcp mislabels
    for d in deps:
    ...
    if bad:
        issues.append(f"BAD_TOOLSET:{bad}")
    # mcp dep without mcp toolset
