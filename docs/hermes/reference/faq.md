...
Set your provider with `hermes model` or by editing `~/./.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Hermes Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/./`.
...
**Solution:** Hermes auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/./config.yaml`:
...
:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/./.env` for conflicting entries.
:::
...
- Check gateway logs: `cat ~/./logs/gateway.log | tail -50`
...
Configure in `~/./config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/./hermes-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
nohup hermes gateway run > ~/./logs/gateway.log 2>&1 &
```
...
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai$HERMES_HOME.gateway.plist
```
...
Verify your `~/./config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
There is no hard limit. Each profile is just a directory under `~/./profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Hermes can route subagents to a different model automatically. Set this in `~/./config.yaml`:
...
2. On the **source machine**, create a full backup:
   ...
   hermes backup
   ```
   This creates a zip of your entire `~/./` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/hermes-backup-<timestamp>.zip`.
...
| **Scope** | Global (entire `~/$HERMES_HOME` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='hermes-agent' ~/./ newmachine:~/./
```
