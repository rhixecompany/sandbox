...
2. **context** — caller-supplied `system_message`, project context files (`$HERMES_HOME.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`), then the worktree-dependent git workspace snapshot, operator instructions and platform hints
...
```
# Layer 1: Agent Identity (from ~/./SOUL.md)
...
`SOUL.md` lives at `~/./SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hermes_md(cwd_path)       # 1. $HERMES_HOME.md / HERMES.md (walks to git root)
...
|----------|-------|-------------|-------|
| 1 | `$HERMES_HOME.md`, `HERMES.md` | CWD up to git root | Hermes-native project config |
...
- **YAML frontmatter stripped** — `$HERMES_HOME.md` frontmatter is removed (reserved for future config overrides)
...
1. `$HERMES_HOME.md` / `HERMES.md` (walks to git root)
...
- `~/./SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/./MEMORY.md` and `~/./USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `$HERMES_HOME.md`, `HERMES.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
