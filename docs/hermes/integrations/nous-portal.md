...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/./auth.json` is the only credential on disk, and Hermes mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
4. Sets Nous as your inference provider in `~/./config.yaml` (when you pick a model)
...
After `hermes setup --portal`, `~/./config.yaml` will look like:
...
The OAuth refresh token is stored separately at `~/./auth.json` (not in `config.yaml` — credentials and configuration are kept separate by design).
