...
Hermes could not find a usable API key. Add one of these to `~/./.env`:
...
Hermes may hide low-throughput Gemma models from the picker by default. If you intentionally want to evaluate one, set the model ID directly in `~/./config.yaml`.
