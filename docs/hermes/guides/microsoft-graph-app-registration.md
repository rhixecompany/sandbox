...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/./.env` at the end.
...
Put the three values you collected into `~/./.env`:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/./.env` with the new value.
