...
After the first login, credentials are stored under `~/./auth.json` and refreshed automatically before they expire.
...
3. Hermes polls xAI until approval, then saves tokens to `~/./auth.json`.
...
After login, `~/./config.yaml` will contain:
