...
After the first login, credentials are stored under `~/./auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/./.env
```
...
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/./auth.json` under the `minimax-oauth` key.
...
After login, `~/./config.yaml` will contain entries similar to:
