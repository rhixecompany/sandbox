...
| `local` | Write access to `~/./cron/output/` |
...
2. `~/./logs/agent.log` for scheduler messages (or `errors.log` for warnings)
...
```bash
ls ~/./scripts/your-script.py   # Must exist
hermes cron edit <job_id> --script ~/./scripts/your-script.py
```
...
Jobs are stored in `~/./cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/./cron/jobs.json
chmod 600 ~/./cron/jobs.json   # Your user should own it
```
...
2. Check `~/./logs/agent.log` for scheduler messages and `~/./logs/errors.log` for warnings
