...
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/./.env`
...
**Option 2:** Set it manually in `~/./.env`:
...
Control how much detail the bot shows when using tools. In `~/./config.yaml`:
...
Customize how the bot communicates by editing `~/./SOUL.md`:
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u hermes-gateway` (Linux) or `~/./logs/gateway.log` (macOS) |
| Cron job output | `~/./cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/./cron/jobs.json` |
