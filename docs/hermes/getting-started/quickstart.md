...
# ~/./hermes-agent on Linux/macOS or %LOCALAPPDATA%\hermes\hermes-agent on Windows):
cd ~/./hermes-agent
...
Hermes ships with a catalog of bundled skills already installed in `~/./skills/`. You can add more from the Skills Hub, or write your own.
...
(If you installed without `[all]`, run `cd ~/./hermes-agent && uv pip install -e ".[acp]"` first.)
