...
description: "Bitwarden Secrets Manager documentation"
sidebar_position: 99
---

# Bitwarden Secrets Manager

Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/./.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. Hermes stores that single token in `~/./.env` as `BWS_ACCESS_TOKEN`.
3. Every time `hermes` (or the gateway, or a cron job) starts, after `~/./.env` has loaded, Hermes calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
...
The `bws` binary is auto-downloaded into `~/./bin/` on first use — no `apt`, no `brew`, no `sudo`.
...
1. Download and verify `bws v2.0.0` into `~/./bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/./.env` as `BWS_ACCESS_TOKEN`.
...
| `encrypted_cache.enabled` | `false` | Store the last successful fetch in an AES-GCM encrypted cache at `~/./cache/bws_cache.enc.json`. |
...
| `auto_install` | `true` | When true, `bws` is auto-downloaded into `~/./bin/` on first use. |
...
- **Single-machine personal setups** where `~/./.env` is fine. You're trading one credential for another and adding a network dependency at startup.
