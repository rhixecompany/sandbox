...
Configure which events the agent sees in `~/./config.yaml` under the Home Assistant platform's `extra` section:
...
The adapter reads credentials from `~/./.env` (auto-merged at startup) or
