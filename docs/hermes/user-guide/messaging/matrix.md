...
Optional behavior settings in `~/./config.yaml`:
...
cd ~/./hermes-agent && uv pip install -e ".[matrix]"
```
...
- Stores encryption keys in `~/./platforms/matrix/store/` (legacy installs: `~/./matrix/store/`)
...
If you delete `~/./platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.
...
2. Delete the local crypto store and restart Hermes:
   ```bash
   rm -f ~/./platforms/matrix/store/crypto.db*
   # restart hermes
   ```
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/./.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/./platforms/matrix/store/crypto.db
   rm -f ~/./platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/./.env`:
...
services:
  hermes-matrix:
    ...
    volumes:
      - ./matrix-store:/root/./platforms/matrix/store
```
...
RUN cd ~/./hermes-agent && uv pip install -e ".[matrix]"
...
**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/./.env` and restart the gateway. Use the full `@user:server` format.
