...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/./photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/./.env`
...
(device token, dashboard project id) lives in `~/./auth.json` under
...
**Pre-authorize specific numbers** (in `~/./.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/./.env`):
