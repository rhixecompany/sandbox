...
1. Generate the manifest. New Slack apps must use Agent view:
   ...
   ```
   This writes `~/./slack-manifest.json` and prints paste-in
...
3. Paste the new contents of `~/./slack-manifest.json`
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/./config.yaml`.
...
- Tokens should be stored in `~/./.env` (file permissions `600`)
