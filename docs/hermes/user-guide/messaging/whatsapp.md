...
The Baileys bridge saves its session under `~/./platforms/whatsapp/session`. This means:
...
| **Session not persisting** | Check that `~/./platforms/whatsapp/session` exists and is writable. If containerized, mount it as a persistent volume. |
...
| **Bot replies to strangers with a pairing code** | Set `whatsapp.unauthorized_dm_behavior: ignore` in `~/./config.yaml` if you want unauthorized DMs to be silently ignored instead. |
...
- The `~/./platforms/whatsapp/session` directory contains full session credentials — protect it like a password
- Set file permissions: `chmod 700 ~/./platforms/whatsapp/session`
