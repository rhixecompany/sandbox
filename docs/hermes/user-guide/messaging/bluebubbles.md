...
description: "BlueBubbles (iMessage) documentation"
sidebar_position: 99
---
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/./config.yaml` (default: `true`). There is no corresponding environment variable.
