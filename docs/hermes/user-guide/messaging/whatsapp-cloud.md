...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/./.env` → restart the gateway.
...
2. Generate a **Verify Token** — the wizard does this for you with `secrets.token_urlsafe(32)`; if you're configuring manually, run:
   ...
   ```
   Save it as `WHATSAPP_CLOUD_VERIFY_TOKEN` in `~/./.env`.
...
In addition to Meta's recipient whitelist, Hermes has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/./.env`:
...
All settings live in `~/./.env`.  Required values are in **bold**.
...
- **Verify token mismatch** — the token in `~/./.env`'s `WHATSAPP_CLOUD_VERIFY_TOKEN` must match exactly what you typed into Meta's dashboard.  Run the curl probe above to confirm the gateway's verify handshake works locally first.
