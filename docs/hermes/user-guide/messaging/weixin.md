...
5. Save the account credentials automatically to `~/./weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/./.env`:
...
- Tokens are saved per account+peer to `~/./weixin/accounts/<account_id>.context-tokens.json`
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `cd ~/./hermes-agent && uv pip install -e ".[messaging]"`. Alternatively, open the URL printed above the QR |
