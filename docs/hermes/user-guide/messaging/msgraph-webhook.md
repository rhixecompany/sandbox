...
- A strong shared secret to use as the `clientState` value. Generate with `openssl rand -hex 32` and put it in `~/./.env` as `MSGRAPH_WEBHOOK_CLIENT_STATE`.
...
Or via env vars in `~/./.env` (auto-merged on startup):
