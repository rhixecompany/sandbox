...
`~/./config.yaml`, `~/./.env`, and even the shell environment — for
...
```text
/etc/hermes/
├── config.yaml     # managed config layer (wins over ~/./config.yaml)
└── .env            # managed env layer (wins over ~/./.env + shell)
```
