...
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/./hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Outbound webhooks](#outbound-webhooks)** | `hooks.outbound:` list in `~/./config.yaml` | CLI + Gateway | Push signed lifecycle events to external HTTP endpoints — CI, dashboards, other agents |
...
Each hook is a directory under `~/./hooks/` containing two files:
...
LOG_FILE = Path.home() / "$HERMES_HOME" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/./hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/./hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / "$HERMES_HOME" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/./hooks/session-webhook/HOOK.yaml
...
```python
# ~/./hooks/session-webhook/handler.py
import httpx
...
A popular pattern from the community: drop a Markdown checklist at `~/./BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."
...
1. A file at `~/./BOOT.md` with natural-language startup instructions.
...
Create `~/./BOOT.md`. Write it as if you were giving instructions to a human assistant:
...
**`~/./hooks/boot-md/handler.py`**

```python
"""Run ~/./BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / "$HERMES_HOME" / "BOOT.md"
...
Delete `~/./BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/./hooks/`
...
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/./config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/./agent-hooks/` (by convention) | `~/./plugins/<name>/` | `~/./hooks/<name>/` |
...
hooks:
  pre_tool_call:
    - matcher: "terminal|write_file|patch"
      command: "~/./agent-hooks/secret-scan.sh"
      timeout: 10
      fail_closed: true
```
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/./agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/./agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/./agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/./agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/./agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/./agent-hooks/inject-cwd-context.sh
...
hooks:
  subagent_stop:
    - command: "~/./agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/./agent-hooks/log-orchestration.sh
log=~/./logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Hermes sees it, then persists the decision to `~/./shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.
...
Manual allowlisting is useful for non-TTY or service-account deployments where an operator cannot answer the first-use prompt interactively. The allowlist file is `~/./shell-hooks-allowlist.json`, and the expected format is an `approvals` array. Each approval records the hook `event` and the exact `command` string:
...
```json
{
  "approvals": [
    {
      "event": "post_llm_call",
      "command": "/home/hermes/./hooks/my-hook.py"
    }
  ]
}
```
...
- Keep scripts inside `~/./agent-hooks/` so the path is easy to audit.
...
Add a `hooks.outbound:` list to `~/./config.yaml`:
...
Secrets: prefer `secret_env` (the name of an environment variable, typically set in `~/./.env`) over an inline `secret:` literal, so the config file stays free of credentials. Entries without a secret are delivered unsigned (flagged as `UNSIGNED` by `hermes hooks list`).
