...
2. **An LLM provider configured** — run `hermes model` or set your preferred provider credentials in `~/./.env`
3. **A working base setup** — run `hermes` to verify the agent responds to text before enabling voice

:::tip
The `~/./` directory and default `config.yaml` are created automatically the first time you run `hermes`. You only need to create `~/./.env` manually for API keys.
:::
...
cd ~/./hermes-agent && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/./hermes-agent && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/./hermes-agent && uv pip install -e ".[tts-premium]"
...
Voice mode is available in both the **classic CLI** (`hermes chat`) and the **TUI** (`hermes --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/./logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
:::tip
To disable the mention requirement in server channels, add to `~/./.env`:
```bash
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/./.env`
