...
persona in `~/./SOUL.md`, or as a named preset under
`agent.personalities` in `~/./config.yaml` (switchable per session
via `/personality`).
...
Install MCP servers via `~/./config.yaml` under the `mcp_servers`
