...
/skin mytheme        # switch to a custom skin from ~/./skins/mytheme.yaml
```

Or set the default skin in `~/./config.yaml`:
...
Create YAML files under `~/./skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.
...
- Saves directly to `~/./skins/`
- Activates a skin by updating `~/./config.yaml`
...
6. Click **Save** to write the skin YAML to `~/./skins/`.
...
- User skins in `~/./skins/` take precedence over built-in skins with the same name.
