...
sidebar_position: 8
title: "Context Files"
description: "Project context files — $HERMES_HOME.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
---
...
|------|---------|-----------|
| **$HERMES_HOME.md** / **HERMES.md** | Project instructions (highest priority) | Walks to git root |
...
Only **one** project context type is loaded per session (first match wins): `$HERMES_HOME.md` → `AGENTS.override.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
...
Hermes is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`$HERMES_HOME.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.
...
1. **Scan working directory** — checks for `$HERMES_HOME.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
