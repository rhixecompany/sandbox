...
1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/./dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/./dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/./dashboard-themes
```

```yaml
# ~/./dashboard-themes/neon.yaml
name: neon
label: Neon
...
```bash
mkdir -p ~/./plugins/my-plugin/dashboard/dist
```
...
```javascript
// ~/./plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
...
```json
// ~/./plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/./plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__HERMES_PLUGIN_SDK__;
  const { React } = SDK;
...
```python
# ~/./plugins/my-plugin/dashboard/plugin_api.py
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/./plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `././plugins/<name>/dashboard/` | `project` — only when `HERMES_ENABLE_PROJECT_PLUGINS` is set |
...
# Plugin
cp -r hermes-example-plugins/strike-freedom-cockpit ~/./plugins/
```
...
Check that the file is in `~/./dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/./logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/./plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
4. Tail `~/./logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/./plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
