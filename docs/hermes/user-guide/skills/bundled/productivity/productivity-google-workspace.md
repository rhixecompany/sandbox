...
```bash
GSETUP="python ${HERMES_HOME:-$HOME/$HERMES_HOME}/skills/productivity/google-workspace/scripts/setup.py"
```
...
- Token is stored at `~/./google_token.json` and auto-refreshes.
- Pending OAuth session state/verifier are stored temporarily at `~/./google_oauth_pending.json` until exchange completes.
- If `gws` is installed, `google_api.py` points it at the same `~/./google_token.json` credentials file. Users do not need to run a separate `gws auth login` flow.
