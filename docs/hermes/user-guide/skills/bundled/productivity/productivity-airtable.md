...
4. Store the token in `${HERMES_HOME:-~/$HERMES_HOME}/.env` (or via `hermes setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `${HERMES_HOME:-~/$HERMES_HOME}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
