...
```bash
python ~/./skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/./skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
- configure the server in `~/./config.yaml` using the `native-mcp` skill, or
...
The server-building part may be correct while the Hermes config is not. Load the `native-mcp` skill and configure the server in `~/./config.yaml`, then restart Hermes.
