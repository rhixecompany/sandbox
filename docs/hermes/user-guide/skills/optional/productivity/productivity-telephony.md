...
- save provider credentials into `${HERMES_HOME:-~/$HERMES_HOME}/.env`
...
```bash
SCRIPT="$(find ~/./skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
3. Buy it and save it into `${HERMES_HOME:-~/$HERMES_HOME}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `${HERMES_HOME:-~/$HERMES_HOME}/.env`
