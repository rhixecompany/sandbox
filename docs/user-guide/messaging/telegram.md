...
If you have skill, plugin, or built-in commands that should stay visible in Telegram's `/` picker, prioritize them in `~/./config.yaml`:
...
If you'd rather have the **agent itself** handle audio — for diarization, a custom transcription tool, or just archiving the recording — set `stt.enabled: false` in `~/./config.yaml`:
...
```
[The user sent a voice message: /home/<user>/./cache/audio/<hash>.ogg]
```
...
Add the URLs under `platforms.telegram.extra` in `~/./config.yaml`:
...
You should see a `[Telegram] Cached user voice at /home/<user>/./cache/audio/...` line and **no** "too large" rejection. Combined with `stt.enabled: false` (above), the path to the original audio file then lands in the agent's inbound message for downstream processing.
...
Add topics under `platforms.telegram.extra.dm_topics` in `~/./config.yaml`:
...
Add topic bindings under `platforms.telegram.extra.group_topics` in `~/./config.yaml`:
...
| Voice messages not transcribed | Verify STT is available: install `faster-whisper` for local transcription, or set `GROQ_API_KEY` / `VOICE_TOOLS_OPENAI_KEY` in `~/./.env`. |
...
Configure the response timeout via `agent.clarify_timeout` in `~/./config.yaml` (default `600` seconds). If you don't respond within the timeout, the agent unblocks with a sentinel message and adapts rather than hanging.
