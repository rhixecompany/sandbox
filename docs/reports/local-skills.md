...
│ audit-plans               │                      │ local  │ local │ enabled │
│ batch-skills-audit        │                      │ local  │ local │ enabled │
│ binance-python-api-tutor… │                      │ local  │ local │ enabled │
│ boost-prompt              │                      │ local  │ local │ enabled │
│ boost-prompts             │                      │ local  │ local │ enabled │
│ bun-nextjs                │                      │ local  │ local │ enabled │
│ bun-shell                 │                      │ local  │ local │ enabled │
│ ci-cd-best-practices      │                      │ local  │ local │ enabled │
│ ci-cd-pipeline-builder    │                      │ local  │ local │ enabled │
│ dispatching-parallel-age… │                      │ local  │ local │ enabled │
│ django-application        │                      │ local  │ local │ enabled │
│ django-celery             │                      │ local  │ local │ enabled │
│ drawio-skill              │                      │ local  │ local │ enabled │
│ emblem-ai-prompt-examples │                      │ local  │ local │ enabled │
│ enhance-prompt            │                      │ local  │ local │ enabled │
│ firecrawl-agent           │                      │ local  │ local │ enabled │
│ firecrawl-crawl           │                      │ local  │ local │ enabled │
│ firecrawl-download        │                      │ local  │ local │ enabled │
│ firecrawl-map             │                      │ local  │ local │ enabled │
│ firecrawl-scrape          │                      │ local  │ local │ enabled │
│ firecrawl-search          │                      │ local  │ local │ enabled │
│ fix-prompt-frontmatter    │                      │ local  │ local │ enabled │
│ git-patch-management      │                      │ local  │ local │ enabled │
│ github                    │                      │ local  │ local │ enabled │
│ github-repo-management    │                      │ local  │ local │ enabled │
│ hermes-personality-soul   │                      │ local  │ local │ enabled │
│ implementation-plan       │                      │ local  │ local │ enabled │
│ introspection-only-gener… │                      │ local  │ local │ enabled │
│ joyride                   │                      │ local  │ local │ enabled │
│ litprog-skill             │                      │ local  │ local │ enabled │
│ mcp-coding-agent-setup    │                      │ local  │ local │ enabled │
│ no-git-delete             │                      │ local  │ local │ enabled │
│ no-net-fetch              │                      │ local  │ local │ enabled │
│ notion-spec-to-implement… │                      │ local  │ local │ enabled │
│ paypal-how-it-works       │                      │ local  │ local │ enabled │
│ project-consolidation     │                      │ local  │ local │ enabled │
│ prompt-engineering-patte… │                      │ local  │ local │ enabled │
│ prompt-library-consolida… │                      │ local  │ local │ enabled │
│ prompt-library-maintenan… │                      │ local  │ local │ enabled │
│ prompt-verification       │                      │ local  │ local │ enabled │
│ python-package-manager    │                      │ local  │ local │ enabled │
│ session-audit             │                      │ local  │ local │ enabled │
│ shop-app                  │                      │ local  │ local │ enabled │
│ simplify                  │                      │ local  │ local │ enabled │
│ skill-library-audit       │                      │ local  │ local │ enabled │
│ skills-tools-preflight-c… │                      │ local  │ local │ enabled │
│ smithery-ai-cli           │                      │ local  │ local │ enabled │
│ test-providers-models     │                      │ local  │ local │ enabled │
│ vscode-config-audit       │                      │ local  │ local │ enabled │
│ youtube-full              │                      │ local  │ local │ enabled │
│ distributed-llm-pretrain… │ .restore-backups     │ local  │ local │ enabled │
│ fine-tuning-with-trl      │ .restore-backups     │ local  │ local │ enabled │
...
│ sparse-autoencoder-train… │ .restore-backups     │ local  │ local │ enabled │
...
│ sequential-orchestration  │ autonomous-ai-agents │ local  │ local │ enabled │
...
│ face-obscure-video-chat   │ creative             │ local  │ local │ enabled │
...
│ vsdc-video-masking        │ creative             │ local  │ local │ enabled │
│ web-artifacts-builder     │ creative             │ local  │ local │ enabled │
│ writing-clearly-and-conc… │ creative             │ local  │ local │ enabled │
│ -agents-fix-discover      │ development          │ local  │ local │ enabled │
│ -agents-fix-report        │ development          │ local  │ local │ enabled │
│ -verify-and-dedupe        │ development          │ local  │ local │ enabled │
│ add-hooks-to-config       │ development          │ local  │ local │ enabled │
│ agents-system-prompt-con… │ development          │ local  │ local │ enabled │
│ ai-prompt-engineering-sa… │ development          │ local  │ local │ enabled │
│ analyze-hf-models         │ development          │ local  │ local │ enabled │
│ analyze-skill-codeblocks  │ development          │ local  │ local │ enabled │
│ apply-pending-memory      │ development          │ local  │ local │ enabled │
│ apply-pending-skills      │ development          │ local  │ local │ enabled │
│ apply-vscode-customizati… │ development          │ local  │ local │ enabled │
│ asyncio-9min-youtube      │ development          │ local  │ local │ enabled │
│ asyncio-complete-guide    │ development          │ local  │ local │ enabled │
│ asyncio-full-tutorial     │ development          │ local  │ local │ enabled │
│ asyncio-geeksforgeeks     │ development          │ local  │ local │ enabled │
│ asyncio-hands-on-walkthr… │ development          │ local  │ local │ enabled │
│ asyncio-part1-basics      │ development          │ local  │ local │ enabled │
│ asyncio-patricks-blog     │ development          │ local  │ local │ enabled │
│ audit-227                 │ development          │ local  │ local │ enabled │
│ audit-227-cjs             │ development          │ local  │ local │ enabled │
│ audit-227-js              │ development          │ local  │ local │ enabled │
│ audit-agents-hashes       │ development          │ local  │ local │ enabled │
│ audit-fix-remediate-safe  │ development          │ local  │ local │ enabled │
│ audit-prompts             │ development          │ local  │ local │ enabled │
│ audit-vscode-config       │ development          │ local  │ local │ enabled │
│ audit-vscode-config-v2    │ development          │ local  │ local │ enabled │
│ batch-delete-profiles     │ development          │ local  │ local │ enabled │
│ batch-localappdata-skill… │ development          │ local  │ local │ enabled │
│ batch-remediate           │ development          │ local  │ local │ enabled │
│ batch-remediate-42-59     │ development          │ local  │ local │ enabled │
│ batch-rewrite-worst       │ development          │ local  │ local │ enabled │
│ batch-skill-judge         │ development          │ local  │ local │ enabled │
│ batch-update-hermes-paths │ development          │ local  │ local │ enabled │
│ benchmark-models          │ development          │ local  │ local │ enabled │
│ benchmark-providers       │ development          │ local  │ local │ enabled │
│ benchmark-results         │ development          │ local  │ local │ enabled │
│ benchmark-run-20260710    │ development          │ local  │ local │ enabled │
│ binance-api-reference     │ development          │ local  │ local │ enabled │
│ boost-near-pass-refs      │ development          │ local  │ local │ enabled │
│ build-path-mapping        │ development          │ local  │ local │ enabled │
│ busha-api-quickstart      │ development          │ local  │ local │ enabled │
│ busha-business-api        │ development          │ local  │ local │ enabled │
│ busha-quick-start-guide   │ development          │ local  │ local │ enabled │
│ categorize-skills         │ development          │ local  │ local │ enabled │
│ check-handle              │ development          │ local  │ local │ enabled │
│ chrome-devtools           │ development          │ local  │ local │ enabled │
│ configure-hermes          │ development          │ local  │ local │ enabled │
│ consolidate-skills        │ development          │ local  │ local │ enabled │
...
│ copilot-mcp-server        │ development          │ local  │ local │ enabled │
...
│ create-missing-memories   │ development          │ local  │ local │ enabled │
│ create-missing-souls      │ development          │ local  │ local │ enabled │
│ create-readme             │ development          │ local  │ local │ enabled │
│ crypto-wallet-api-guide   │ development          │ local  │ local │ enabled │
│ crypto-wallet-quickstart  │ development          │ local  │ local │ enabled │
│ cryptoapis-wallet-builder │ development          │ local  │ local │ enabled │
│ dedupe-skills             │ development          │ local  │ local │ enabled │
│ dev-init-code-samples     │ development          │ local  │ local │ enabled │
│ docs-inventory-report     │ development          │ local  │ local │ enabled │
│ execute-workflow          │ development          │ local  │ local │ enabled │
│ face-detection-uv4l-pi    │ development          │ local  │ local │ enabled │
│ find-lock                 │ development          │ local  │ local │ enabled │
│ find-locking-pid          │ development          │ local  │ local │ enabled │
│ fix-copilot-frontmatter   │ development          │ local  │ local │ enabled │
│ fix-crlf-frontmatter      │ development          │ local  │ local │ enabled │
│ fix-duplicate-hooks       │ development          │ local  │ local │ enabled │
│ fix-eslint-mismatches     │ development          │ local  │ local │ enabled │
│ fix-fail-skills           │ development          │ local  │ local │ enabled │
│ fix-frontmatter           │ development          │ local  │ local │ enabled │
│ fix-frontmatter-yaml      │ development          │ local  │ local │ enabled │
│ fix-frontmatter-yaml-cjs  │ development          │ local  │ local │ enabled │
│ fix-orphaned-brackets     │ development          │ local  │ local │ enabled │
│ fix-prompts               │ development          │ local  │ local │ enabled │
│ fix-prompts-comprehensive │ development          │ local  │ local │ enabled │
│ fix-skills                │ development          │ local  │ local │ enabled │
│ fix-yaml-frontmatter      │ development          │ local  │ local │ enabled │
│ flutterwave-transfers-api │ development          │ local  │ local │ enabled │
│ generate-context-files    │ development          │ local  │ local │ enabled │
│ generate-custom-instruct… │ development          │ local  │ local │ enabled │
│ generate-skills           │ development          │ local  │ local │ enabled │
│ generate-vscode-audit-re… │ development          │ local  │ local │ enabled │
│ generate-vscode-configs   │ development          │ local  │ local │ enabled │
│ hello                     │ development          │ local  │ local │ enabled │
│ hello-world               │ development          │ local  │ local │ enabled │
│ hermes-deep-dive-guide    │ development          │ local  │ local │ enabled │
│ hermes-luma-memory-arch   │ development          │ local  │ local │ enabled │
│ hermes-networkchuck-cour… │ development          │ local  │ local │ enabled │
│ hermes-persistent-memory  │ development          │ local  │ local │ enabled │
│ hermes-quickstart         │ development          │ local  │ local │ enabled │
│ how-paypal-works          │ development          │ local  │ local │ enabled │
│ inventory-agents          │ development          │ local  │ local │ enabled │
│ inventory-prompts         │ development          │ local  │ local │ enabled │
│ lcs                       │ development          │ local  │ local │ enabled │
│ list-cli-tools            │ development          │ local  │ local │ enabled │
│ make-skill-template       │ development          │ local  │ local │ enabled │
│ memory-repair             │ development          │ local  │ local │ enabled │
│ merge-config              │ development          │ local  │ local │ enabled │
│ merge-skill               │ development          │ local  │ local │ enabled │
...
│ model-discovery           │ development          │ local  │ local │ enabled │
│ normalize-plans           │ development          │ local  │ local │ enabled │
│ normalize-prompt-actions  │ development          │ local  │ local │ enabled │
│ patch-all-fail-sections   │ development          │ local  │ local │ enabled │
│ patch-fail-structure      │ development          │ local  │ local │ enabled │
│ paypal-getting-started    │ development          │ local  │ local │ enabled │
│ paystack-dev-docs         │ development          │ local  │ local │ enabled │
│ paystack-getting-started  │ development          │ local  │ local │ enabled │
│ phase4-reconstruct        │ development          │ local  │ local │ enabled │
│ phase4-reconstruct-cjs    │ development          │ local  │ local │ enabled │
│ prime-factors             │ development          │ local  │ local │ enabled │
│ prompt-audit-all          │ development          │ local  │ local │ enabled │
│ prompt-audit-all-cjs      │ development          │ local  │ local │ enabled │
│ prompt-batch-audit        │ development          │ local  │ local │ enabled │
│ prompt-builder            │ development          │ local  │ local │ enabled │
│ prompt-engineering        │ development          │ local  │ local │ enabled │
│ prompt-fix-metadata       │ development          │ local  │ local │ enabled │
│ prompt-inventory          │ development          │ local  │ local │ enabled │
│ prompt-repair-frontmatter │ development          │ local  │ local │ enabled │
│ prompts-repair            │ development          │ local  │ local │ enabled │
│ quarantine-skills         │ development          │ local  │ local │ enabled │
│ remove-flat-duplicates    │ development          │ local  │ local │ enabled │
│ repair-frontmatter-closes │ development          │ local  │ local │ enabled │
│ review-pending-skills     │ development          │ local  │ local │ enabled │
│ score-docs                │ development          │ local  │ local │ enabled │
│ session-audit-227         │ development          │ local  │ local │ enabled │
│ session-audit-227-cjs     │ development          │ local  │ local │ enabled │
│ skill-audit               │ development          │ local  │ local │ enabled │
│ skill-creator             │ development          │ local  │ local │ enabled │
│ skills                    │ development          │ local  │ local │ enabled │
│ skills-audit              │ development          │ local  │ local │ enabled │
│ suggest-awesome-github-c… │ development          │ local  │ local │ enabled │
│ suggest-awesome-github-c… │ development          │ local  │ local │ enabled │
│ test-models               │ development          │ local  │ local │ enabled │
│ trim-banking              │ development          │ local  │ local │ enabled │
│ trim-remaining            │ development          │ local  │ local │ enabled │
│ trim-research-reports     │ development          │ local  │ local │ enabled │
│ trim-research-reports-fi… │ development          │ local  │ local │ enabled │
│ update-agents-md          │ development          │ local  │ local │ enabled │
│ update-implementation-pl… │ development          │ local  │ local │ enabled │
│ using-git-worktrees       │ development          │ local  │ local │ enabled │
│ validate-prompt-inventory │ development          │ local  │ local │ enabled │
│ validate-prompts          │ development          │ local  │ local │ enabled │
│ validate-vscode-configs   │ development          │ local  │ local │ enabled │
│ validate-vscode-json      │ development          │ local  │ local │ enabled │
│ verify-frontmatter        │ development          │ local  │ local │ enabled │
│ verify-frontmatter-cjs    │ development          │ local  │ local │ enabled │
...
│ dependency-security-reme… │ devops               │ local  │ local │ enabled │
│ entra-agent-user          │ devops               │ local  │ local │ enabled │
│ env-path-portability      │ devops               │ local  │ local │ enabled │
...
│ monorepo-pr-workflow      │ devops               │ local  │ local │ enabled │
│ msys2-management          │ devops               │ local  │ local │ enabled │
│ multi-stage-dockerfile    │ devops               │ local  │ local │ enabled │
│ pending-store-apply       │ devops               │ local  │ local │ enabled │
...
│ repo-management           │ devops               │ local  │ local │ enabled │
...
│ windows-installer-diagno… │ devops               │ local  │ local │ enabled │
...
│ mcp-ast-grep              │ mcp                  │ local  │ local │ enabled │
│ mcp-builder               │ mcp                  │ local  │ local │ enabled │
│ mcp-cli                   │ mcp                  │ local  │ local │ enabled │
│ mcp-code-sandbox          │ mcp                  │ local  │ local │ enabled │
│ mcp-codex                 │ mcp                  │ local  │ local │ enabled │
│ mcp-docker                │ mcp                  │ local  │ local │ enabled │
│ mcp-fetch                 │ mcp                  │ local  │ local │ enabled │
│ mcp-filesystem            │ mcp                  │ local  │ local │ enabled │
│ mcp-github                │ mcp                  │ local  │ local │ enabled │
│ mcp-memory                │ mcp                  │ local  │ local │ enabled │
│ mcp-mindstudio            │ mcp                  │ local  │ local │ enabled │
│ mcp-playwright            │ mcp                  │ local  │ local │ enabled │
│ mcp-security-audit        │ mcp                  │ local  │ local │ enabled │
│ mcp-sequential-thinking   │ mcp                  │ local  │ local │ enabled │
│ mcp-smithery              │ mcp                  │ local  │ local │ enabled │
...
│ llm-provider-benchmarking │ mlops                │ local  │ local │ enabled │
│ ml-models                 │ mlops                │ local  │ local │ enabled │
│ note-taking-apps          │ note-taking          │ local  │ local │ enabled │
...
│ prompt-consolidation      │ productivity         │ local  │ local │ enabled │
...
│ batch-skills-remediation  │ qa                   │ local  │ local │ enabled │
...
│ discover-mcp-capabilities │ reference            │ local  │ local │ enabled │
...
│ research-toolkit          │ research             │ local  │ local │ enabled │
│ web-research-pipeline     │ research             │ local  │ local │ enabled │
│ async-migration-patterns  │ software-development │ local  │ local │ enabled │
...
│ executing-prompt-workflo… │ software-development │ local  │ local │ enabled │
│ fluentui-blazor           │ software-development │ local  │ local │ enabled │
│ generator-first-doc-work… │ software-development │ local  │ local │ enabled │
...
│ requesting-code-review    │ software-development │ local  │ local │ enabled │
...
│ spike                     │ software-development │ local  │ local │ enabled │
