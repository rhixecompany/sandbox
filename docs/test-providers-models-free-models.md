# Free Models Table — Phase 4.3 Output

> Generated: 2026-06-22 | Source: OpenRouter live API + provider auth inventory

## OpenRouter Free Models (27 models, $0 pricing confirmed via API)

| # | Provider | Model | Context | Type |
|---|----------|-------|---------|------|
| 1 | cognitivecomputations | dolphin-mistral-24b-venice-edition:free | 32K | Uncensored general |
| 2 | cohere | north-mini-code:free | 256K | Agentic coding (30B MoE, 3B active) |
...
| 22 | openrouter | free (router) | 200K | Auto-router to free models |
...
## Other Provider Free Model Status

| Provider | Auth Status | Free Models Available | Notes |
|----------|------------|----------------------|-------|
| copilot | ⚠️ Rate-limited (429) | GPT-4o mini, Claude Haiku, Gemini Flash (known) | Cooldown ~22m remaining; cannot query catalog |
| huggingface | ✓ Active | HF free inference API models | API returned error on direct query; provider times out |
| nous | ✓ Active | Same catalog as OpenRouter (Claude, GPT, Gemini, DeepSeek, Qwen, etc.) | Portal pricing determines free tier; not directly queryable |
| ollama-cloud | ✓ Active | Ollama cloud models | API not accessible from subprocess (no key exposure) |
| openai-api | ✓ Keys present | GPT-4o mini, GPT-4o (low-cost) | Keys not exported to subprocess env |
| openrouter | ✓ Active (managed) | **27 confirmed free models** (listed above) | Live API verified 2026-06-22 |

## Verification

- [x] OpenRouter 27 free models confirmed via live API with $0 pricing
- [ ] Copilot — rate-limited, catalog not queryable
- [ ] HuggingFace — free inference models exist but not directly enumerable
- [ ] Nous — Portal free tier exists but pricing gated internally
- [ ] Ollama Cloud — models exist but not enumerable from subprocess
- [ ] OpenAI API — keys not accessible from subprocess
