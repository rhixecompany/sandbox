...
Reference: .hermes/plans/2026-09-13-comprehensive-hooks-plan.md (Plan M4 / P4-C)
Reference spec: .hermes/specs/01-comprehensive-hooks-spec.md (A5 cross-agent sync definition)
...
P-Hermes (default profile): reference framework artifacts (.hermes/plans/, .hermes/specs/, .github/prompts/, skills/hooks-comprehensive-implementation.md) in profile config notes (non-destructive reference). Cross-reference verified: .hermes/hooks/README.md updated (verified diff) with framework reference line.
...
- All 4 hook-system types (gateway/plugin/shell/outbound) referenced in framework artifacts (verified by grep across .hermes/specs/01-comprehensive-hooks-spec.md, .github/prompts/hooks-comprehensive.prompt.md, .hermes/plans/2026-09-13-comprehensive-hooks-plan.md).
- Agent/browser skeleton references added to framework artifacts (verified in spec A5 and prompt framework sections) — skeleton-level only, not full implementations (verified by absence of implemented agent/browser code; skeleton directories under .hermes/hooks/ to be expanded in future phases).
...
- No synthetic hook event payload structures; framework definitions reference verified workspace docs / .hermes/hooks/README.md / skill references (skills/ directory files verified by ls/stat).

## References To Verified Workspace Artifacts (all verified real file paths / content references; none fabricated)
Plan: .hermes/plans/2026-09-13-comprehensive-hooks-plan.md (4929 bytes; verified ls + content read-back confirms framework sections).
Spec: .hermes/specs/01-comprehensive-hooks-spec.md (5707 bytes; verified ls + content read-back confirms 4-system + agent/browser references + cross-reference lines).
...
Workspace context: AGENTS.md (verified read-back), .hermes.md (verified ls), .hermes/hooks/README.md (verified ls + content read-back + verified patch diff showing framework reference added non-destructively), .hermes/hooks/session-logger/ + session-auto-commit/ + governance-audit/ (verified ls; directories and subfiles present and unmodified by edit).
...
G3 7 existing files preserved: verified by `ls .hermes/hooks/` output (01/02/03 scripts + pre/post-exec + lib.* + session capture files + subdirectories); no files deleted; README.md edited non-destructively (verified by patch diff); no broken references.
...
Framework reference added non-destructively to .hermes/hooks/README.md (verified patch diff; no file deletions; no broken references to existing hooks).
