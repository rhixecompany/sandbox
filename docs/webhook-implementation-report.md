...
|---|---|---|---|---|
| G1 | SG1 (spec) | `.hermes/specs/webhook-full.md` | ✅ PASS | File exists (12549 chars); sections 1–13; no fabricated session IDs; security invariant §6 present |
| G2 | SG2 (plan) | `.hermes/plans/webhook-execution-plan.md` | ✅ PASS | Milestones M1–M4; sequential; 8 SG defined |
...
| G8 | SG8 (this report + spec update) | `.hermes/specs/webhook-implementation.md` + this file | ✅ PASS (this gate) | 7/7 prior gates verified; open items listed; session log included |
...
```
.hermes/plans/webhook-execution-plan.md
.hermes/specs/webhook-full.md
.hermes/specs/webhook-implementation.md
...
- `INSECURE_NO_AUTH` is NOT configured as any route's live secret. It appears only in documentation/warning text (`docs/webhook-routes-config.md`, `skills/per-route-toolsets.md`, `.hermes/specs/webhook-full.md` §6) describing the dangerous misconfiguration. This is intentional documentation, not a live configuration.

## Open Items (post-completion — user action)

- [ ] Apply `docs/webhook-routes-config.md` route definitions to real `~/.hermes/config.yaml`.
- [ ] Rotate `.env.webhook-example` placeholder secrets → real values in production `~/.hermes/.env`; do NOT commit real secrets.
