# Hermes Agent Project Overrides

Project context overrides per hierarchy: `.hermes.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`

## Profiles

| Profile           | Model                                                    | Provider | Purpose |
| ----------------- | -------------------------------------------------------- | -------- | ------- |
| **default** ⬤     | Verify with `hermes profile list` / `hermes config show` |          |         |
| alexa             | Verify with `hermes profile list` / `hermes config show` |          |         |
| code-architect    | Verify with `hermes profile list` / `hermes config show` |          |         |
| creative-director | Verify with `hermes profile list` / `hermes config show` |          |         |
| exec-assistant    | Verify with `hermes profile list` / `hermes config show` |          |         |
| patient-tutor     | Verify with `hermes profile list` / `hermes config show` |          |         |
| research-analyst  | Verify with `hermes profile list` / `hermes config show` |          |         |

## MCP Servers

Run `hermes mcp list` for current enabled servers.

## Hooks

Run `hermes hooks list` for current active hooks.

## Plugins

Run `hermes plugins list` for current enabled plugins.

## Live Provider/Model State

Use live Hermes config as source of truth:

- `hermes profile list`
- `hermes config show`
- `hermes fallback list`

## Session Startup Reference

1. Read `SESSION_REPORT.md` first
2. Load 5 mandatory skills: using-superpowers, user-communication-preferences, session-audit-report, hermes-profiles, validate-memories
3. Run `hermes profile use <name>` per task type
4. Profile routing: code→architect, research→analyst, design→creative, planning→exec, teaching→tutor, ops→adminbot, general→default

## Multi-File Change Protocol (≥5 files)

See parent `SOUL.md` for the canonical 14-skill stack and protocol.

## File Hierarchy

| File                       | Purpose                           |
| ----------- | --------------------------------- |
| `.hermes.md`               | Hermes-specific project overrides |
| `AGENTS.md`                | General agent guidance            |
| `PROJECT_RULES.md`         | Workspace-level rules             |
| `MASTER_RULES.md`          | Universal agent rules             |
| `CLAUDE.md`                | Claude-specific guidance          |
| `.cursorrules`             | Cursor IDE rules                  |
| `SESSION_REPORT.md`        | Session history                   |
| `HERMES_PROFILE_REPORT.md` | Profile audit                     |

## MCP Servers & Tools

| Priority | MCP Server            | Equivalent Native                                  | Purpose                                       |
| -------- | --------------------- | -------------------------------------------------- | --------------------------------------------- |
| 1        | `filesystem`          | `read_file`, `write_file`, `search_files`, `patch` | File read/write/stat/search ops               |
| 2        | `github`              | `gh` CLI                                           | PR, issues, file contents, repos              |
| 3        | `ast-grep`            | `grep`/`rg`                                        | AST-based code search and replace             |
| 4        | `playwright`          | `browser_*` tools                                  | Browser automation for interactive pages      |
| 5        | `fetch`               | `curl`                                             | HTTP web page content extraction              |
| 6        | `sequential-thinking` | N/A                                                | Structured multi-step reasoning               |
| 7        | `code-sandbox`        | `terminal`                                         | Isolated Node.js sandbox for running code     |
| 8        | `mcp-docker`          | `docker` CLI                                       | Container management + GitHub ops             |
| 9        | `memory`              | N/A                                                | Persistent cross-session memory               |
| 10       | `python-quality`      | Manual                                             | Ruff lint + Pyright typecheck on Python files |
| 11       | `tooling-lint`        | Manual                                             | ESLint, Prettier, markdownlint, cspell        |
| 12       | `tooling-config`      | Manual                                             | pre-commit, git-cliff, .gitignore validation  |

## Enhanced Tooling Config (mcp==2.0.0, ~/myvenv, uv/uvx/uvm)

This workspace uses `~/myvenv` as the canonical Python virtualenv for all tooling MCP servers (`python-quality`, `tooling-config`, `tooling-lint`).

- `mcp==2.0.0` is the pinned MCP server package version
- Python interpreter for tooling servers: `~/myvenv/bin/python`
- Package manager: `uv` / `uvx` / `uvm`
- Environment variables set on tooling servers: `PYTHONPATH=~/myvenv/lib/site-packages`, `MCP_VERSION=2.0.0`, `UV_USE=true`, `UVM_ACTIVE=~/myvenv`
- See `.opencode/opencode.json` (MCP server commands) and `.vscode/mcp.json` (VS Code MCP settings) for the applied references.
- DRY reference: `.omo/templates/myvenv-reference.md`

## Active Hermes profile: default

Other profiles (if any) live under `C:\Users\Alexa\AppData\Local\hermes\profiles\<name>\`. Each profile has its own skills/, plugins/, cron/, and memories/ that affect a different session than this one. Do not modify another profile's skills/plugins/cron/memories unless the user explicitly directs you to.

You are in the Hermes terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags are NOT intercepted here (they print as literal text) — deliver a file by stating its absolute path or URL in plain text. Cron jobs scheduled from this session are LOCAL-ONLY: their output is saved (viewable via cronjob action='list') but is NOT delivered back into this session — there is no live-delivery channel here. If the user wants to be notified when a job runs, the job's `deliver` must target a gateway-connected messaging platform (e.g. `deliver='telegram'` or `'all'`). Do not promise that a `deliver='origin'` or default-deliver cron job will message them in this session.
