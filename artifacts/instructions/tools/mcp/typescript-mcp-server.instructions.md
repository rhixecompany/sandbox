...
- Support completions for better UX using `completable()` wrapper from `@<REDACTED_BLOB>.js`
...
- Use `getDisplayName()` from `@<REDACTED_BLOB>.js` for UI display names
...
import { StreamableHTTPServerTransport } from "@<REDACTED_BLOB>.js";
...
server.registerTool(
	...
	async ({ text }) => {
		...
		const summary = response.content.type === "text" ? response.content.text : "Unable to summarize";
...
```typescript
import { completable } from "@<REDACTED_BLOB>.js";
