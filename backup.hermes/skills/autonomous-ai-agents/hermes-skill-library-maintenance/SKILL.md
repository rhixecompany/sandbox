...
author: Alexa
---## Goal
Use when Audit, plan, and execute large-scale fixes to Hermes skill definitions (metadata, formatting, content). Multi-phase workflow for bulk patching. to accomplish the associated tasks and objectives.
...
### Pitfall 13: `skill_view()` Returns Cached Content — Don't Use for Post-Patch Verification
...
- `hermes-agent-skill-authoring` — Writing and structuring new skills
- `hermes-agent-diagnostics-configuration` — Verifying Hermes setup
