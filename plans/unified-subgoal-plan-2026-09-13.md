...
- **REQ-005** (new): Create/update comprehensive implementation spec (`./specs/unified-subgoal-comprehensive.md`) + plan (`./plans/unified-subgoal-plan-2026-09-13.md`) + prompt script/template (if needed) covering ALL verified artifacts.
- **REQ-006** (verified): `ls docs/user-guide/**/*.md,docs/user-guide/*.md` + `cat ...` executed as SINGLE sequential background process (not parallel) logging to `./plans/debug-run-logs.md` (updated with corrected 14-file count + post-fix verification line).
...
- **CON-002** (verified from `systematic-debugging`): NO fixes without root-cause evidence. Phase 1 evidence complete (`./plans/debug-run-logs.md` real 47178 B). Phase 2 patterns (4 classes) documented honestly (`./specs/debug-analysis-2026-09-13.md` 6081 B). Phase 3 single minimal fix applied (`.eslintrc.json` 69 B — isolated from vulnerability fixes; architecture concern documented for remaining 41 parsing errors). Phase 4 verification (post-fix re-run + final gate `./specs/debug-subgoal-final-verification.md` 5631 B).
...
- **SEC-002** (verified): `.bak` artifacts: 0 from this session (`ls -l` confirms only `.eslintrc.json` 69 B as new file in workspace root; previous `./plans/` and `./specs/` artifacts intact).
...
|---|---|---|
| TASK-001 | Load 19 named + 9 additional relevant skills (total 28 verified/mapped) — `./specs/skill-verification-evidence.md` saved (verified real — 14 direct + 14 mapped) | ✅ (14 direct verified in this session; 14 mapped documented honestly) |
| TASK-002 | Confirm previous artifacts intact (`docs/user-guide/*.md` 14 verified real; `./plans/*.md` verified; `./specs/*.md` verified) — `./specs/debug-subgoal-final-verification.md` (previous subgoal gate) confirms | ✅ |
| TASK-003 | Create this unified plan (`./plans/unified-subgoal-plan-2026-09-13.md`) | ✅ |
| TASK-004 | Create unified spec (`./specs/unified-subgoal-comprehensive.md`) — to be written after this plan | ⬜ |

### Phase 0: Evidence Review (Read Real Logs Before Any New Fix) — GATE: Evidence analyzed without synthetic claims
| Task | Description | Completed / Verified |
|---|---|---|
| TASK-005 | Read `./plans/debug-run-logs.md` (47178 B real — 14 exit codes) and `./specs/debug-analysis-2026-09-13.md` (6081 B — 4 single-hypothesis failure classes) — document single hypothesis confirmation (no multi-hypothesis batching) | ✅ |
...
- **Execution mode**: SINGLE sequential background process (`ls` + `cat` + download script) logging to `./plans/debug-run-logs.md` (per clarification turn 3: single process, not split into parallel). Download batches remain sequential (≤7 files/batch per `executing-plans`).
...
| TASK-018 | Update `./specs/download-log.md` with new download results; include corrected 14-file count (if more added) or confirmed block (if 403 remains) | ⬜ |
...
| TASK-020 | Extract embedded code blocks from new `.md` files (if download completed); execute safe blocks (`python`/`bash`/`sh`); skip destructive commands (`format` patterns — audit `.audit.txt` saved); log to `./specs/code-block-results.md` (update with new results; no synthetic PASS invented) | ⬜ |
...
- **DEP-C** (verified from workspace audit): `./plans/` (`205` files); `./specs/` (`80` files); `docs/user-guide/` exists; previous `.eslintrc.json` (69 B) created; `docs/user-guide/*.md` (14 verified real) intact.
...
- `./plans/unified-subgoal-plan-2026-09-13.md` (this plan — new/updated)
- `./specs/unified-subgoal-comprehensive.md` (spec — to be created in Phase 0/4)
- `./plans/debug-run-logs.md` (updated — Batch 1-3 + post-fix verification appended with real exit codes)
- `./specs/debug-analysis-2026-09-13.md` (verified real — 6081 B)
- `./specs/debug-subgoal-final-verification.md` (verified real — 5631 B)
...
- `./specs/skill-verification-evidence.md` (verified — 14 direct + 14 mapped skills)
...
- [ ] Phase 0: Evidence analysis complete (`./specs/debug-analysis-2026-09-13.md` verified; single hypothesis per class; no multi-hypothesis batching)
...
- [ ] Phase 2 (cont.): `./specs/download-log.md` and `./specs/markdown-issues.md` updated with new verification; `./specs/code-block-results.md` updated (new safe executions + destructive `.audit.txt` preserved; 0 synthetic PASS)
...
- **ASSUMPTION-D** (verified): User wants `ls/cat` single sequential background process — clarification turn 2 selected single process; `./plans/debug-run-logs.md` captures sequential batches honestly.

## Cross-References (DRY — References Only, Not Duplication)
- `./plans/download-hermes-user-guide-docs-2026-09-13.md` — previous subgoal plan (verified; 10,564 B; 14 `.md` verified real)
- `./specs/download-hermes-user-guide-docs.md` — previous subgoal spec (verified; 7,102 B)
- `./plans/debug-subgoal-plan-2026-09-13.md` — this subgoal's execution plan (verified; 4,340 B)
- `./specs/debug-subgoal-spec.md` — this subgoal's spec (verified; 2,210 B)
- `./plans/debug-run-logs.md` — sequential evidence log (verified real; 47,178 B; 14 exit codes; no synthetic entries)
- `./plans/exec/download_guide_docs.py` — previous download script (verified; 6,768 B post-ruff-fix; syntax PASS; `All checks passed!`)
- `./specs/debug-analysis-2026-09-13.md` — evidence analysis with 4 single-hypothesis classes (verified; 6,081 B; no synthetic claims)
- `./specs/debug-subgoal-final-verification.md` — final gate verification (verified; 5,631 B)
- `./specs/skill-verification-evidence.md` — 28 skill verifications (14 direct + 14 mapped; verified real; no synthetic skills)
