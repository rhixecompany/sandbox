---
goal: Migrate all .hermes/**/*.md files to hermes root, consolidate/dedupe, then implement run-all-goals.prompt.md
version: 1.0.0
...
Migrate 1200+ `.hermes/**/*.md` files into the hermes root folder, consolidate and dedupe duplicates, fix conflicts, update all cross-references, then implement `.github/prompts/general/run-all-goals/run-all-goals.prompt.md`.

## 1. Requirements & Constraints

- **REQ-001**: All 1200+ `.hermes/**/*.md` files must be migrated to hermes root
...
- **REQ-004**: Update every affected file that references `.hermes/` paths
- **REQ-005**: Delete `.hermes/` directory ONLY after root is fully updated
...
- GOAL-001: Catalog all 1200 `.hermes/**/*.md` files by subdirectory and content type

| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-001 | Inventory all .hermes/**/*.md files with sizes and content hashes | | 2026-09-14 |
| TASK-002 | Identify duplicate content across files (by hash) | | 2026-09-14 |
| TASK-003 | Map all cross-references to `.hermes/` paths in affected files | | 2026-09-14 |

### Phase 2: Migration & Consolidation
- GOAL-002: Move files from `.hermes/` subdirectories to hermes root

| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-004 | Create root-level directory structure matching .hermes/ subdirs | | 2026-09-14 |
| TASK-005 | Copy all .hermes/**/*.md files to corresponding root locations | | 2026-09-14 |
...
- GOAL-004: Update all files that reference `.hermes/` paths

| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-012 | Update AGENTS.md, CLAUDE.md, .hermes.md, SOUL.md, USER.md, MEMORY.md | | 2026-09-14 |
| TASK-013 | Update all .prompt.md files referencing .hermes/ paths | | 2026-09-14 |
| TASK-014 | Update skill files and SKILL.md references | | 2026-09-14 |
| TASK-015 | Verify no remaining `.hermes/` references in affected files | | 2026-09-14 |

### Phase 5: Delete .hermes/ & Verify
- GOAL-005: Remove .hermes/ directory after verification

| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-016 | Final integrity check on all migrated files | | 2026-09-14 |
| TASK-017 | Delete .hermes/ directory (git-tracked) | | 2026-09-14 |
...
- **ALT-002**: Keep .hermes/ as symlinks to root — rejected, user explicitly wants deletion
...
- **FILE-001**: `.hermes/` directory (source — 1200+ .md files)
...
- **FILE-006**: All `.prompt.md` files referencing `.hermes/` paths

## 6. Testing

- **TEST-001**: Verify all 1200+ files migrated successfully (count check)
- **TEST-002**: Verify no file content lost (hash comparison)
- **TEST-003**: Verify no broken references (grep for .hermes/ in all files)
- **TEST-004**: Verify .env unchanged (size check)
- **TEST-005**: Verify .hermes/ directory deleted
...
- **RISK-001**: Some .hermes/ files may have hardcoded paths that break after migration
- **RISK-002**: Duplicate content consolidation may lose metadata or context
- **RISK-003**: Deleting .hermes/ may break external tools that depend on it
- **ASSUMPTION-001**: All .hermes/ .md files are git-tracked and can be recovered via reflog
- **ASSUMPTION-002**: The workspace is clean (verified: status clean)

## 8. Related Specifications

- Link: `.hermes/specs/run-all-goals-spec.md`
- Link: `.hermes/specs/run-all-goals-five-day-consolidated-spec.md`
- Link: `.hermes/plans/2026-09-14_hermes-root-migration-plan.md` (this plan)
