...
type ComicStatusEnum = "Ongoing" | "Hiatus" | "Completed" | "Dropped" | "Season End" | "Coming Soon";
	...
	devtools(
		persist(
			(set) => ({
				...
				setStatus: (status) => set({ status, page: 1 }, false, "setStatus"),
				toggleGenre: (genreId) =>
					set(
						(s) => ({
				...
				setTypeId: (typeId) => set({ typeId, page: 1 }, false, "setTypeId"),
	...
	devtools(
		(set, get) => ({
			...
			setBookmarks: (bookmarks) =>
				set(
					{
						bookmarks: Object.fromEntries(bookmarks.map((b) => [b.comicId, b])),
						isLoaded: true,
					},
					false,
					"setBookmarks",
	...
	devtools(
		(set, get) => ({
			...
			bulkSetProgress: (entries) =>
				set(
					{
						progress: Object.fromEntries(entries.map((e) => [e.comicId, e])),
					},
					false,
					"bulkSetProgress",
				),
