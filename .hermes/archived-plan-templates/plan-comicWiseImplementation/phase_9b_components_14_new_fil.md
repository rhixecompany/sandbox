		...
		<Card className="@container/card overflow-hidden transition-shadow hover:shadow-md">
			<Link href={`/comics/${comic.slug}`}>
				...
				<CardContent className="p-2">
					...
					{comic.rating !== undefined && <p className="text-xs text-muted-foreground">★ {comic.rating.toFixed(1)}</p>}
				</CardContent>
			</Link>
		</Card>
	);
...
const STATUSES = ["Ongoing", "Hiatus", "Completed", "Dropped", "Season End", "Coming Soon"] as const;

export function ComicFilterBar() {
	const { search, status, setSearch, setStatus, reset } = useComicFiltersStore();
