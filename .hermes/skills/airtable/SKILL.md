...
Automated reasoning and workflow tool for `airtable`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
4. Store the token in `~/AppData/Local/hermes/.env` (or via `hermes setup`):
   ```
...
- **Auth header:** `Authorization: Bearer $AIRTA...KEY`
...
- **`AIRTABLE_API_KEY` flows from `~/AppData/Local/hermes/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
...
- When working with airtable tools or integrations
- When automating or managing airtable tasks
- When troubleshooting airtable configurations
- **Triggers**: "Airtable", "airtable" related operations
...
Perform the airtable operations following the skill instructions.
