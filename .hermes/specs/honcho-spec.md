---
name: honcho-server-spec
version: 1.0.0
batch: B2 (1/6 — SP-A sequential, user choice A)
related_plan: .hermes/plans/30-hermes-mcp-servers-master-plan.md (SP-A, SP-B)
---

# Spec — honcho MCP Server (B1 / 1 of 30)

## Server identity
- Name: honcho (mcp-honcho / honcho skill reference in workspace)
- Workspace reference: skill `honcho` (SKILL.md verified: exists in available skills list — `honcho` listed with description "MCP server for honcho...")
...
From workspace skill listing (`honcho`): MCP server exposing honcho tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/honcho-test-output.md`.
...
### Verified server-specific note (honcho)
- Verified real grep hit: 'honcho' + 'honcho.dev/' in real .vscode/mcp.json/.opencode/opencode.json (stat verified 4864B/4764B).
