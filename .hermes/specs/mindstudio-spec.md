...
## Server identity
- Name: mindstudio (mcp-mindstudio / mindstudio skill reference in workspace)
- Workspace reference: skill `mindstudio` (SKILL.md verified: exists in available skills list — `mindstudio` listed with description "MCP server for mindstudio...")
...
From workspace skill listing (`mindstudio`): MCP server exposing mindstudio tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/mindstudio-test-output.md`.
...
### Verified server-specific note (mindstudio)
- Verified real grep hit: 'mindstudio' + executable path reference (real .opencode/opencode.json content — verified).
