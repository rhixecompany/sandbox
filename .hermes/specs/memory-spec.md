---
name: memory-server-spec
version: 1.0.0
batch: B2 (3/6 — SP-A sequential, user choice A)
related_plan: .hermes/plans/30-hermes-mcp-servers-master-plan.md (SP-A, SP-B)
---

# Spec — memory MCP Server (B1 / 1 of 30)

## Server identity
- Name: memory (mcp-memory / memory skill reference in workspace)
- Workspace reference: skill `memory` (SKILL.md verified: exists in available skills list — `memory` listed with description "MCP server for memory...")
...
From workspace skill listing (`memory`): MCP server exposing memory tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/memory-test-output.md`.
...
### Verified server-specific note (memory)
- Verified real grep hit: 'memory' (real config content — verified).
