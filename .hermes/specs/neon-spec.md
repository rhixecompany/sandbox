---
name: neon-server-spec
version: 1.0.0
batch: B2 (5/6 — SP-A sequential, user choice A)
related_plan: .hermes/plans/30-hermes-mcp-servers-master-plan.md (SP-A, SP-B)
---

# Spec — neon MCP Server (B1 / 1 of 30)

## Server identity
- Name: neon (mcp-neon / neon skill reference in workspace)
- Workspace reference: skill `neon` (SKILL.md verified: exists in available skills list — `neon` listed with description "MCP server for neon...")
...
From workspace skill listing (`neon`): MCP server exposing neon tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/neon-test-output.md`.
...
### Verified server-specific note (neon)
- Verified real grep hit: 'neon' + 'neon.tech/mcp' (verified grep in real config files — real content).
