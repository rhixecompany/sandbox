---
name: tooling-lint-server-spec
version: 1.0.0
batch: B4 (batch 4 of 5 — sequential per clarification A; SP-A verified real; SP-B/SP-C/SP-D/SP-E BLOCKED verified real; SP-F N/A verified)
related_plan: .hermes/plans/30-hermes-mcp-servers-master-plan.md (SP-A, SP-B)
---

# Spec — tooling-lint MCP Server (B1 / 1 of 30)

## Server identity
- Name: tooling-lint (mcp-tooling-lint / tooling-lint skill reference in workspace)
- Workspace reference: skill `tooling-lint` (SKILL.md verified: exists in available skills list — `tooling-lint` listed with description "MCP server for tooling-lint...")
...
From workspace skill listing (`tooling-lint`): MCP server exposing tooling-lint tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/tooling-lint-test-output.md`.
...
### Verified server-specific note (tooling-lint)
