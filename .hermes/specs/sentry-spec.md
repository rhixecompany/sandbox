---
name: sentry-server-spec
version: 1.0.0
batch: B1 (1/6)
related_plan: .hermes/plans/30-hermes-mcp-servers-master-plan.md (SP-A, SP-B)
---

# Spec — sentry MCP Server (B1 / 1 of 30)

## Server identity
- Name: sentry (mcp-sentry / sentry skill reference in workspace)
- Workspace reference: skill `sentry` (SKILL.md verified: exists in available skills list — `sentry` listed with description "MCP server for sentry...")
...
From workspace skill listing (`sentry`): MCP server exposing sentry tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/sentry-test-output.md`.
...
### Verified server-specific note (sentry)
