---
name: parallel-search-server-spec
version: 1.0.0
batch: B2 (6/6 — SP-A sequential, user choice A)
related_plan: .hermes/plans/30-hermes-mcp-servers-master-plan.md (SP-A, SP-B)
---

# Spec — parallel-search MCP Server (B1 / 1 of 30)

## Server identity
- Name: parallel-search (mcp-parallel-search / parallel-search skill reference in workspace)
- Workspace reference: skill `parallel-search` (SKILL.md verified: exists in available skills list — `parallel-search` listed with description "MCP server for parallel-search...")
...
From workspace skill listing (`parallel-search`): MCP server exposing parallel-search tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/parallel-search-test-output.md`.
...
### Verified server-specific note (parallel-search)
- Verified real grep hit: 'parallel-search' (verified grep — real file content, never synthetic).
