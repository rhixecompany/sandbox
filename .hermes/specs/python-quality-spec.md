---
name: python-quality-server-spec
version: 1.0.0
batch: B1 (1/6)
related_plan: .hermes/plans/30-hermes-mcp-servers-master-plan.md (SP-A, SP-B)
---

# Spec — python-quality MCP Server (B1 / 1 of 30)

## Server identity
- Name: python-quality (mcp-python-quality / python-quality skill reference in workspace)
- Workspace reference: skill `python-quality` (SKILL.md verified: exists in available skills list — `python-quality` listed with description "MCP server for python-quality...")
...
From workspace skill listing (`python-quality`): MCP server exposing python-quality tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/python-quality-test-output.md`.
...
### Verified server-specific note (python-quality)
