...
# Spec — tavily MCP Server (B1 / 1 of 30)

## Server identity
- Name: tavily (mcp-tavily / tavily skill reference in workspace)
- Workspace reference: skill `tavily` (SKILL.md verified: exists in available skills list — `tavily` listed with description "MCP server for tavily...")
...
From workspace skill listing (`tavily`): MCP server exposing tavily tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/tavily-test-output.md`.
...
### Verified server-specific note (tavily)
