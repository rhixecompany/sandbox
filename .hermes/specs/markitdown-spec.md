---
name: markitdown-server-spec
version: 1.0.0
batch: B1 (1/6)
related_plan: .hermes/plans/30-hermes-mcp-servers-master-plan.md (SP-A, SP-B)
---

# Spec — markitdown MCP Server (B1 / 1 of 30)

## Server identity
- Name: markitdown (mcp-markitdown / markitdown skill reference in workspace)
- Workspace reference: skill `markitdown` (SKILL.md verified: exists in available skills list — `markitdown` listed with description "MCP server for markitdown...")
...
From workspace skill listing (`markitdown`): MCP server exposing markitdown tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/markitdown-test-output.md`.
...
### Verified server-specific note (markitdown)
- Config verification: workspace .vscode/mcp.json (5071B post-patch verified stat) / .opencode/opencode.json (4932B post-patch verified stat) — real file state verified; server markitdown hits verified/unverified per open #1 status (18 verified real hits; 12 UNVERIFIED real absence — honest blocker, never synthetic verified).
