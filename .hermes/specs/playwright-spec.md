---
name: playwright-server-spec
version: 1.0.0
batch: B1 (1/6)
related_plan: .hermes/plans/30-hermes-mcp-servers-master-plan.md (SP-A, SP-B)
---

# Spec — playwright MCP Server (B1 / 1 of 30)

## Server identity
- Name: playwright (mcp-playwright / playwright skill reference in workspace)
- Workspace reference: skill `playwright` (SKILL.md verified: exists in available skills list — `playwright` listed with description "MCP server for playwright...")
...
From workspace skill listing (`playwright`): MCP server exposing playwright tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/playwright-test-output.md`.
...
### Verified server-specific note (playwright)
