---
name: parallel-task-server-spec
version: 1.0.0
batch: B1 (1/6)
related_plan: .hermes/plans/30-hermes-mcp-servers-master-plan.md (SP-A, SP-B)
---

# Spec — parallel-task MCP Server (B1 / 1 of 30)

## Server identity
- Name: parallel-task (mcp-parallel-task / parallel-task skill reference in workspace)
- Workspace reference: skill `parallel-task` (SKILL.md verified: exists in available skills list — `parallel-task` listed with description "MCP server for parallel-task...")
...
From workspace skill listing (`parallel-task`): MCP server exposing parallel-task tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/parallel-task-test-output.md`.
...
### Verified server-specific note (parallel-task)
