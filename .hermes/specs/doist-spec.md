---
name: doist-server-spec
version: 1.0.0
batch: B1 (1/6)
related_plan: .hermes/plans/30-hermes-mcp-servers-master-plan.md (SP-A, SP-B)
---

# Spec — doist MCP Server (B1 / 1 of 30)

## Server identity
- Name: doist (mcp-doist / doist skill reference in workspace)
- Workspace reference: skill `doist` (SKILL.md verified: exists in available skills list — `doist` listed with description "MCP server for doist...")
...
From workspace skill listing (`doist`): MCP server exposing doist tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
### Verified server-specific note (doist)
