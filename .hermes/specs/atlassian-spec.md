...
# Spec — atlassian MCP Server (B1 / 1 of 30)

## Server identity
- Name: atlassian (mcp-atlassian / atlassian skill reference in workspace)
- Workspace reference: skill `atlassian` (SKILL.md verified: exists in available skills list — `atlassian` listed with description "MCP server for atlassian...")
...
From workspace skill listing (`atlassian`): MCP server exposing atlassian tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/atlassian-test-output.md`.
...
### Verified server-specific note (atlassian)
