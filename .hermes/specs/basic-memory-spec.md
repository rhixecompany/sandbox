---
name: basic-memory-server-spec
version: 1.0.0
batch: B1 (1/6)
related_plan: .hermes/plans/30-hermes-mcp-servers-master-plan.md (SP-A, SP-B)
---

# Spec — basic-memory MCP Server (B1 / 1 of 30)

## Server identity
- Name: basic-memory (mcp-basic-memory / basic-memory skill reference in workspace)
- Workspace reference: skill `basic-memory` (SKILL.md verified: exists in available skills list — `basic-memory` listed with description "MCP server for basic-memory...")
...
From workspace skill listing (`basic-memory`): MCP server exposing basic-memory tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/basic-memory-test-output.md`.
...
### Verified server-specific note (basic-memory)
- Config verification: workspace .vscode/mcp.json (5071B post-patch verified stat) / .opencode/opencode.json (4932B post-patch verified stat) — real file state verified; server basic-memory hits verified/unverified per open #1 status (18 verified real hits; 12 UNVERIFIED real absence — honest blocker, never synthetic verified).
