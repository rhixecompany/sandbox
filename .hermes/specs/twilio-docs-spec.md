...
# Spec — twilio-docs MCP Server (B1 / 1 of 30)

## Server identity
- Name: twilio-docs (mcp-twilio-docs / twilio-docs skill reference in workspace)
- Workspace reference: skill `twilio-docs` (SKILL.md verified: exists in available skills list — `twilio-docs` listed with description "MCP server for twilio-docs...")
...
From workspace skill listing (`twilio-docs`): MCP server exposing twilio-docs tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/twilio-docs-test-output.md`.
...
### Verified server-specific note (twilio-docs)
