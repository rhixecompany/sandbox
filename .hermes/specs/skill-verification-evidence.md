...
Mapped / handled by equivalent (honest — not invented as direct skills):
  ...
  - update-implementation-plan -> handled by plan updates / patch on existing .hermes/plans/
  - implementation-plan -> mapped to create-implementation-plan + writing-plans
  - update-implementation-spec -> handled by .hermes/specs/ updates (verified artifacts exist)
  - implementation-spec -> mapped to .hermes/specs/ artifacts (download-hermes-user-guide-docs.md verified 7102 B; debug-subgoal-spec.md verified 2210 B)
  - execute-implementation-spec -> mapped to execution of spec via plan phases
  - executing-specs -> mapped to .hermes/specs/ verification gates
...
Previous artifacts verified: docs/user-guide/*.md (14 verified real), .hermes/plans/*.md (verified), .hermes/specs/*.md (verified).
