---
name: sequential-thinking-server-spec
version: 1.0.0
batch: B1 (1/6)
related_plan: .hermes/plans/30-hermes-mcp-servers-master-plan.md (SP-A, SP-B)
---

# Spec — sequential-thinking MCP Server (B1 / 1 of 30)

## Server identity
- Name: sequential-thinking (mcp-sequential-thinking / sequential-thinking skill reference in workspace)
- Workspace reference: skill `sequential-thinking` (SKILL.md verified: exists in available skills list — `sequential-thinking` listed with description "MCP server for sequential-thinking...")
...
From workspace skill listing (`sequential-thinking`): MCP server exposing sequential-thinking tools. Specific tool names must come from server spec/docs (open item #1 of master plan — not yet resolved).
...
For each tool name: one minimal script invocation saved to `.hermes/plans/sequential-thinking-test-output.md`.
...
### Verified server-specific note (sequential-thinking)
