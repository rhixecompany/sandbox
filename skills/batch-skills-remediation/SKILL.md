...
- **Corrupt hub cache blocks `repair-official`**: If `hermes skills repair-official --restore --yes all` fails with `UnicodeDecodeError`, one or more files under `~/./skills/.hub/` contain non-UTF8 data (common after interrupted downloads). Fix: `find ~/AppData/Local/hermes/skills/.hub -name "*.lock" -o -name "*.json" | xargs rm -f`, then re-run the repair command.
