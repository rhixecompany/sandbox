...
- Replace `C:\\Users\\<user>\\*` with env-var-derived paths in ALL languages (shell, Python, PS1, BAT, TS). See `references/python-script-path-hardcoding-remediation.md` for the Python-specific pattern — the same `_HOME = os.environ.get("HOME", os.environ.get("USERPROFILE"))` approach applies across languages but Python has special pathlib/os.path nuances.
...
6. **Bump `$ErrorActionPreference` to `"Stop"`** on PowerShell scripts that use color output.
