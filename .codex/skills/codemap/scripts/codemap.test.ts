// @ts-nocheck - External mjs module, types inferred at runtime
...
const { computeFileHash, computeFolderHash, loadState, PatternMatcher, selectFiles } = await import("./codemap.mjs");
...
describe("selectFiles", () => {
	test("respects include and exclude patterns", () => {
		...
		const selected = selectFiles(root, ["src/**/*.ts", "package.json"], ["**/*.test.ts", "node_modules/"], [], []).map(
...
describe("loadState", () => {
	test("migrates legacy cartography state", () => {
		...
		expect(JSON.parse(readFileSync(path.join(slimDir, "codemap.json"), "utf8"))).toEqual(legacyState);
	});
});
