...
app.post("/webhooks/coderabbit/review", validateWebhook, (req, res) => {
	...
	const cwd = pull_request?.base?.repo?.clone_url ? `/tmp/${pull_request.base.repo.name}` : process.cwd();

	const mode = pull_request ? "interactive" : "plain";
	const type = pull_request ? "all" : "uncommitted";

	exec(`"${CR_BIN}" review --mode ${mode} --type ${type} --cwd "${cwd}"`, { cwd }, (error, stdout, stderr) => {
		if (error) {
