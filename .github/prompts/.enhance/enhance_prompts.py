...
def fix_goal_section(body, desc):
    ...
    # If body starts with a heading, insert before it (identical prepend in both branches)
    body = goal_block + "\n\n" + body
    return body
...
def enhance_prompt(path, dry_run=True):
    ...
    original_body = body
    _ = original_body  # placeholder for future diff reporting
    new_fm = dict(fm)
...
def main():
    ...
    print("=== Prompt Enhancement Batch ===\n")
    ...
    for pf in prompt_files:
        ...
        if changes:
            for _, desc in changes:
                change_summary[desc] += 1
            file_changes.append(
                {"file": pf.name, "changes": [c[0] + ": " + c[1] for c in changes], "count": len(changes)}
            )
