...
def categorize(name, desc, body):
    ...
    if re.search(r"fix|repair|patch|sync|migrat|remediat|audit|cleanup", t):
        return "fix-sync"
    if re.search(r"code|program|implement|function|api|script|build", t):
        return "code"
    if re.search(r"research|investigat|search|find|analyz|review|document", t):
        return "research"
    return "general"
...
def enhance(pf, dry=True):
    ...
    if "dependencies" not in fm or fm["dependencies"] is None:
        fm["dependencies"] = []
        changes.append(("FM", "dependencies"))
    if "metadata" not in fm:
        fm["metadata"] = {"hermes": {}}
        changes.append(("FM", "metadata"))
