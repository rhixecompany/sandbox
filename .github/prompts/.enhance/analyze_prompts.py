...
def check_file(path):
    ...
    for dep in deps:
        if isinstance(dep, str) and dep.startswith("skill:"):
            skill_name = dep[6:]
    ...
    if not has_execution and not re.search(r"^#{2,3}\s+Phase\s+\d", body, re.MULTILINE):
    ...
    if "## Rules" in found_sections:
        ...
        if rules_section_match:
            ...
            if (
                "_shared/rules-core" not in rules_text
                and "rules-core" not in rules_text
                and not re.search(r"`templates/_shared/rules-core", rules_text)
            ):
...
def main():
    ...
    print("=== Prompt Library Enhancement Analyzer ===\n")
