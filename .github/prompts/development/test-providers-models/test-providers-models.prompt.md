...
## Core Rules (from `_shared/rules-core.md`)
...
6. **No vision gap** — No verified working free model has vision. Do not promote a paid/vision model into the free fallback chain.
7. **429 storm mitigation** — Live `hermes chat` probes rate-limit fast. Delegate to subagents (parallel, isolated) and let each fall back to the baseline on error.
8. **String-encoded lists fix** — `hermes config set fallback_providers '[...]'` may serialize as a string; verify and repair via terminal Python.
9. **Provider alias mismatch** — `fallback_providers` uses provider *names* (opencode-zen, openrouter, …), not the `providers:` dict keys (e.g. `ollama-launch`). Set each provider's `default_model` so the right free model is used.
10. **Root vs profiles drift** — Root config may diverge from named profiles; align root, then propagate with `scripts/sync_profile_configs.py`.
11. **Stale `:free` promotions** — Models expire; always re-probe before presenting as working.
...
AUTHORIZED PROVIDERS (hermes auth list, 2026-09-07):
...
Result for the 2026-08-07 verified set (no vision anywhere → reasoning → context):
...
> Config guard: `write_file`/`patch` refuse to edit `config.yaml`. Use `hermes config set` (CLI) or terminal Python (`yaml` + `open()`). If `fallback_providers` is stored as a string instead of a list, fix it with a terminal Python one-liner before verifying.
