
┌─────────────────────────────────────────────────────────┐
│              ⚕ Hermes Configuration                    │
└─────────────────────────────────────────────────────────┘

◆ Paths
  Config:       C:\Users\Alexa\AppData\Local\hermes\config.yaml
  Secrets:      C:\Users\Alexa\AppData\Local\hermes\.env
  Install:      C:\Users\Alexa\AppData\Local\hermes\hermes-agent

◆ API Keys
  OpenRouter     sk-o...2431
  OpenAI (STT/TTS) (not set)
  Exa            (not set)
  Parallel       (not set)
  Firecrawl      (not set)
  Tavily         tvly...V8Eu
  Perplexity     (not set)
  Browserbase    (not set)
  Browser Use    (not set)
  FAL            (not set)
  Anthropic      (not set)

◆ Model
  Model:        {'base_url': 'https://openrouter.ai/api/v1', 'default': 'thinkingmachines/inkling:free', 'provider': 'openrouter', 'api_mode': 'chat_completions'}
  Max turns:    150

◆ Display
  Personality:  teacher
  Reasoning:    on
  Bell:         complete=on, prompt=on
  User preview: first 2 line(s), last 2 line(s)

◆ Terminal
  Backend:      local
  Working dir:  
  Timeout:      360s

◆ Timezone
  Timezone:     Africa/Lagos

◆ Context Compression
  Enabled:      yes
  Threshold:    90%
  Target ratio: 20% of threshold preserved
  Protect last: 40 messages
  Protect first: 6 non-system head messages
  Model:        (auto)

◆ Messaging Platforms
  Telegram:     configured
  Discord:      not configured

◆ Skill Settings
  hermes.skill.skills.enabled true  [hermes-skills]

────────────────────────────────────────────────────────────
  hermes config edit     # Edit config file
  hermes config set <key> <value>
  hermes setup           # Run setup wizard

