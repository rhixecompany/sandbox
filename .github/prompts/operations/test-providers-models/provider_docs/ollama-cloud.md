...
- **Auth file verified:** `~/AppData/Local/hermes/auth.json (provider=ollama-cloud)`
- **Auth mechanism:** api_key
- **Inference endpoint:** `https://api.ollama.com/v1`
...
- `~/AppData/Local/hermes/config.yaml` should reference this provider's model
  format (`ollama-cloud:<model-id>`) for the primary model and any fallbacks.
