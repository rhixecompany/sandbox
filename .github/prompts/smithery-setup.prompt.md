...
- [1. Log in to Smithery](#1-log-in-to-smithery)
- [2. Connect to the Exa search server](#2-connect-to-the-exa-search-server)
...
- [→ https://auth.smithery.ai/...](#→-https://authsmitheryai/)
- [Visit the URL to authorize](#visit-the-url-to-authorize)
- [2. List a User's Connections](#2-list-a-user's-connections)
  - [3. Use Tools Across Connections](#3-use-tools-across-connections)
- [List tools for a connection](#list-tools-for-a-connection)
- [Call a tool](#call-a-tool)
- [Namespace MCP Endpoint](#namespace-mcp-endpoint)
...
- [Connect to a server that requires OAuth](#connect-to-a-server-that-requires-oauth)
- [If auth is required, the CLI outputs the setup URL:](#if-auth-is-required-the-cli-outputs-the-setup-url:)
- [→ auth_required](#→-auth_required)
- [→ https://auth.smithery.ai/...](#→-https://authsmitheryai/)
- [→ connection_id: abc-123-github](#→-connection_id:-abc-123-github)
- [Visit the URL to authorize, then retry](#visit-the-url-to-authorize-then-retry)
- [After authorization, the connection is ready](#after-authorization-the-connection-is-ready)
- [Handling Configuration (`input_required`)](#handling-configuration-`input_required`)
- [Interactive: CLI prompts for missing fields](#interactive:-cli-prompts-for-missing-fields)
- [Non-interactive: pass config upfront](#non-interactive:-pass-config-upfront)
- [Create — returns input_required if config is missing](#create-—-returns-input_required-if-config-is-missing)
- [Update with required config (same endpoint, PUT is an upsert)](#update-with-required-config-same-endpoint-put-is-an-upsert)
- [Service Tokens](#service-tokens)
- [Use the scoped token to call tools](#use-the-scoped-token-to-call-tools)
- [Advanced](#advanced)
...
- [Uplink](#uplink)
- [Quick Start](#quick-start)
- [Pull a hosted Smithery package and run it locally — no setup required](#pull-a-hosted-smithery-package-and-run-it-locally-—-no-setup-required)
- [Point at an MCP server already running locally](#point-at-an-mcp-server-already-running-locally)
- [Let the CLI spawn and manage a stdio MCP server](#let-the-cli-spawn-and-manage-a-stdio-mcp-server)
- [Call it like any other connection](#call-it-like-any-other-connection)
- [How it works](#how-it-works)
- [Connection lifecycle](#connection-lifecycle)
- [One tunnel per connection](#one-tunnel-per-connection)
- [Security](#security)
- [Limitations](#limitations)
- [Token Scoping](#token-scoping)
- [Scope a Token to a User](#scope-a-token-to-a-user)
- [Multi-Level Access (Workspace / Org)](#multi-level-access-workspace-/-org)
- [Narrow a Token](#narrow-a-token)
- [Narrow the broad token to just one user's connections](#narrow-the-broad-token-to-just-one-user's-connections)
- [Narrow the broad token to just one user's connections](#narrow-the-broad-token-to-just-one-user's-connections)
- [Operation Scoping](#operation-scoping)
  - [Read-Only Dashboard Token](#read-only-dashboard-token)
  - [Execute-Only Agent Token](#execute-only-agent-token)
  - [Multi-Resource Token](#multi-resource-token)
- [Constraint Reference](#constraint-reference)
- [Two grants: read alice's connections OR read/write servers](#two-grants:-read-alice's-connections-or-read/write-servers)
- [Two grants: read alice's connections OR read/write servers](#two-grants:-read-alice's-connections-or-read/write-servers)
- [Security Best Practices](#security-best-practices)
- [Deep Linking](#deep-linking)
- [Protocol Specification](#protocol-specification)
- [Example Configurations](#example-configurations)
  - [stdio-based Configuration:](#stdio-based-configuration:)
  - [HTTP-based Configuration:](#http-based-configuration:)
- [Handling Deep links](#handling-deep-links)
  - [Deeplink Handler](#deeplink-handler)
  - [Stdio Example](#stdio-example)
  - [HTTP Example](#http-example)
- [Listing Your Client](#listing-your-client)
- [Benefits of Getting Listed](#benefits-of-getting-listed)
- [How to Get Listed](#how-to-get-listed)
- [Attribution Requirements](#attribution-requirements)
  - [Option 1: Registry API Integration](#option-1:-registry-api-integration)
  - [Option 2: Backlink Integration](#option-2:-backlink-integration)
...
> ## Documentation Index
> Fetch the complete documentation index at: [https://smithery.ai/docs/llms.txt](https://smithery.ai/docs/llms.txt)
...
```text theme={null}
[https://mcp.smithery.run/{namespace}](https://mcp.smithery.run/{namespace})
```
...
```bash theme={null}
smithery mcp add [http://localhost:9090/mcp](http://localhost:9090/mcp) --id chrome --force
```
...
> Deep links provide a smooth way to integrate Smithery MCPs into supported clients.
...
<img src="[https://mintcdn.com/smithery/VM0SEWDjb0U0Bzut/images/magic_link_flow.png?fit=max&auto=format&n=VM0SEWDjb0U0Bzut&q=85&s=a26e6841e4ec089cdaebe632ccf23868"](https://mintcdn.com/smithery/VM0SEWDjb0U0Bzut/images/magic_link_flow.png?fit=max&auto=format&n=VM0SEWDjb0U0Bzut&q=85&s=a26e6841e4ec089cdaebe632ccf23868") alt="Magic Link Integration Flow" width="2304" height="1728" data-path="images/magic_link_flow.png" />
...
| url | URL of the MCP server (required for http type) | `"[https://exa.run.tools"`](https://exa.run.tools"`) |
...
{
"type": "http",
"url": "[https://exa.run.tools"](https://exa.run.tools")
}
```
...
<img src="[https://mintcdn.com/smithery/r4y1nsnAhcafdzOF/images/clients.png?fit=max&auto=format&n=r4y1nsnAhcafdzOF&q=85&s=ab48680e3f0259f799ed81d56b6b4d19"](https://mintcdn.com/smithery/r4y1nsnAhcafdzOF/images/clients.png?fit=max&auto=format&n=r4y1nsnAhcafdzOF&q=85&s=ab48680e3f0259f799ed81d56b6b4d19") alt="MCP Clients" width="1032" height="952" data-path="images/clients.png" />
...
- This creates a smooth experience for users to explore MCP servers
...
const { transport } = await createConnection({
mcpUrl: '[https://exa.run.tools',](https://exa.run.tools',)
});
...
const servers = [
'[https://exa.run.tools',](https://exa.run.tools',)
'[https://gmail.run.tools',](https://gmail.run.tools',)
];
