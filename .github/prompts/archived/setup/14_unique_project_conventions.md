...
> **⚠ Conflict note:** `.github/prompts/instructions/performance.instructions.md` says "Use `React.memo` for expensive components" — this directly contradicts the React Compiler rule. **This setup prompt is authoritative.** `memo()`, `useMemo`, `useCallback` are all forbidden. The performance instructions file needs updating.
...
From `.github/prompts/instructions/nextjs.instructions.md`:
...
From `.github/prompts/instructions/security.instructions.md`:
