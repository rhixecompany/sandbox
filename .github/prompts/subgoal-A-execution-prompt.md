...
- DRY enforcement: identity/routing by .hermes.md; prefs by user-communication-preferences; protocol by multi-file-change-protocol; session achievements by .hermes/plans/ + .hermes/specs/; .env protection by .hermes.md + specs/exposure-correction.md; 0 duplication; identity preserved.
...
2. Execute git stash applications (4 stashes from ~/AppData/Local/Hermes/hermes-agent); resolve conflicts with .hermes/mcp.json / config.yaml.
...
4. Create/update artifacts: plan (`.hermes/plans/subgoal-A-*`), spec (`.hermes/specs/subgoal-A-spec-*`), script (`.hermes/scripts/` or `scripts/`), skill (if needed — `.hermes/skills/` or workspace `skills/`), validation evidence (`results/` or `.hermes/reports/`).
...
- Gate A3: Conflicts resolved (verified by `.hermes/mcp.json` valid JSON + `python -m json.tool`).
- Gate A4: Systematic-debugging phase executed (verified by `.hermes/plans/debug-subgoal-plan-2026-09-13.md` reference + real exit codes).
