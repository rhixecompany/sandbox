...
- **App Router First**: Always use the App Router (`src/app/` directory) for new projects - it's the modern standard
...
- Always use the App Router (`src/app/` directory) for new Next.js projects
...
- Implement middleware in `src/proxy.ts` at root for auth, redirects, and request modification
...
- Show proper file structure with exact file paths in the `src/app/` directory
