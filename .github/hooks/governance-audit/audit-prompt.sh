...
# Governance Audit: Scan pre-LLM user messages for threat signals before agent processing (Hermes hook)
#
...
# Extract prompt text from Hermes input (JSON with user_message or prompt field)
PROMPT=""
if command -v jq &>/dev/null; then
  PROMPT=$(printf '%s' "$INPUT" | jq -r '.user_message // .userMessage // .prompt // empty' 2>/dev/null || echo "")
fi
if [[ -z "$PROMPT" ]]; then
  PROMPT="$INPUT"
fi
