...
- **Threat detection**: Scans `pre_llm_call` user messages for data exfiltration, privilege escalation, system destruction, prompt injection, and credential exposure
...
|-------|--------|----------|
| `on_session_start` | `audit-session-start.sh` | `governance/audit.log` |
| `on_session_end` | `audit-session-end.sh` | `governance/audit.log` |
| `pre_llm_call` | `audit-prompt.sh` | `governance/audit.log` |
