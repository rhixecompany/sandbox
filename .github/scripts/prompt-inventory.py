#!/usr/bin/env python3
"""Generate prompt inventory report."""
...
SANDBOX = Path(r"C:\Users\Alexa\Desktop\SandBox")
ROOT = Path(SANDBOX / "prompts")
OUT = Path(SANDBOX / "docs/prompt-inventory.md")
...
async def main():
    ...
    for p in files:
        ...
        [m.start() for m in re.finditer(r"(?m)^(---|\.\.\.)\s*$", text)]
        [m.start() for m in re.finditer(r"(?m)^(---|\.\.\.)\s*$", text[3:])]
        ...
        def has(pattern: str, yml: str) -> bool:
            return re.search(pattern, yml, flags=re.MULTILINE) is not None

        def val(pat: str, yml: str):
            m = re.search(pat, yml, flags=re.MULTILINE)
            return m.group(1).strip().strip('"').strip("'") if m else ""

        for key in ["name", "title", "description", "version", "license", "author"]:
            if not has(rf"(?m)^{re.escape(key)}\s*:", yaml):
        ...
        version_str = val(r"(?m)^version:\s*(.+)", yaml)
        if version_str and not re.fullmatch(
            r"(0|[1-9]\d*)\.(0|[1-9]\d*)(?:\.(0|[1-9]\d*))?(?:[-+.][0-9A-Za-z.-]+)?", version_str
        ):
            issues.append("version_not_semver")

        trigger = val(r"(?m)^trigger:\s*(.+)", yaml)
        if trigger != p.stem:
        ...
        meta.update(
            {
                "name": val(r"(?m)^name:\s*(.+)", yaml) or p.stem,
                "title": val(r"(?m)^title:\s*(.+)", yaml),
                "trigger": trigger,
                "tags": tags,
                "issues": issues,
                "valid": not issues,
    ...
    lines += [
        ...
        "## All files",
        "",
        "| File | Name | Title | Trigger | Tags | Valid | Issues |",
        "|------|------|-------|---------|------|--------|---------|",
    ]
    for r in results:
        lines.append(
            f"| {r['file']} | {r['name']} | {r['title']} | {r['trigger']} | {', '.join(r['tags'])} | {'yes' if r['valid'] else 'no'} | {', '.join(r['issues'])} |"
        )
