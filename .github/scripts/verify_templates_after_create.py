...
for fp in sorted(glob.glob(os.path.join(P,"*.prompt.md"))):
    ...
    for m in ref_re.finditer(body):
        total+=1; pathpart=m.group(0).split("#")[0]
        cand=os.path.normpath(os.path.join(P,pathpart))
        if not os.path.exists(cand):
            dead+=1; fl=True
    if fl: files+=1; fset.add(os.path.basename(fp))

# 2) count created template files
tmpl_files=0
for root,_,fs in os.walk(os.path.join(P,"templates")):
...
for fp in sorted(glob.glob(os.path.join(P,"*.prompt.md"))):
    raw=open(fp,encoding="utf-8",errors="replace").read()
    if not raw.startswith("---"): CLEAN=False; bad+=1; continue
    m=re.match(r"^---\s*\n(.*?)\n---\s*\n?",raw,re.DOTALL)
