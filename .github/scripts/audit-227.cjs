...
const rootHermes = path.join(process.env.LOCALAPPDATA || process.env.USERPROFILE || 'C:\\Users\\Alexa', 'hermes');
...
function walk(dir, rel='') {
  ...
  for (const name of fs.readdirSync(dir)) {
    ...
    if (stat.isDirectory()) walk(full, rel ? rel + '/' + name : name);
    else {
      const cur = rel ? rel + '/' + name : name;
      if (dir === rootPrompts && name.endsWith('.prompt.md')) promptCandidates.push(cur);
