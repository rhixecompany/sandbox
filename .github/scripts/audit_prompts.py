...
async def main():
    ...
    for fpath in all_files:
        ...
        content = await loop.run_in_executor(None, _read_text, fpath)
            ...
            file_issues.append(
                {
                    "severity": "HIGH",
                    "type": "frontmatter",
                    "detail": "Unclosed frontmatter fence (no closing --- in first 60 lines)",
                }
            )
                ...
                file_issues.append(
                    {
                        "severity": "HIGH",
                        "type": "frontmatter",
                        "detail": f"Double frontmatter fences ({fence_count} --- markers in first 60 lines)",
                    }
                )
                    ...
                    file_issues.append(
                        {
                            "severity": "HIGH",
                            "type": "frontmatter",
                            "detail": "Frontmatter YAML does not parse as a mapping",
                        }
                    )
            except yaml.YAMLError as e:
                file_issues.append(
                    {"severity": "HIGH", "type": "frontmatter", "detail": f"Frontmatter YAML parse error: {e}"}
                )
                            ...
                            file_issues.append(
                                {
                                    "severity": "MEDIUM",
                                    "type": "skills-prose",
                                    "detail": f"Line {j + 1}: skills: section has dependency-style prose instead of list: '{next_line[:80]}'",
                                }
                            )
                        break
            ...
            file_issues.append(
                {
                    "severity": "LOW",
                    "type": "formatting",
                    "detail": f"Trailing whitespace on {len(trailing_ws)} lines (first: {trailing_ws[:3]})",
                }
            )
        ...
        if tab_lines:
            file_issues.append(
                {"severity": "LOW", "type": "formatting", "detail": f"Tab characters on {len(tab_lines)} lines"}
            )
                ...
                file_issues.append(
                    {
                        "severity": "MEDIUM",
                        "type": "structure",
                        "detail": f"Line {hline}: Heading level skip (h{prev_level} -> h{level}): '{htext[:60]}'",
                    }
                )
            prev_level = level
        ...
        todo_lines = [
            ...
            if re.search(r"\b(TODO|FIXME|HACK|XXX)\b", l) and not l.strip().startswith("#")
        ]
        if todo_lines:
            file_issues.append(
                {"severity": "LOW", "type": "content", "detail": f"TODO/FIXME markers on {len(todo_lines)} lines"}
            )
        ...
        if broken_links:
            file_issues.append(
                {"severity": "MEDIUM", "type": "content", "detail": f"Potentially broken links: {broken_links[:3]}"}
            )
        ...
        if fpath.parent == PROMPTS_DIR and not fname.endswith(".prompts.md"):
            file_issues.append(
                {"severity": "LOW", "type": "naming", "detail": "File in Prompts/ doesn't use .prompts.md extension"}
            )
            ...
            file_issues.append(
                {
                    "severity": "LOW",
                    "type": "naming",
                    "detail": "File in .github/prompts/ doesn't use .prompt.md extension",
                }
            )
    ...
    await loop.run_in_executor(None, _write_json, SANDBOX / "docs" / "audit_results.json", output)
