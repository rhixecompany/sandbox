...
async def main():
    ...
    text = await loop.run_in_executor(None, _read_json, os.path.join(ROOT, "results", "_agents_fix_discovery.json"))
    d = json.loads(text)
    ...
    report = "\n".join(L)
    await loop.run_in_executor(None, _write_text, os.path.join(ROOT, "results", "agents-fix.output.md"), report)
