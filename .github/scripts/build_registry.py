...
with open(os.path.join(DOCS, "prompt-registry.md"), "w", encoding="utf-8") as f:
    ...
    f.write("**Location:** `~/AppData/Local/hermes/prompts/`  \n")
    f.write(f"**Total prompts:** {len(data)}  \n")
    f.write(f"**Skills available (for `skill:` deps):** {len(KS)}\n\n")
    f.write("---\n\n")
    f.write("## 1. What This Library Is\n\n")
    f.write(
        "A curated, self-contained library of 211 executable Hermes **prompts** — each a YAML-fronted Markdown artifact that packages the full execution context (goal, workflow, required toolsets, skills, dependencies) for a reproducible task. Prompts cover Azure/cloud ops, .NET/Java/Python code-gen, GitHub PR/issue workflows, planning & specs, prompt-library maintenance itself, research pipelines, agent orchestration, and more.\n\n"
    )
    f.write("## 2. Maintenance Pass Performed (this session)\n\n")
    f.write(
        "Loaded and applied all 10 prompt-related skills: `prompt-verification`, `prompt-library-maintenance`, `fix-prompt-frontmatter`, `prompt-batch-fixer`, `prompt-management`, `validate-prompts`, `fix-prompts`, `audit-prompts`, `boost-prompts`, `ai-prompt-engineering-safety-review`.\n\n"
    )
    f.write(
        "Then executed a deterministic, body-preserving repair across the library and an independent re-verification:\n\n"
    )
    f.write(
        "- **Schema/integrity:** all 211 prompts pass an independent verifier (frontmatter completeness, DEPS==SKILLS, toolset validity, name/trigger consistency, skill & MCP resolution). 0 issues.\n"
    )
    f.write(
        "- **36 files fixed:** 22 `trigger:` normalized to `/<slug>`; 3 `name:` synced to filename; 13 bidirectional DEPS==SKILLS sync; 8 toolset normalizations.\n"
    )
    f.write(
        "- **2 corrupted files repaired:** `web-research-pipeline` (MCP server relabeled `skill:mcp-fetch`→`tool:mcp-fetch`; self-referencing `skill:web-research-pipeline` dependency removed) and `update-implementation-plan` (a duplicated frontmatter block collapsed into one).\n"
    )
    ...
    f.write(
        "- **Skill / Tool / Prompt dependency audit:** every `skill:` ref resolves to a real `SKILL.md`; every `tool:mcp-*` is a known-good MCP server; every `tool:` native-tool ref (terminal, file, web, browser, search_files, patch, write_file, execute_code, delegate_task, etc.) is valid. **0 issues.**\n"
    )
    f.write(
        "- **Cross-prompt delegation:** `prompt:` references resolve correctly (the `.prompt.md` suffix is tolerated). **0 dangling, 0 mislabeled.**\n"
    )
    f.write(
        "- **Content structure & safety:** no injection/jailbreak patterns; no un-gated destructive operations. 5 prompts are intentionally heading-less prose/persona styles (not defects).\n\n"
    )
