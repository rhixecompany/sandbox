...
async def main():
    ...
    for f in glob.glob(os.path.join(HERMES_HOME, "pending", "skills", "*.json")):
        if os.path.basename(f).startswith("_"):
            continue
        await loop.run_in_executor(None, shutil.copy, f, os.path.join(PENDING, os.path.basename(f)))
    ...
    def do_create(p):
        ...
        _write_sync(os.path.join(d, "SKILL.md"), content)
