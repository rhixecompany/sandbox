...
3. Store in `~/AppData/Local/hermes/.env`:
   ```
...
Add those exports to your shell profile (or to `~/AppData/Local/hermes/.env`) so every session inherits them.
...
- When working with notion tools or integrations
- When automating or managing notion tasks
- When troubleshooting notion configurations
- **Triggers**: "Notion", "notion" related operations
...
Perform the notion operations following the skill instructions.
