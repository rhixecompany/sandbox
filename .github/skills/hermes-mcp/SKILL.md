...
Automated reasoning and workflow tool for `hermes-mcp`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
| `context7` | HTTP | Docs lookup | API key (header `CONTEXT7_API_KEY`) |
| `github` | `npx @modelcontextprotocol/server-github` | GitHub | Pat |
| `neon` | Remote HTTP `https://mcp.neon.tech/mcp` | Neon DB mgmt | `Authorization: Bearer <napi_key>` (npm pkg `@neondatabase/mcp-server-neon` is DEPRECATED) |
| `sentry` | Remote HTTP `https://mcp.sentry.dev/mcp` | Sentry issues | `Authorization: Sentry-Bearer <sntrys_token>` |
...
grep -r "api_key\|password\|token" $LOCALAPPDATA/hermes/config.yaml

# 2. Verify no shell injection in args
grep -A 5 "mcp_servers:" $LOCALAPPDATA/hermes/config.yaml | grep "args:"

# 3. Check pinned versions (not @latest)
grep -B 2 -A 2 "command:" $LOCALAPPDATA/hermes/config.yaml
...
| `args` as JSON string (`args: '["-y",...]'`) | Critical | Pydantic `StdioServerParameters` list_type validation error — use YAML list |
| `patch`/`write_file` refuse to edit `config.yaml` | High | Security guard — use `python` file I/O (read → replace → write) with backup |
| Windows subprocess `npx` fails (WinError 2) | High | Use full path to `npx.cmd` (e.g. `C:\nvm4w\nodejs\npx.cmd`) in Python subprocess |
...
Set up required environment, dependencies, and configuration for "Hermes MCP".

### Phase 2: Execution

Run the primary "Hermes MCP" operations according to the defined requirements.
