...
Automated reasoning and workflow tool for `soul-enhancer`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
> **Automated path:** `scripts/enhance_soul.py` validates/repairs all SOUL.md files in
> one shot (live profiles + `--mirror` for the workspace mirror). Manual steps below
> remain for one-off edits.
>
> ```bash
> # Validate structure + headers for all profiles (and the workspace mirror)
> python "$LOCALAPPDATA/hermes/skills/devops/soul-enhancer/scripts/enhance_soul.py" --check --mirror
> # Repair **Profile:** header / missing **Identity:** in place
> python "$LOCALAPPDATA/hermes/skills/devops/soul-enhancer/scripts/enhance_soul.py" --fix --mirror
> # Scope to one profile
> python "$LOCALAPPDATA/hermes/skills/devops/soul-enhancer/scripts/enhance_soul.py" --check --profile alexa
> ```
...
```bash
# Automated: same checks as step 3 but for every profile at once
python "$LOCALAPPDATA/hermes/skills/devops/soul-enhancer/scripts/enhance_soul.py" --check --mirror

# Manual per-file checks
...
- [ ] "SOUL.md Profile Enhancer" operations completed successfully
...
| Skill | Purpose |
|-------|---------|
| `hermes-personality-soul` | Persona→tone→traits mapping for profile identity content |
| `create-missing-souls` | Create missing/stub profile SOUL.md files |
| `profile-maintenance` | Audit and update USER.md, MEMORY.md, SOUL.md |
...
# Fix Profile/Identity headers in place
python ~/AppData/Local/hermes/skills/devops/soul-enhancer/scripts/enhance_soul.py --fix

# Propagate root SOUL.md headers to all profiles
...
After soul enhancement operations, validate with:

```bash
python ~/AppData/Local/hermes/skills/devops/validate-memories/scripts/validate_memories.py
```

Or use the post-soul-enhancement hook at `~/AppData/Local/hermes/hooks/post-soul-enhancement/validate_soul.sh`.
