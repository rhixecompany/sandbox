...
### Batch Profile Operations (Python Subprocess Piping)

When operating on multiple profiles (bulk delete, recreate), use Python subprocess to handle interactive confirmations:
...
for p in profiles:
    result = subprocess.run(
        ...
        input=f"{p}\n",          # pipes the profile name as confirmation
        text=True, capture_output=True, timeout=30
    )
    print(result.stdout[-300:])
```

The `input=f"{p}\n"` technique works because Hermes CLI reads confirmation from stdin. This avoids manual typing for each profile.

### Best-Practice Profile Recreation

When recreating profiles after deletion, use `--clone-from` with `--description`:

```bash
hermes profile create <name> \
  --clone-from default \
  --description "One-line purpose description used by kanban orchestrator"
```

Benefits over bare `hermes profile create <name> --clone`:
- Pins the source explicitly (not dependent on which profile happens to be active)
- `--description` feeds the kanban task router with role information
- Automatically creates the `.bat` wrapper alias

### Reserved Profile Names

The following profile names are **reserved** and cannot be created via `hermes profile create`:
- `default` — system profile (cannot be created separately; uses root directory)
- `hermes` — collides with the Hermes system binary

An attempt to create a reserved profile name fails with:
```
Error: Profile name '<name>' is reserved — it collides with either the Hermes 
installation itself or a common system binary. Pick a different name.
```

### Profile Deletion and Gateway Lock (Windows)

On Windows, a running gateway process holds `gateway.lock` in the profile directory. This prevents `hermes profile delete` from working:

```
RuntimeError: Could not remove profile directory: [WinError 32] The process cannot 
access the file because it is being used by another process: 'gateway.lock'
```

**Detection:** Check if the gateway is running via `hermes profile list` (look for `running` in the Gateway column).

**Remediation Steps:**
1. Find the PID holding the lock:
   ```bash
   cat $LOCALAPPDATA/hermes/profiles/<name>/gateway.lock
   # Returns: {"pid": 22484, "kind": "hermes-gateway", ...}
   ```
2. Kill the process (may need elevated/Admin rights):
   ```bash
   taskkill /F /PID 22484
   # PowerShell alternative: Stop-Process -Id 22484 -Force
   ```
3. After the process is killed, retry:
   ```bash
   hermes profile delete <name>
   ```

**Note:** If the process is running elevated (common for gateway services started with admin rights), `taskkill` from a non-admin shell will fail with "Access is denied". In that case:
- Use Task Manager to manually end `pythonw.exe`
- Or run the terminal as Administrator

### TUI Process Self-Lock (mcp-stderr.log)

When running inside the Hermes TUI, the running session holds an open handle to each profile's `logs/mcp-stderr.log`. This prevents `hermes profile delete` even when the gateway is stopped:

```
RuntimeError: [WinError 32] The process cannot access the file because it is 
being used by another process: '...mcp-stderr.log'
```

**Root cause:** The TUI process writes MCP server stderr to this file. The handle is owned by the current session — you cannot kill yourself.

**Detection:** `hermes profile list` shows gateway=stopped but delete still fails with the mcp-stderr.log path. No gateway.lock file involved.

**Remediation Steps (any of these work):**

1. **Rename the locked file** (breaks the name-based lock, original path becomes free):
   ```bash
   cmd.exe /c "rename C:\Users\<user>\AppData\Local\hermes\profiles\<name>\logs\mcp-stderr.log mcp-stderr.log.locked"
   ```
   Then retry: `hermes profile delete <name>`
   *Why this works:* Windows file locks are per-inode, not per-path. Renaming severs the path-to-inode binding for the original filename without closing the handle. The `rmtree` then succeeds because the original path no longer exists.

2. **Hard-nuke via Windows `rd`** (bypasses Python's shutil.rmtree entirely):
   ```bash
   cmd.exe /c "rd /s /q C:\Users\<user>\AppData\Local\hermes\profiles\<name>"
   ```
   **Caveat:** Hermes TUI auto-recreates profile directories on the next `hermes` CLI invocation. The nuke is ephemeral unless you immediately use the freed name. Combine with step 3.

3. **Manual bootstrap** (when `hermes profile create` says "already exists"):
   After nuking the directory with `rd /s /q`, manually copy the essentials from a sibling profile or from root:
   ```bash
   # Create directory structure
   mkdir -p $LOCALAPPDATA/hermes/profiles/<name>/{memories,logs}
   # Copy config from an existing working profile
   cp $LOCALAPPDATA/hermes/profiles/code-architect/config.yaml $LOCALAPPDATA/hermes/profiles/<name>/config.yaml
   # Copy SOUL.md from root
   cp $LOCALAPPDATA/hermes/SOUL.md $LOCALAPPDATA/hermes/profiles/<name>/SOUL.md
   # Create minimal USER.md compact pointer
   printf '# USER.md — Alexa\n\nSee authoritative source: `$LOCALAPPDATA/hermes/memories/USER.md`\n' > $LOCALAPPDATA/hermes/profiles/<name>/memories/USER.md
   ```
   After this, `hermes profile list` shows the profile with the correct model.

4. **Cleanest fix (requires restart):** Note the blocked profile, restart the terminal session, then delete immediately:
   ```bash
   hermes profile delete <name>
   hermes profile create <name> --clone-from default
   ```

### --clone-all Timeout on Large Skill Libraries

On Windows, `hermes profile create <name> --clone-from default --clone-all` can **time out** when the default profile has 300+ skills (each copied individually). The `--clone-all` flag copies skills/, plugins/, cron/, skins/, workspace/, and home/ directories.

**Fix:** Omit `--clone-all` and create with the minimal flag:
```bash
hermes profile create <name> --clone-from default --description "Role description"
```
Skills inherit from default at runtime — the profile doesn't need its own copy. If you later need profile-specific skills, add them individually.

### Hermes Auto-Recreates Deleted Profile Directories

While the TUI is running, Hermes monitors for missing profile directories and auto-recreates them on the next CLI invocation. This means:
- `cmd.exe /c rd /s /q` removes the directory only momentarily
- The next `hermes` command (even `list`) recreates it
- The lock file returns with it

**Strategy:** When fighting auto-recreation:
- Nuke the dir, then do NOT run any `hermes` CLI commands that might trigger recreation
- Directly copy config files into the recreated dir instead (see step 3 above)
- Or accept the limitation and schedule cleanup for the next session restart
...
2. **Cross-reference for consistency:**
   - Read ALL context files before editing: `.hermes.md`, `AGENTS.md`
...
3. **Fix discovered issues in priority order:**
...
**Special case: Default profile** — The `default` profile does NOT use `~/AppData/Local/hermes/profiles/default/`. It uses the root `~/AppData/Local/hermes/` directory directly:
...
```bash
grep -A 3 "^model:" $LOCALAPPDATA/hermes/config.yaml
```

### Source 2: Defined Provider Blocks
```bash
grep -A 2 "^  [a-z]" $LOCALAPPDATA/hermes/config.yaml | grep -E "^\s+\w+:"
```

### Source 3: .env Credential Keys
```bash
grep -oP '^[A-Z_]+_KEY[^=]*|^[A-Z_]+_TOKEN[^=]*' $LOCALAPPDATA/hermes/.env | sort -u
...
du -sh $LOCALAPPDATA/hermes/{logs,sessions,checkpoints,skills}
...
> **Note:** The `search` toolset was merged into `web`. Run `hermes tools list` for the actual current list — this table documents available toolsets conceptually, not runtime state.
...
2. `~/AppData/Local/hermes/profiles/<name>/SOUL.md` — profile standards
3. `~/AppData/Local/hermes/SOUL.md` — global standards
...
[ -f $LOCALAPPDATA/hermes/memories/USER.md ] && echo "USER.md (default) ✓"
[ -f $LOCALAPPDATA/hermes/SOUL.md ] && echo "SOUL.md (default) ✓"
wc -c $LOCALAPPDATA/hermes/memories/USER.md  # <1375
wc -c $LOCALAPPDATA/hermes/SOUL.md

# Non-default profiles use profiles/<name>/memories/
[ -f $LOCALAPPDATA/hermes/profiles/code-architect/memories/USER.md ] && echo "USER.md (code-architect) ✓"
...
wc -c $LOCALAPPDATA/hermes/memories/MEMORY.md       # <1760 (80%)
ls -lh $LOCALAPPDATA/hermes/logs/*.log              # recent, <100MB

# 4. Toolsets
hermes tools list | grep "✓ enabled" | wc -l    # actual enabled count
...
**Remediation Steps:**
1. Read current active model: `grep -A 3 "^model:" ~/AppData/Local/hermes/config.yaml`
...
**Detection:** `memories/USER.md` contains stale model/profile/MCP-count info that doesn't match current runtime.
**Root Cause:** Updating config.yaml's model/provider without refreshing `memories/USER.md` to match.
**Remediation Steps:**
1. Keep `memories/USER.md` to a short identity summary plus profile-specific preferences.
2. Cross-reference model string, active profile name, and MCP server count against config.yaml and `hermes profile list`.
3. Re-read after writing to confirm it stayed compact, under the 1375-byte char limit.
4. Check size: `wc -c ~/AppData/Local/hermes/memories/USER.md` should be under 1375.
...
**Remediation Steps**:
1. Check each profile USER.md: `wc -c ~/AppData/Local/hermes/profiles/*/memories/USER.md`
...
**Remediation Steps:**
1. Check each profile-specific SOUL.md: `wc -l ~/AppData/Local/hermes/profiles/*/SOUL.md`
...
**Detection:** Looking for `~/AppData/Local/hermes/profiles/default/` returns "No such file or directory".
**Root Cause:** The `default` profile is the system profile and lives at `~/AppData/Local/hermes/` directly, not in `profiles/default/`.
**Remediation Steps:**
1. Accept that `default` has no subdirectory under `profiles/`.
2. For default profile files, use root paths: `~/AppData/Local/hermes/memories/USER.md`, `~/AppData/Local/hermes/SOUL.md`, `~/AppData/Local/hermes/memories/MEMORY.md`, `~/AppData/Local/hermes/memories/USER.md`.
...
**Detection:** `hermes profile delete default` fails with "Cannot delete the default profile (~/AppData/Local/hermes). To remove everything, use: hermes uninstall".
...
**Remediation Steps:**
1. Check existence: `[ -f ~/AppData/Local/hermes/profiles/<name>/SOUL.md ]`
...
grep -c "MASTER_RULES" ~/Desktop/SandBox/{AGENTS.md,.hermes.md} $LOCALAPPDATA/hermes/{SOUL.md,USER.md}
...
**Detection:** Context files (`.hermes.md`, `AGENTS.md`) disagree on profile tables, MCP server counts, plugin lists, or toolset counts.
...
3. Propagate changes to `.hermes.md`, `AGENTS.md`
4. Verify all tables match across files:
   ...
   - Toolset list
5. **Verify toolset claims against actual runtime:**
   - **Get the real count:** `hermes tools list | grep "✓ enabled" | wc -l` — this is authoritative. Do NOT use `grep -c "enabled"` which also matches "disabled" lines and gives inflated totals.
   - Check that every toolset name listed in context files appears in the `hermes tools list` output
   - If a toolset name in context files doesn't appear in the tools list (e.g. a renamed/removed toolset like `search` or `moa`), remove it from all context files and decrement the count
   - The context file count must match the real count above, not just be internally consistent across files
6. Verify with `git diff --stat` to confirm all files were updated

### Placeholder Persona Descriptions
**Detection:** Profile config.yaml has `placeholder` (or empty) as a persona description instead of a meaningful role definition.
**Root Cause:** Profile was created with a template default and never customized. Common for unconfigured profiles.
**Remediation Steps:**
1. Check all profile configs: `for f in ~/AppData/Local/hermes/profiles/*/config.yaml; do if grep -q "placeholder\|^\s*$" "$f" 2>/dev/null; then echo "$(basename $(dirname $f)): PLACEHOLDER"; fi; done`
2. Replace `placeholder` with a concise 1-2 line description matching the profile's purpose
3. If the profile has no clear purpose yet, a placeholder is acceptable — but it should be noted as deferred
4. Verify with `hermes config check`

### Root USER.md Deleted — memories/USER.md Is Now Authoritative
**Detection:** Default profile has `memories/USER.md` but root `~/AppData/Local/hermes/USER.md` no longer exists.
**Root Cause:** The root USER.md was intentionally removed as part of a migration. `memories/USER.md` is now the single authoritative user identity file for the default profile.
**Remediation Steps:**
1. Check `memories/USER.md` exists at both root and non-default profile paths:\n   `[ -f ~/AppData/Local/hermes/memories/USER.md ] && [ -f ~/AppData/Local/hermes/profiles/*/memories/USER.md ]`
2. Ensure `memories/USER.md` is within the 1375-byte size limit: `wc -c ~/AppData/Local/hermes/memories/USER.md`
3. Validate its content has identity model provider and preferences (is not a stub)
4. Remove any remaining references to root `~/AppData/Local/hermes/USER.md` in scripts and documentation — they are now stale.
5. Verify with `hermes config check`
**Pitfall:** Do not recreate root `USER.md` — it was intentionally removed. All validation should check `memories/USER.md` only.
...
**Remediation Steps:**
1. Read the current active model from config.yaml: `grep -A 3 "^model:" ~/AppData/Local/hermes/config.yaml`
2. Check all configured profile USER.md files for model info:
   ```bash
   for f in $LOCALAPPDATA/hermes/profiles/*/memories/USER.md; do
...
**Models per profile are not listed here — check `config.yaml` (`grep -A 3 "^model:" ~/AppData/Local/hermes/config.yaml`) for the current active model per profile.** This table skips the Model column to avoid DRY drift: config.yaml is the single source of truth.
