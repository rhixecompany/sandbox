...
## Management
```bash
/skill update                     # Check hub updates (interactive)
hermes skills update              # Apply all pending hub updates at once
...
**`hermes skills update` bulk guidance:**
When 50+ updates are pending, the command can take several minutes (each update triggers a security scan). Run in background with a generous timeout (300s+) to avoid premature cutoff:
```bash
hermes skills update &
# OR via terminal tool:
terminal(command="hermes skills update", background=true, notify_on_complete=true)
```
For very large batches (75+), consider looping a subset at a time with `hermes skills install <identifier> --yes` for individual skills instead of the bulk update hammer.