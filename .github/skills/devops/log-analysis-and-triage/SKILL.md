---
author: Hermes Agent
description: Systematic workflow for analyzing multi-file log directories, categorizing
  by type and age, detecting false positives vs. real failures, and deciding what
  to keep vs. delete. Includes heuristics for distinguishing tool unavailability from
  detection/interop failures.
license: MIT
name: log-analysis-and-triage
title: Log Analysis and Triage — False Positives, Categorization, Cleanup
trigger: '• User asks to "read logs" or "identify issues" in a directory
...
### ⚠ shlex.split POSIX Mode Mangles Windows Backslash Paths (Hermes Hooks)
**Symptom:** `WARNING agent.shell_hooks: shell hook failed (event=... command=C:\Program Files\Git\usr\bin\bash.exe ...): command not found` — repeated on every session start/end/pre_llm_call.  
**Cause:** Hermes' `agent/shell_hooks.py:_spawn()` uses `shlex.split(spec.command)` which defaults to POSIX mode where `\` is an escape character. On Windows, `C:\Program Files\Git\usr\bin\bash.exe` gets split into `['C:Program', 'FilesGitusrbinbash.exe', ...]` → `FileNotFoundError`.  
**Fix:** In `config.yaml`, wrap the hook command in a YAML single-quoted scalar with forward-slash paths:
```yaml
- command: '"C:/Program Files/Git/usr/bin/bash.exe" -c "$HOME/AppData/Local/hermes/hooks/session-logger/hook.sh"'
```
**Verification:**
```bash
python3 -c "
import yaml, shlex
cfg = yaml.safe_load(open(r'C:\\Users\\Alexa\\AppData\\Local\\hermes\\config.yaml'))
for evt, entries in cfg['hooks'].items():
    for e in entries:
        argv = shlex.split(e['command'])
        assert 'bash' in argv[0], f'Expected bash, got {argv[0]}'
        print(f'✓ [{evt}] {argv[0]} {argv[1]} {argv[2]}')
"
```
...
| `shell hook failed (...): command not found` | **Real failure** (hooks disabled) | Root cause (Windows): `shlex.split` POSIX mode mangles `\` backslash paths in config.yaml hooks block. The path `C:\Program Files\...` becomes `C:Program` → `FileNotFoundError`. Fix: rewrite hook command with forward slashes and wrap entire command in YAML single-quoted scalar: `'"C:/Program Files/Git/usr/bin/bash.exe" -c "/c/Users/.../hook.sh"'`. Verify 4/4 hooks with `python3 -c "import yaml,shlex; cfg=yaml.safe_load(open(...)); [shlex.split(e['command']) for evt,entries in cfg['hooks'].items() for e in entries]"` — each should parse with bash.exe as argv[0]. |
| `API call failed ... HTTP 400: ... model is not supported` | **Expected** (chain fallback) | First provider in chain doesn't serve that model. Hermes falls through to the next provider. Check `agent.log` for the follow-up successful call. If ALL providers return 400, it's a real failure. |
| `check_fn _*_check returned False` | **Noise** (capability check) | Tools like browser CDP, kanban, image-gen, xai-video are not available in the current session mode (CLI vs TUI). Harmless — they simply don't appear in the tool list. |
| `[Telegram] ... getaddrinfo failed` | **Noise** (self-healing) | DNS resolution failure for `api.telegram.org`. Telegram reconnection logic retries with fallback IPs. Resolves automatically when DNS is reachable again. |
| `Failed to connect to OAuth notifications ... dockerBackendApiServer ... refused` | **Noise** (expected) | Docker Desktop is installed but not running. The MCP Docker server tries to connect to Docker's named pipe and fails gracefully. No impact on non-Docker workflows. |
| `[WinError 5] Access is denied: auth.json.tmp -> auth.json` | **Real failure** (historical/race) | Windows permission error during credential pool atomic replace. Caused by concurrent session writes. Can trigger a gateway crash loop (1012/1006 websocket close spam). Fix: restart gateway or kill conflicting processes holding the lock. |
| `Context engine ... not found — falling back to built-in compressor` | **Noise** (cosmetic) | Named context engine not configured. Falls back gracefully. No functional impact. |
| `Skill 'X' generates slash command '/X' which collides with a core Hermes command` | **Noise** (cosmetic) | Auto-skipped registration. No functional impact. |
| `WAL checkpoint (TRUNCATE) at close failed: disk I/O error` | **Ambiguous** | Hermes SQLite WAL checkpoint failed on session close. If one-off, harmless. If persistent, investigate disk/filesystem issues or antivirus interference. |
...
```powershell
# Line-range removal
$lines = Get-Content $env:LOCALAPPDATA\hermes\config.yaml
$newLines = @(); $skip = 409..413
for ($i = 0; $i -lt $lines.Count; $i++) { if ($skip -notcontains $i) { $newLines += $lines[$i] } }
Set-Content $env:LOCALAPPDATA\hermes\config.yaml -Value $newLines

# Regex replacement
$config = Get-Content $env:LOCALAPPDATA\hermes\config.yaml -Raw
$config = $config -replace 'old-text', 'new-text'
Set-Content $env:LOCALAPPDATA\hermes\config.yaml -Value $config -NoNewline

# Validate after edit
python -c "import yaml; yaml.safe_load(open(r'$env:LOCALAPPDATA\hermes\config.yaml', encoding='utf-8')); print('YAML VALID')"
```
...
- `references/windows-hook-path-mangling.md` — Full diagnosis trace: shlex.split → hook "command not found" → config.yaml path fix
