...
```bash
rm $LOCALAPPDATA/hermes/mcp-tokens/linear*.json
```
...
hooks:
  on_session_start:
    - command: C:\Program Files\Git\usr\bin\bash.exe 
        $HOME/AppData/Local/hermes/hooks/session-logger/hook.sh
```
...
## 6. Skills Hub Lock File Corruption (UnicodeDecodeError)

**Symptom:** `hermes doctor` → `⚠ Skills Hub → Lock file (corrupted or unreadable)`. `skills/.hub/lock.json` fails to parse.

**Root Cause:** A raw non-UTF8 byte (e.g. `0x97`, a Windows-1252 em-dash) written into a JSON string value. `json.loads(Path.read_text(encoding="utf-8"))` raises `UnicodeDecodeError`, which `HubLockFile.load()` and `_read_index_cache()` did NOT catch (only `JSONDecodeError, OSError`). Fix applied 2026-07-31 to `hermes-agent/tools/skills_hub.py` (lines ~3363, ~3417).

**Repair the file (preserves entries):**
```python
raw = open("skills/.hub/lock.json", "rb").read()
data = json.loads(raw.decode("cp1252"))   # lossless: 0x97 → em-dash
open("skills/.hub/lock.json", "w", encoding="utf-8", newline="\n").write(
    json.dumps(data, indent=2, ensure_ascii=False) + "\n")
```
Then verify: `hermes doctor | grep -A 3 "Skills Hub"` → `✓ Lock file OK (N hub-installed skill(s))`.

## 7. Skill Slash-Command Duplicate Warning Flood

**Symptom:** ~150 lines of `WARNING agent.skill_commands: Skill 'X' maps to slash command /X already claimed by 'X'; keeping the first and skipping this one.` in `logs/errors.log`.

**Root Cause:** A process scanned multiple skill roots (user `skills/` + bundled `hermes-agent/skills` + `hermes-agent/optional-skills` — the trees mirror each other, ~160 duplicated skill groups). The normal gateway/CLI scan only walks `SKILLS_DIR` + `skills.external_dirs` (both single-root) and produces ZERO warnings — reproduce with `scan_skill_commands()` directly. The flood came from a background curator session doing `skill_manage` → rescan in a multi-root context.

**Verdict:** Benign — first-wins dedup is by design (the warning is informational). Do NOT delete the duplicate trees (breaks hub install source / user skill index). Not reproducible via standard paths; ignore or fix at the emitter level.
...
10. **Hub lock** — `hermes doctor` → if `Lock file (corrupted or unreadable)`, repair via cp1252→utf-8 (pitfall 6)
11. **Skill dup warnings** — Confirm benign; check emitter scans multiple roots (pitfall 7)