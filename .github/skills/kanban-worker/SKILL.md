...
You can configure the gateway to receive cross-profile Kanban task notifications by adding `notification_sources` to `~/AppData/Local/hermes/config.yaml`.
...
- When performing kanban worker operations or tasks
- When managing kanban worker infrastructure or configurations
- When automating or debugging kanban worker workflows
- **Triggers**: "set up devops/kanban-worker", "configure kanban worker", "debug kanban worker issue"
...
Perform the kanban worker operations following the instructions in this skill.
...
- [ ] Kanban Worker operations completed successfully
...
Kanban Worker is a skill for handling kanban worker tasks and automation workflows. Use this skill when you need to perform kanban worker operations efficiently.
