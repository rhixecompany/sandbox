...
Automated reasoning and workflow tool for `polymarket`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
- Markets can be illiquid — large orders move prices significantly
- Check expiration dates before relying on market probabilities
- API rate limits apply — batch queries when possible
- Order book depth varies by market — verify before large trades

## Reference Files

- `templates/report-template.md` — Structured market report template
- `scripts/polymarket_cli.py` — CLI for querying markets and orderbooks
# Pitfalls
...
Set up required environment, dependencies, and configuration for Polymarket.

### Phase 2: Execution

Run the primary Polymarket operations according to the defined requirements.
