...
2. **Scan local skills** in `.github/skills/` (or the repo's skill location):
...
1. List all folders in `.github/skills/`.
...
4. If the user requests the update, replace the entire local skill folder with
   the remote version, preserving its location in `.github/skills/`.
