...
Automated reasoning and workflow tool for `smithery-ai-cli`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
- User asks to discover, search for, or install MCP tools via Smithery
- User wants to connect external services (email, Slack, GitHub, Jira, databases, etc.)
- User needs to find or call tools from the Smithery marketplace

## When NOT to Use

- Direct `mcp` server configuration in config.yaml is preferred for permanent MCP servers
- User has a specific MCP tool and doesn't need discovery
...
## Pitfalls\n\n- `smithery` CLI is a separate tool from native Hermes MCP support — know which you need\n- OAuth flow requires human interaction (`auth_required` status). If CLI shows it, tell the user to open the URL\n- Connection IDs are not in JSON-RPC body — use metadata for connection-level restriction\n- Piped output is JSONL, not human-readable; filter with `grep` / `jq`\n- Some commands (like `smithery auth login`) may show a timeout message but actually succeed in the background - check with `smithery mcp list` to verify authentication\n- When adding a server fails to initialize (e.g., status 522), the CLI still returns a connectionId - use `smithery tool list <id>` to verify the connection is actually usable\n- Error responses are often wrapped in JSON - always check the `status.state` field to determine success/failure\n
## Verification Checklist

- [ ] `smithery --help` works and shows CLI version
- [ ] Can search, add, list, and call MCP tools
- [ ] Authentication flow understood (OAuth URL for human)
- [ ] Token scoping policies correctly constrain access
