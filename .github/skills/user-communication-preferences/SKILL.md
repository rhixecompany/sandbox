...
Automated reasoning and workflow tool for `user-communication-preferences`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
- **Mandatory for every session.** This skill auto-loads all superpowers skills on startup.
...
- If a session reveals a reusable correction, make a minimal skill-library update instead of treating the pass as a no-op. Prefer the smallest durable improvement over deferring maintenance.
- If a session reveals a reusable correction, make a minimal skill-library update instead of treating the pass as a no-op. Prefer the smallest durable improvement over deferring maintenance.
...
15. **Explicit blocker reporting (2026-07-01)** — If progress is blocked, state the blocker clearly and stop. Include the precise failing evidence: command, path, event, and what changed since the last successful step.
...
- **Path migration pattern.** When a project restructures (e.g., moving `Bash/` → `projects/Bash/`, `Resume_maker/` → `projects/Resume_maker/`), update all cross-references in one pass: AGENTS.md, README.md, docs/architecture, prompts/, and workspace configs. Use `grep -r "old/path/"` to find all references; fix with targeted patches, not bulk sed. The old path often appears in directory trees, code fences, and inline references — each needs a context-aware replacement.
- **Path migration pattern.** When a project restructures (e.g., moving `Bash/` → `projects/Bash/`, `Resume_maker/` → `projects/Resume_maker/`), update all cross-references in one pass: AGENTS.md, README.md, docs/architecture, prompts/, and workspace configs. Use `grep -r "old/path/"` to find all references; fix with targeted patches, not bulk sed. The old path often appears in directory trees, code fences, and inline references — each needs a context-aware replacement.
...
### Phase 1: Setup — Load Superpowers Skills

1. **Auto-discover superpowers skills** — These are skills originally from the "obra/superpowers" collection:
   ...
   - `receiving-code-review` (software-development)
   - `refactor` (software-development)
   - `executing-plans` (software-development)
   - `code-wiki` (software-development)

2. **Load each superpowers skill** — Call `skill_view(name)` for each to verify availability and register their workflows.

3. **Verify all loaded** — Confirm no skill returned an error. If any is missing, report it as a blocker.

4. **Proceed to task** — All superpowers workflows are now available for the current session.

### Phase 2: Execute

Perform the user communication preferences operations following the skill instructions. Use loaded superpowers skills as needed.
