...
3. **SCENE_FN** — Scene function renders to pixel canvas (`uint8 H,W,3`). Composes multiple character grids via `_render_vf()` + pixel blend modes. See `ascii-video-composition.md`
4. **TONEMAP** — Percentile-based adaptive brightness normalization. See `ascii-video-composition.md` § Adaptive Tonemap
5. **SHADE** — Post-processing via `ShaderChain` + `FeedbackBuffer`. See `ascii-video-shaders.md`
...
- **Hardware detection** — auto-detect cores/RAM, set quality profile. See `ascii-video-optimization.md`
...
1. **Hardware detection + quality profile** — `ascii-video-optimization.md`
2. **Input loader** — mode-dependent; `ascii-video-inputs.md`
3. **Feature analyzer** — audio FFT, video luminance, or synthetic
4. **Grid + renderer** — multi-density grids with bitmap cache; `ascii-video-architecture.md`
5. **Character palettes** — multiple per project; `ascii-video-architecture.md` § Palettes
6. **Color system** — HSV + discrete RGB + harmony generation; `ascii-video-architecture.md` § Color
7. **Scene functions** — each returns `canvas (uint8 H,W,3)`; `ascii-video-scenes.md`
8. **Tonemap** — adaptive brightness normalization; `ascii-video-composition.md`
9. **Shader pipeline** — `ShaderChain` + `FeedbackBuffer`; `ascii-video-shaders.md`
10. **Scene table + dispatcher** — time → scene function + config; `ascii-video-scenes.md`
...
macOS Pillow: `textbbox()` returns wrong height. Use `font.getmetrics()`: `cell_height = ascent + descent`. See `ascii-video-troubleshooting.md`.

### ffmpeg Pipe Deadlock

Never `stderr=subprocess.PIPE` with long-running ffmpeg — buffer fills at 64KB and deadlocks. Redirect to file. See `ascii-video-troubleshooting.md`.

### Font Compatibility

Not all Unicode chars render in all fonts. Validate palettes at init — render each char, check for blank output. See `ascii-video-troubleshooting.md`.

### Per-Clip Architecture

For segmented videos (quotes, scenes, chapters), render each as a separate clip file for parallel rendering and selective re-rendering. See `ascii-video-scenes.md`.
...
- When you need to use ascii video for creative or design tasks
- When a project requires ascii video output or integration
- When automating or generating ascii video-style content
- **Triggers**: "Ascii Video" required for a project
...
- [ ] Ascii Video output was generated successfully
