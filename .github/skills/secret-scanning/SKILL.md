...
description: Guide for configuring and managing GitHub secret scanning, push protection,
  custom patterns, and secret alert remediation. For pre-commit secret scanning in
  AI coding agents via the GitHub MCP Server, this skill references the Advanced Security
  plugin (`advanced-security@copilot-plugins`). Use this skill when enabling secret
  scanning, setting up push protection, defining custom patterns, triaging alerts,
  resolving blocked pushes, or when an agent needs to scan code for secrets before
  committing.
license: MIT
...
Automated reasoning and workflow tool for `secret-scanning`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
Set up required environment, dependencies, and configuration for Secret Scanning.

### Phase 2: Execution

Run the primary Secret Scanning operations according to the defined requirements.
