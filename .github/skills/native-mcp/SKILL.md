...
Automated reasoning and workflow tool for `native-mcp`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
Add MCP servers to `~/AppData/Local/hermes/config.yaml` under the `mcp_servers` key:
...
1. Reads `mcp_servers` from `~/AppData/Local/hermes/config.yaml`
...
No `mcp_servers` key in `$LOCALAPPDATA/hermes/config.yaml`, or it's empty. Add at least one server.
...
Set up required environment, dependencies, and configuration for Native MCP.

### Phase 2: Execution

Run the primary Native MCP operations according to the defined requirements.
