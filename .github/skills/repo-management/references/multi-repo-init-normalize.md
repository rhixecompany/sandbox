...
For batch commit/push/PR runs, see `batch-pr-workflow.md` (same directory).

## Batch Snapshot + PRs

Use this when the workspace already contains many repos and the task is to snapshot every repo consistently.

1. Discover the actual git repos first; treat plain folders separately.
2. Process leaf repos before the root workspace repo.
3. If `git switch -C` fails because the repo is mid-rebase, run `git rebase --quit` and retry the branch switch.
4. Stage and commit each repo independently so one repo's hook or dirty state does not block the whole batch.
5. Reuse an existing PR when `gh pr list --head <branch> --state open --json url` returns one.
6. For root snapshots, exclude nested repos or local helper directories so they are not accidentally staged into the root commit.
