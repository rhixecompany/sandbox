...
2. If it does not resolve, install and verify an isolated CLI under a writable, persistent Hermes runtime directory. Prefer the current Hermes home at `tools/box-cli`; `HERMES_HOME` is optional, and Hermes uses its platform default when it is unset (`~/$HERMES_HOME` on macOS/Linux and `%LOCALAPPDATA%\hermes` on Windows).
...
node --version
npm --version
BOX_CLI_HOME="${HERMES_HOME:-$HOME/$HERMES_HOME}/tools/box-cli"
