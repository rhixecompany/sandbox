...
- Dispatch sibling parallel subagents that share a mutable config file without coordination. If parallel tasks all need to modify a shared resource (e.g. `.yamllint.yaml`, a `.env` template, a `docker-compose.override.yml`), sibling subagents will overwrite each other's changes, creating an infinite edit-loop and config corruption. Either dispatch the shared-config fix as a SINGLE sequential task BEFORE the bulk parallel tasks, or give each subagent a complete, static config they don't need to modify.
...
- [ ] "Subagent Driven Development" operations completed successfully
