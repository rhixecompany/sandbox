...
Automated reasoning and workflow tool for `linear`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
```bash
SCRIPT=$(dirname "$(find $LOCALAPPDATA/hermes -path '*skills/productivity/linear/scripts/linear_api.py' 2>/dev/null | head -1)")/linear_api.py
...
- When working with linear tools or integrations
- When automating or managing linear tasks
- When troubleshooting linear configurations
- **Triggers**: "Linear", "linear" related operations
