...
Automated reasoning and workflow tool for `darwinian-evolver`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
```bash
mkdir -p ~/./cache/darwinian-evolver && cd ~/./cache/darwinian-evolver
...
```bash
cd ~/./cache/darwinian-evolver/darwinian_evolver \
...
Open `~/./cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/./skills/research/darwinian-evolver
DE_DIR=~/./cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Hermes core.
   Custom driver scripts under `~/./skills/...` are user-side and fine.
...
- [ ] "Darwinian Evolver" operations completed successfully
...
Set up required environment, dependencies, and configuration for "Darwinian Evolver".

### Phase 2: Execution

Run the primary "Darwinian Evolver" operations according to the defined requirements.
