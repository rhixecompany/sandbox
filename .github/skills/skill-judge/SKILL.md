...
Automated reasoning and workflow tool for `skill-judge`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
For auditing 191+ skills, create a batch Python script at `$LOCALAPPDATA/hermes\scripts\batch_skill_judge.py` that:

1. Finds all SKILL.md files under `$LOCALAPPDATA/hermes/skills/` (supports `--folder`, `--category`, `--tag` filters)
...
- **2026-06-29 calibration (362 skills, post-remediation)**: After running the full execute-all-prompts pipeline (audit → dedupe → judge → remediate → consolidate), scored 45 ✅ ≥80 (12.4%), 317 ⚠️ 60-79 (87.6%), 0 🔴 <60 (0%). Average 72.5/100. Key finding: targeted remediation on <60 skills (adding title, version, author, license, tags, pitfalls, workflow sections) eliminated all FAIL ratings. Remaining WARN skills (60-79) have adequate structure but lack depth — next improvement cycle should focus on adding concrete examples and reference files to push them ≥80. The batch_skill_judge.py script with `--skill <name>` re-verification after patching is the reliable calibration loop.
- **Duplicate skill detection by name**: Skills can exist in both a flat root path (`skills/<name>/SKILL.md`) and a category subdirectory (`skills/<category>/<name>/SKILL.md`). Before deleting or remediating, always check for duplicate names: `grep -rh "^name:" --include="SKILL.md" <skills_dir> | sort | uniq -d`. The richer version (more lines, richer frontmatter with version/license/metadata) is authoritative — merge the smaller into it, then `rm -rf` the flat root copy. Confirmed 9 such duplicates in 2026-06-28 audit (stable-diffusion, qdrant, modal, lambda-labs, accelerate, here-now, cli, flash-attention, torchtitan).
...
- **Fence-aware heading parsing (2026-07-31)**: Heading analysis (duplicate-heading and heading-jump checks) MUST skip lines inside code fences. A naive `^#{1,4} ` scanner counts docstrings, file-tree examples, and shell-output blocks inside fenced code as structural headings — on the 591-skill audit this inflated the C-grade (2+ major issues) from 0 to 22 false positives. Detection: track fence state — a fence opens at a line matching `^ {0,3}(`{3,})` with only whitespace after; it closes at a run of >= open length with only whitespace after. Only record a heading when fence depth == 0. Apply the same fence-state tracking to the unclosed-fence (F5) check so a template fence inside a body block is not mistaken for a closer/opener. Verified: re-running the audit fence-aware dropped 15 of 22 C-grade skills to A-/B — they were clean all along.
...
- **Batch mode pitfalls**: 
  ...
  - **YAML tags format sensitivity**: The `batch_skill_judge.py` scorer originally only recognized inline YAML array format for tags (`tags: [a, b]`). Skills using YAML list format (`tags:\n  - a\n  - b`) scored FM=18 instead of 20 despite having all required fields. The scorer now accepts both formats. When debugging low FM scores for skills with complete frontmatter, check tags format first.** When scanning a skill, check for standalone YAML blocks (`---\nmetadata:\n  hermes:\n    tags: []\n---`) that appear AFTER the frontmatter closing `---`. These are copy-paste artifacts from template scaffolding that get embedded in the body. They break Structure scoring (orphaned `---` fences) and create false-positive frontmatter matches. Detection: grep for `^---` — if the second `---` appears well past line 20 (not the frontmatter close), it's a body fragment. Fix: remove the fragment entirely.
- **Verify before deleting duplicates (2026-06-22)**: When instructed to delete duplicate skills, ALWAYS verify the skill exists in ≥2 paths first: `find skills/ -name "SKILL.md" | xargs grep -l "name: <skill-name>"`. If only one path exists, that's the only copy — move it to the correct category directory instead of deleting. Deleting the only copy of a skill with no hub source to reinstall from is an unrecoverable loss. **Detection pattern for duplicate names across directories**: `grep -rh "^name:" --include="SKILL.md" <skills_dir>/ | sort | uniq -d` — this finds skill names that appear in multiple SKILL.md files. Always compare content/lines of both copies before deleting either; the richer version (more lines, version/license/metadata) keeps if both are meaningful.
- **Re-verification after remediation loop**: After patching a skill from FAIL (<60) to target (≥60), re-run `--skill <name>` through batch_skill_judge.py to confirm the score improved and the rating changed. The SKILL object's rating field (PASS/WARN/FAIL) is derived from the score thresholds and is the canonical post-fix status — trust the re-run, not the patch output.
...
- `references/evaluation-criteria.md` — Detailed scoring rubric for all 5 dimensions
- `references/evaluation-criteria.md` — Detailed scoring rubric for all 5 dimensions
- `references/evaluation-criteria.md` — Detailed scoring rubric for all 5 dimensions
- `references/evaluation-criteria.md` — Detailed scoring rubric for all 5 dimensions
...
- [ ] "Skill Judge" operations completed successfully
