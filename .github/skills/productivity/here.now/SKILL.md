...
| Skill | Purpose |
|-------|---------|
| `terminal` | Run publish.sh and drive.sh scripts |
| `curl` | Direct API calls for auth and publishing |

## Pitfalls

- **Anonymous site expiry**: Without API key, sites expire in 24 hours. Save the claim URL immediately.
- **Credentials file**: Always save API key to `~/.herenow/credentials` immediately — don't ask the user to do it.
- **State file is cache only**: Never present `.herenow/state.json` path as URL or source of truth.
- **File structure**: Place `index.html` at the root of the published directory, not in a subdirectory.

## Verification Checklist

- [ ] `curl`, `file`, `jq` available on PATH
- [ ] API key saved to `~/.herenow/credentials` (if authenticated)
- [ ] Site published and URL returned
- [ ] For anonymous sites: claim URL shared with user
- [ ] For Drive operations: scoped token with narrow `pathPrefix` and short TTL
...
- **`scripts/publish.sh`** — Site publishing (create/update, anonymous & authenticated)
- **`scripts/drive.sh`** — Private Drive storage operations (ls, put, cat, share)
- **Current docs**: https://here.now/docs
