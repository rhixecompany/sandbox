...
Automated reasoning and workflow tool for `code-wiki`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
Invoke through the `terminal` tool from the target repo's root, then use `read_file` / `search_files` / `write_file` to produce the wiki. Default output location is `~/./wikis/<repo-name>/`. Only write into the repo (`docs/wiki/`) when the user explicitly requests it.
...
```bash
OUTPUT_DIR="$HOME/./wikis/$REPO_NAME"
...
- **In-repo output without asking.** Default is `~/./wikis/`. Only write into the repo when the user explicitly requests it.
...
- [ ] "Code Wiki" operations completed successfully
