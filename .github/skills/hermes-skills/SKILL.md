...
Automated reasoning and workflow tool for `hermes-skills`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
2. **Use a Skill** (slash command):
   ...
   /github create PR for auth refactor
...
4. **Create Custom Skill**:
   ...
   # Scaffolds $LOCALAPPDATA/hermes/skills/my-skill/ with SKILL.md + references/
   ```
...
6. **Manage Skills**:
   ```bash
   /skill update                     # Check for updates (interactive)
   hermes skills update              # Apply all pending updates
...
- **`hermes skills update` with 50+ pending updates can timeout** → Each update triggers a security scan (2-5s per skill). 75 updates can take 3-8 minutes. Run in background mode with `background=true, notify_on_complete=true` and a generous timeout (300s+) rather than a short foreground call. For very large batches, consider looping `hermes skills install <identifier> --yes` for individual skills instead of the bulk hammer. See `references/batch-install.md` for the loop pattern.
- **Community prompt-engineering/optimization skills often blocked by security scan** → These skills frequently mention agent configuration files (how-to docs explain agent behaviour) or include robustness test patterns in examples. The scanner treats documentation as actual config modification risk. Attempted installs from 3 different authors all received DANGEROUS verdicts. See `references/community-skill-install.md` for workarounds and which skill types tend to pass.
...
| Examples | `official/github/github` | `skills-sh/agents365-ai/365-skills/drawio-skill` |
...
- `references/community-skill-install.md` — Community skill blocking patterns, multi-source install reference
...
- [ ] Hermes Skills — Discovery, Creation, Hub Installation & Management operations completed successfully
...
Set up required environment, dependencies, and configuration for Hermes Skills — Discovery, Creation, Hub Installation & Management.

### Phase 2: Execution

Run the primary Hermes Skills — Discovery, Creation, Hub Installation & Management operations according to the defined requirements.
