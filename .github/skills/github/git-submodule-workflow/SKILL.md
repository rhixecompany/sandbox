...
git config -f .gitmodules submodule."projects/repo-a".url "https://github.com/org/repo-a.git"
git config -f .gitmodules submodule."projects/repo-a".branch "main"
```

**Verify:** Check `.gitmodules` after each addition:
```bash
cat .gitmodules
```

### Step 2.3 — Initialize & Update Submodules

Initialize all submodules (downloads their repositories):
...
**Expected output:**
```
Cloning into 'projects/repo-a'...
Cloning into 'projects/repo-b'...
...
Submodule path 'projects/repo-a': checked out '<commit-sha>'
```

**Pitfall — "fatal: destination path 'projects/repo-a' already exists":** The directory already exists on disk from the earlier `git rm --cached`. This is fine. The submodule init will reuse the existing directory. If git refuses, move the directory temporarily:
```bash
mv projects/repo-a projects/repo-a.backup
git submodule update --init --recursive
# If successful, rm -rf projects/repo-a.backup
```
...
**Expected output:**
```
 <commit-hash> projects/repo-a (main)
 <commit-hash> projects/repo-b (develop)
```

**If status shows "modified content, untracked content":** This is normal after fresh conversion from local directories. Submodules are initialized but may have local changes or untracked files. No action required unless you want to synchronize state.
