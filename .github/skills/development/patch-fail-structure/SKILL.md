---
name: patch-fail-structure
title: Patch Fail Structure
description: Patches fail structure in skill files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `patch_fail_structure.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/patch_fail_structure.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/patch_fail_structure.py`

**Type:** Python

**Usage:**
```bash
python patch_fail_structure.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python patch_fail_structure.py [options]
```

## When to Use

- When you need to perform patch fail structure as part of your workflow
