...
name: skill-audit
title: Skill Audit
description: Script for skill audit
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `skill_audit.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/skill_audit.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/skill_audit.py`

**Type:** Python

**Usage:**
```bash
python skill_audit.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python skill_audit.py [options]
```

## When to Use

- When you need to perform skill audit as part of your workflow
