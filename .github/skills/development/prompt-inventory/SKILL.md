---
name: prompt-inventory
title: Prompt Inventory
description: Performs inventory on prompts
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `prompt-inventory.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/prompt-inventory.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/prompt-inventory.py`

**Type:** Python

**Usage:**
```bash
python prompt-inventory.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python prompt-inventory.py [options]
```

## When to Use

- When you need to perform prompt inventory as part of your workflow
