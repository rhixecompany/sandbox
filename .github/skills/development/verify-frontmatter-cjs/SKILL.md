---
name: verify-frontmatter-cjs
title: Verify Frontmatter
description: Verifies frontmatter correctness across skill files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
The script is located at `~/AppData/Local/hermes/scripts/verify-frontmatter.cjs`.
...
**Usage:**
```bash
node verify-frontmatter.cjs [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && node verify-frontmatter.cjs [options]
```

## When to Use

- When you need to perform verify frontmatter as part of your workflow
