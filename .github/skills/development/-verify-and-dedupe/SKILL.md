---
name: -verify-and-dedupe
title: Verify And Dedupe
description: Script for Verify And Dedupe operations
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `_verify_and_dedupe.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/_verify_and_dedupe.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/_verify_and_dedupe.py`

**Type:** Python

**Usage:**
```bash
python _verify_and_dedupe.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python _verify_and_dedupe.py [options]
```

## When to Use

- When you need to perform verify and dedupe as part of your workflow
