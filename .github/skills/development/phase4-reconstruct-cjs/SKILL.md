---
name: phase4-reconstruct-cjs
title: Phase4 Reconstruct
description: Phase 4 reconstruction script for skill auditing
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `phase4-reconstruct.cjs` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/phase4-reconstruct.cjs`.
...
**Usage:**
```bash
node phase4-reconstruct.cjs [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && node phase4-reconstruct.cjs [options]
```

## When to Use

- When you need to perform phase4 reconstruct as part of your workflow
...
- When the environment does not support Node.js (CommonJS) execution
