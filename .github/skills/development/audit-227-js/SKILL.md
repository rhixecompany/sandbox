...
Wrapper skill for the `audit-227.js` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/audit-227.js`.
...
**Usage:**
```bash
node audit-227.js [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && node audit-227.js [options]
```
...
- When the environment does not support Node.js execution
