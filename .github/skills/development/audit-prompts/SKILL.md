...
name: audit-prompts
title: Audit Prompts
description: Audits prompts for issues and quality concerns
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `audit_prompts.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/audit_prompts.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/audit_prompts.py`

**Type:** Python

**Usage:**
```bash
python audit_prompts.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python audit_prompts.py [options]
```

## When to Use

- When you need to perform audit prompts as part of your workflow
