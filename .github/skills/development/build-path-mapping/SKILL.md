---
name: build-path-mapping
title: Build Path Mapping
description: Builds path mappings between script and skill locations
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `build_path_mapping.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/build_path_mapping.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/build_path_mapping.py`

**Type:** Python

**Usage:**
```bash
python build_path_mapping.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python build_path_mapping.py [options]
```

## When to Use

- When you need to perform build path mapping as part of your workflow
