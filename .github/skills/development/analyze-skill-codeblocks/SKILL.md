---
name: analyze-skill-codeblocks
title: Analyze Skill Codeblocks
description: Analyzes HuggingFace model metadata and configurations
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `analyze_skill_codeblocks.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/analyze_skill_codeblocks.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/analyze_skill_codeblocks.py`

**Type:** Python

**Usage:**
```bash
python analyze_skill_codeblocks.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python analyze_skill_codeblocks.py [options]
```

## When to Use

- When you need to perform analyze skill codeblocks as part of your workflow
