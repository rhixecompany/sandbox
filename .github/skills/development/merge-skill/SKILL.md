...
name: merge-skill
title: Merge Skill
description: Merges skill files or configurations
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `merge_skill.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/merge_skill.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/merge_skill.py`

**Type:** Python

**Usage:**
```bash
python merge_skill.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python merge_skill.py [options]
```

## When to Use

- When you need to perform merge skill as part of your workflow
