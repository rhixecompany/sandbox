...
name: fix-prompts
title: Fix Prompts
description: Fixes prompts issues across files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `fix_prompts.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/fix_prompts.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/fix_prompts.py`

**Type:** Python

**Usage:**
```bash
python fix_prompts.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python fix_prompts.py [options]
```

## When to Use

- When you need to perform fix prompts as part of your workflow
