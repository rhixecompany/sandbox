...
description: Generates context files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `generate_context_files.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/generate_context_files.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/generate_context_files.py`

**Type:** Python

**Usage:**
```bash
python generate_context_files.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python generate_context_files.py [options]
```

## When to Use

- When you need to perform generate context files as part of your workflow
