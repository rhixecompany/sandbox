---
name: fix-fail-skills
title: Fix Fail Skills
description: Fixes fail skills issues across files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `fix_fail_skills.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/fix_fail_skills.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/fix_fail_skills.py`

**Type:** Python

**Usage:**
```bash
python fix_fail_skills.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python fix_fail_skills.py [options]
```

## When to Use

- When you need to perform fix fail skills as part of your workflow
