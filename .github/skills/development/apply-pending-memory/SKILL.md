---
name: apply-pending-memory
title: Apply Pending Memory
description: Applies VS Code customizations from templates
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `apply_pending_memory.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/apply_pending_memory.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/apply_pending_memory.py`

**Type:** Python

**Usage:**
```bash
python apply_pending_memory.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python apply_pending_memory.py [options]
```

## When to Use

- When you need to perform apply pending memory as part of your workflow
