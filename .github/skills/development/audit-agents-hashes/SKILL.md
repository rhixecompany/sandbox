---
name: audit-agents-hashes
title: Audit Agents Hashes
description: Audit script for agents-hashes analysis
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `audit-agents-hashes.ps1` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/audit-agents-hashes.ps1`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/audit-agents-hashes.ps1`

**Type:** PowerShell

**Usage:**
```bash
powershell -File audit-agents-hashes.ps1 [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && powershell -File audit-agents-hashes.ps1 [options]
```

## When to Use

- When you need to perform audit agents hashes as part of your workflow
...
- When the environment does not support PowerShell execution
