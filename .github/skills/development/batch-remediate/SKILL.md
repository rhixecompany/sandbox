---
name: batch-remediate
title: Batch Remediate
description: Batch processing for remediate
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `batch_remediate.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/batch_remediate.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/batch_remediate.py`

**Type:** Python

**Usage:**
```bash
python batch_remediate.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python batch_remediate.py [options]
```

## When to Use

- When you need to perform batch remediate as part of your workflow
