...
title: Quarantine Skills
description: Quarantines low-quality or failing skills
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `quarantine_skills.sh` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/quarantine_skills.sh`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/quarantine_skills.sh`

**Type:** Bash

**Usage:**
```bash
bash quarantine_skills.sh [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && bash quarantine_skills.sh [options]
```

## When to Use

- When you need to perform quarantine skills as part of your workflow
...
- When the environment does not support Bash execution
