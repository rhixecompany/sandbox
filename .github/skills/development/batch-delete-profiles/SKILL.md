---
name: batch-delete-profiles
title: Batch Delete Profiles
description: Batch processing for delete profiles
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `batch_delete_profiles.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/batch_delete_profiles.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/batch_delete_profiles.py`

**Type:** Python

**Usage:**
```bash
python batch_delete_profiles.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python batch_delete_profiles.py [options]
```

## When to Use

- When you need to perform batch delete profiles as part of your workflow
