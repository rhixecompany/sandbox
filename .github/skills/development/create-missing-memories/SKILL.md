...
title: Create Missing Memories
description: Creates missing memories files for profiles
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `create_missing_memories.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/create_missing_memories.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/create_missing_memories.py`

**Type:** Python

**Usage:**
```bash
python create_missing_memories.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python create_missing_memories.py [options]
```

## When to Use

- When you need to perform create missing memories as part of your workflow
