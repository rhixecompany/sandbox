---
name: inventory-prompts
title: Inventory Prompts
description: Creates inventory of prompts in the system
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `inventory_prompts.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/inventory_prompts.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/inventory_prompts.py`

**Type:** Python

**Usage:**
```bash
python inventory_prompts.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python inventory_prompts.py [options]
```

## When to Use

- When you need to perform inventory prompts as part of your workflow
