---
name: list-cli-tools
title: List Cli Tools
description: Lists available cli-tools
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `list-cli-tools.sh` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/list-cli-tools.sh`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/list-cli-tools.sh`

**Type:** Bash

**Usage:**
```bash
bash list-cli-tools.sh [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && bash list-cli-tools.sh [options]
```

## When to Use

- When you need to perform list cli tools as part of your workflow
