...
description: Validates prompts for correctness
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `validate_prompts.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/validate_prompts.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/validate_prompts.py`

**Type:** Python

**Usage:**
```bash
python validate_prompts.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python validate_prompts.py [options]
```

## When to Use

- When you need to perform validate prompts as part of your workflow
