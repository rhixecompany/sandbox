---
name: patch-all-fail-sections
title: Patch All Fail Sections
description: Patches all fail sections in skill files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `patch_all_fail_sections.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/patch_all_fail_sections.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/patch_all_fail_sections.py`

**Type:** Python

**Usage:**
```bash
python patch_all_fail_sections.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python patch_all_fail_sections.py [options]
```

## When to Use

- When you need to perform patch all fail sections as part of your workflow
