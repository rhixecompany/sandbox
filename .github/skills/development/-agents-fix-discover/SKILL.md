---
name: -agents-fix-discover
title: Agents Fix Discover
description: Script for Agents Fix Discover operations
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `_agents_fix_discover.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/_agents_fix_discover.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/_agents_fix_discover.py`

**Type:** Python

**Usage:**
```bash
python _agents_fix_discover.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python _agents_fix_discover.py [options]
```

## When to Use

- When you need to perform agents fix discover as part of your workflow
