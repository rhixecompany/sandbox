...
title: Create Missing Souls
description: Creates missing souls files for profiles
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `create_missing_souls.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/create_missing_souls.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/create_missing_souls.py`

**Type:** Python

**Usage:**
```bash
python create_missing_souls.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python create_missing_souls.py [options]
```

## When to Use

- When you need to perform create missing souls as part of your workflow
