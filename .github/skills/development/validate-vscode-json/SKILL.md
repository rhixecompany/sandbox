...
title: Validate Vscode Json
description: Validates vscode json for correctness
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `validate_vscode_json.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/validate_vscode_json.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/validate_vscode_json.py`

**Type:** Python

**Usage:**
```bash
python validate_vscode_json.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python validate_vscode_json.py [options]
```

## When to Use

- When you need to perform validate vscode json as part of your workflow
