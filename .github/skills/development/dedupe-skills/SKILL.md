...
name: dedupe-skills
title: Dedupe Skills
description: Deduplicates overlapping or redundant skills
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `dedupe_skills.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/dedupe_skills.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/dedupe_skills.py`

**Type:** Python

**Usage:**
```bash
python dedupe_skills.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python dedupe_skills.py [options]
```

## When to Use

- When you need to perform dedupe skills as part of your workflow
