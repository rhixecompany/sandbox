---
name: prompt-batch-audit
title: Prompt Batch Audit
description: Performs batch audit on prompt files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `prompt_batch_audit.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/prompt_batch_audit.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/prompt_batch_audit.py`

**Type:** Python

**Usage:**
```bash
python prompt_batch_audit.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python prompt_batch_audit.py [options]
```

## When to Use

- When you need to perform prompt batch audit as part of your workflow
