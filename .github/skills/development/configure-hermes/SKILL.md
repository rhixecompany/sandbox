...
description: Configures Hermes settings and provider integrations
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `configure_hermes.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/configure_hermes.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/configure_hermes.py`

**Type:** Python

**Usage:**
```bash
python configure_hermes.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python configure_hermes.py [options]
```

## When to Use

- When you need to perform configure hermes as part of your workflow
