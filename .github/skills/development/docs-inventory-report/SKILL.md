---
name: docs-inventory-report
title: Docs Inventory Report
description: Generates documentation inventory reports
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `docs-inventory-report.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/docs-inventory-report.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/docs-inventory-report.py`

**Type:** Python

**Usage:**
```bash
python docs-inventory-report.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python docs-inventory-report.py [options]
```

## When to Use

- When you need to perform docs inventory report as part of your workflow
