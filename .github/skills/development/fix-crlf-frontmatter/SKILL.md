---
name: fix-crlf-frontmatter
title: Fix Crlf Frontmatter
description: Fixes crlf frontmatter issues across files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `fix_crlf_frontmatter.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/fix_crlf_frontmatter.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/fix_crlf_frontmatter.py`

**Type:** Python

**Usage:**
```bash
python fix_crlf_frontmatter.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python fix_crlf_frontmatter.py [options]
```

## When to Use

- When you need to perform fix crlf frontmatter as part of your workflow
