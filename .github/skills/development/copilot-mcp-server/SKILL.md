...
description: Sets up Copilot MCP server integration
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `copilot_mcp_server.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/copilot_mcp_server.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/copilot_mcp_server.py`

**Type:** Python

**Usage:**
```bash
python copilot_mcp_server.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python copilot_mcp_server.py [options]
```

## When to Use

- When you need to perform copilot mcp server as part of your workflow
