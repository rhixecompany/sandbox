---
name: generate-vscode-configs
title: Generate Vscode Configs
description: Generates vscode configs
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `generate_vscode_configs.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/generate_vscode_configs.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/generate_vscode_configs.py`

**Type:** Python

**Usage:**
```bash
python generate_vscode_configs.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python generate_vscode_configs.py [options]
```

## When to Use

- When you need to perform generate vscode configs as part of your workflow
