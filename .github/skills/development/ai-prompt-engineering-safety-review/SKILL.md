---
author: Hermes Agent
description: Review AI-generated prompts for safety issues, bias, injection risks, or harmful content before deployment. Use during prompt auditing, red-teaming, or pre-deployment validation of LLM prompts.
license: MIT
metadata:
  hermes:
    tags: [imported, safety, prompt-engineering, red-teaming, llm, audit]
...
```bash
# Run safety review on a prompt file
ai-prompt-engineering-safety-review --input prompts/my-prompt.prompt.md

# Batch review a directory
ai-prompt-engineering-safety-review --dir prompts/ --output audit-report.md

# Dry run to preview findings
ai-prompt-engineering-safety-review --input prompt.prompt.md --dry-run
```

## Error Handling

- **Prompt file not found:** Exits with code 1, prints error to stderr
- **Invalid frontmatter:** Warns, continues with content-only analysis
- **Dry-run mode:** Uses `--dry-run` flag, outputs plan without modifying files
...
- `prompt-engineering` — Create safe prompts
- `enhance-markdown` — Improve prompt structure
- `godmode` — Attack vector knowledge
...
- `references/injection-patterns.md` — Known injection patterns and detection rules
- `references/bias-checklist.md` — Demographic bias detection checklist
- `references/leakage-vectors.md` — Data exfiltration patterns to watch for
- `references/model-specific-notes.md` — Safety behavior differences across models