---
name: normalize-prompt-actions
title: Normalize Prompt Actions
description: Normalizes prompt actions formatting
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `normalize_prompt_actions.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/normalize_prompt_actions.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/normalize_prompt_actions.py`

**Type:** Python

**Usage:**
```bash
python normalize_prompt_actions.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python normalize_prompt_actions.py [options]
```

## When to Use

- When you need to perform normalize prompt actions as part of your workflow
