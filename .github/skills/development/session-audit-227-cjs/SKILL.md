---
name: session-audit-227-cjs
title: Session Audit 227
description: Session audit script for Hermes sessions
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `session-audit-227.cjs` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/session-audit-227.cjs`.
...
**Usage:**
```bash
node session-audit-227.cjs [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && node session-audit-227.cjs [options]
```

## When to Use

- When you need to perform session audit 227 as part of your workflow
