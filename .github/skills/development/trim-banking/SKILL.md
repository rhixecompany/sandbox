...
name: trim-banking
title: Trim Banking
description: Trims banking content from files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `trim_banking.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/trim_banking.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/trim_banking.py`

**Type:** Python

**Usage:**
```bash
python trim_banking.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python trim_banking.py [options]
```

## When to Use

- When you need to perform trim banking as part of your workflow
