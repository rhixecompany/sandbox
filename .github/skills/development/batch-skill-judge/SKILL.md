---
name: batch-skill-judge
title: Batch Skill Judge
description: Batch processing for skill judge
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `batch_skill_judge.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/batch_skill_judge.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/batch_skill_judge.py`

**Type:** Python

**Usage:**
```bash
python batch_skill_judge.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python batch_skill_judge.py [options]
```

## When to Use

- When you need to perform batch skill judge as part of your workflow
