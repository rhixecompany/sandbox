...
name: find-lock
title: Find Lock
description: Finds lock in the system
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `find_lock.ps1` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/find_lock.ps1`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/find_lock.ps1`

**Type:** PowerShell

**Usage:**
```bash
powershell -File find_lock.ps1 [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && powershell -File find_lock.ps1 [options]
```

## When to Use

- When you need to perform find lock as part of your workflow
