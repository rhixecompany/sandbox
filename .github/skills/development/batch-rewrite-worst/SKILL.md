---
name: batch-rewrite-worst
title: Batch Rewrite Worst
description: Batch processing for rewrite worst
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `batch_rewrite_worst.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/batch_rewrite_worst.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/batch_rewrite_worst.py`

**Type:** Python

**Usage:**
```bash
python batch_rewrite_worst.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python batch_rewrite_worst.py [options]
```

## When to Use

- When you need to perform batch rewrite worst as part of your workflow
