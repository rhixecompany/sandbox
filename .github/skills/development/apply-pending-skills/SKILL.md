---
name: apply-pending-skills
title: Apply Pending Skills
...
Wrapper skill for the `apply_pending_skills.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/apply_pending_skills.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/apply_pending_skills.py`

**Type:** Python

**Usage:**
```bash
python apply_pending_skills.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python apply_pending_skills.py [options]
```

## When to Use

- When you need to perform apply pending skills as part of your workflow
