...
title: Review Pending Skills
description: Script for Review Pending Skills operations
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `review_pending_skills.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/review_pending_skills.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/review_pending_skills.py`

**Type:** Python

**Usage:**
```bash
python review_pending_skills.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python review_pending_skills.py [options]
```

## When to Use

- When you need to perform review pending skills as part of your workflow
