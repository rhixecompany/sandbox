...
Wrapper skill for the `generate_skills.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/generate_skills.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/generate_skills.py`

**Type:** Python

**Usage:**
```bash
python generate_skills.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python generate_skills.py [options]
```

## When to Use

- When you need to perform generate skills as part of your workflow
