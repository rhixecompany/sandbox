---
name: fix-yaml-frontmatter
title: Fix Yaml Frontmatter
description: Fixes yaml frontmatter issues across files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `fix_yaml_frontmatter.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/fix_yaml_frontmatter.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/fix_yaml_frontmatter.py`

**Type:** Python

**Usage:**
```bash
python fix_yaml_frontmatter.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python fix_yaml_frontmatter.py [options]
```

## When to Use

- When you need to perform fix yaml frontmatter as part of your workflow
