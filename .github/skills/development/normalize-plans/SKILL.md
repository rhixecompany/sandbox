---
name: normalize-plans
title: Normalize Plans
description: Normalizes plans formatting
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `normalize_plans.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/normalize_plans.py`.
...
**Usage:**
```bash
python normalize_plans.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python normalize_plans.py [options]
```

## When to Use

- When you need to perform normalize plans as part of your workflow
