...
name: fix-skills
title: Fix Skills
description: Fixes skills issues
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `fix-skills.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/fix-skills.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/fix-skills.py`

**Type:** Python

**Usage:**
```bash
python fix-skills.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python fix-skills.py [options]
```

## When to Use

- When you need to perform fix skills as part of your workflow
