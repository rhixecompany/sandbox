...
description: Updates agents.md file with current configuration
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `update_agents_md.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/update_agents_md.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/update_agents_md.py`

**Type:** Python

**Usage:**
```bash
python update_agents_md.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python update_agents_md.py [options]
```

## When to Use

- When you need to perform update agents md as part of your workflow
