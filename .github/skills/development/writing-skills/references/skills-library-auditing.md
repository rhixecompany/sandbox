...
ls -d $LOCALAPPDATA/hermes/skills/*/SKILL.md

# Categorized skills count by category
ls -d $LOCALAPPDATA/hermes/skills/*/*/SKILL.md | cut -d/ -f9 | sort | uniq -c | sort -rn
```
...
MSYS-style absolute paths (`/c/Users/Alexa/AppData/Local/hermes/...`) get baked into skill docs during copy-paste from terminal output. Replace with `$HOME/AppData/Local/hermes/` for portability.

**Detect it:**
```bash
# Root skills
grep -rn '/c/Users/Alexa/AppData/Local/hermes' $LOCALAPPDATA/hermes/skills/ --include='*.md' 2>/dev/null

# Profile copies
grep -rn '/c/Users/Alexa/AppData/Local/hermes' $LOCALAPPDATA/hermes/profiles/ --include='*.md' 2>/dev/null
```

**Fix (root skills):**
```bash
find $LOCALAPPDATA/hermes/skills -name "*.md" \
  -not -path "*index-cache*" -not -path "*__pycache__*" \
  -exec sed -i 's|/c/Users/Alexa/AppData/Local/hermes|$LOCALAPPDATA/hermes|g' {} +
```

**Fix (profile copies — per-profile to avoid timeout):**
```bash
# Do NOT bulk-sed all profiles at once — 60+ files times out.
# Iterate per-profile instead:
for profile_dir in $LOCALAPPDATA/hermes/profiles/*/skills; do
  find "$profile_dir" -name "*.md" -exec sed -i \
    's|/c/Users/Alexa/AppData/Local/hermes|$LOCALAPPDATA/hermes|g' {} +
done
```

**Pitfall:** Bulk `sed` on 60+ profile files in one call times out (180s limit). Always batch per-profile (~11 files each).

**Note:** The `df -h /c/Users/Alexa` command (without `/AppData/Local/hermes` suffix) should NOT be replaced — it references the user home, not the hermes directory.
...
- [ ] No hardcoded MSYS paths — use `$HOME/AppData/Local/hermes` instead
