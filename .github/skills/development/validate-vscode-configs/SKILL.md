---
name: validate-vscode-configs
title: Validate Vscode Configs
description: Validates vscode configs for correctness
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
# Validate Vscode Configs

## Overview

Wrapper skill for the `validate_vscode_configs.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/validate_vscode_configs.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/validate_vscode_configs.py`

**Type:** Python

**Usage:**
```bash
python validate_vscode_configs.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python validate_vscode_configs.py [options]
```

## When to Use

- When you need to perform validate vscode configs as part of your workflow
