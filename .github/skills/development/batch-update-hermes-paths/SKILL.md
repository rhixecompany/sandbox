---
name: batch-update-hermes-paths
title: Batch Update Hermes Paths
description: Batch processing for update hermes paths
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `batch_update_hermes_paths.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/batch_update_hermes_paths.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/batch_update_hermes_paths.py`

**Type:** Python

**Usage:**
```bash
python batch_update_hermes_paths.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python batch_update_hermes_paths.py [options]
```

## When to Use

- When you need to perform batch update hermes paths as part of your workflow
