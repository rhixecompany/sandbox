---
name: remove-flat-duplicates
title: Remove Flat Duplicates
description: Script for Remove Flat Duplicates operations
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `remove_flat_duplicates.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/remove_flat_duplicates.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/remove_flat_duplicates.py`

**Type:** Python

**Usage:**
```bash
python remove_flat_duplicates.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python remove_flat_duplicates.py [options]
```

## When to Use

- When you need to perform remove flat duplicates as part of your workflow
