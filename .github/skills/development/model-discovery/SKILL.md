...
description: Discovers or validates available LLM models
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `model_discovery.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/model_discovery.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/model_discovery.py`

**Type:** Python

**Usage:**
```bash
python model_discovery.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python model_discovery.py [options]
```

## When to Use

- When you need to perform model discovery as part of your workflow
