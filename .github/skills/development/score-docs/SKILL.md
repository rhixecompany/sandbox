...
name: score-docs
title: Score Docs
description: Scores documentation quality metrics
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `score-docs.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/score-docs.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/score-docs.py`

**Type:** Python

**Usage:**
```bash
python score-docs.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python score-docs.py [options]
```

## When to Use

- When you need to perform score docs as part of your workflow
