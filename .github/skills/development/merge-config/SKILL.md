...
name: merge-config
title: Merge Config
description: Merges config files or configurations
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `merge_config.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/merge_config.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/merge_config.py`

**Type:** Python

**Usage:**
```bash
python merge_config.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python merge_config.py [options]
```

## When to Use

- When you need to perform merge config as part of your workflow
