---
name: fix-eslint-mismatches
title: Fix Eslint Mismatches
description: Fixes eslint mismatches issues across files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `fix_eslint_mismatches.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/fix_eslint_mismatches.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/fix_eslint_mismatches.py`

**Type:** Python

**Usage:**
```bash
python fix_eslint_mismatches.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python fix_eslint_mismatches.py [options]
```

## When to Use

- When you need to perform fix eslint mismatches as part of your workflow
