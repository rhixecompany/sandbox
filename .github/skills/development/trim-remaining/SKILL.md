---
name: trim-remaining
title: Trim Remaining
description: Trims remaining content from files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `trim_remaining.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/trim_remaining.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/trim_remaining.py`

**Type:** Python

**Usage:**
```bash
python trim_remaining.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python trim_remaining.py [options]
```

## When to Use

- When you need to perform trim remaining as part of your workflow
