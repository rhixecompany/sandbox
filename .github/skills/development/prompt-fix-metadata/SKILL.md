---
name: prompt-fix-metadata
title: Prompt Fix Metadata
description: Performs fix metadata on prompt files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `prompt_fix_metadata.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/prompt_fix_metadata.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/prompt_fix_metadata.py`

**Type:** Python

**Usage:**
```bash
python prompt_fix_metadata.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python prompt_fix_metadata.py [options]
```

## When to Use

- When you need to perform prompt fix metadata as part of your workflow
