...
name: skills-audit
title: Skills Audit
description: Performs audit on skill files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `skills-audit.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/skills-audit.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/skills-audit.py`

**Type:** Python

**Usage:**
```bash
python skills-audit.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python skills-audit.py [options]
```

## When to Use

- When you need to perform skills audit as part of your workflow
