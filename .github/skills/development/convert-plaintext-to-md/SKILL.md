---
author: Hermes Agent
description: Convert plain text files, notes, or unstructured content into well-formatted Markdown with proper headings, lists, code blocks, and structure
license: MIT
metadata:
  hermes:
    tags: [imported, markdown, conversion, documentation, formatting]
...
Convert plain text files, notes, or unstructured content into well-formatted Markdown with proper headings, lists, code blocks, and structure. This skill provides a systematic approach to text-to-markdown conversion with resumability and verification.
...
- `documentation-writer` — Full documentation creation
- `enhance-markdown` — Formatting polish

## Usage Examples

```bash
# Convert a plain text file
convert-plaintext-to-md --input notes.txt --output notes.md

# Dry run to preview
convert-plaintext-to-md --input notes.txt --dry-run

# Batch convert a directory
convert-plaintext-to-md --dir ./plain-notes --out-dir ./markdown-notes

# Specify heading style
convert-plaintext-to-md --input notes.txt --heading-style atx
```

## Error Handling

- **File not found:** Exits with code 1, prints error to stderr
- **Empty file:** Warns, outputs minimal markdown with frontmatter only
- **Permission denied:** Exits with code 2, prints error to stderr
