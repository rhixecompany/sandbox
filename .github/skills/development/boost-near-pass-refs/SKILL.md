---
name: boost-near-pass-refs
title: Boost Near Pass Refs
description: Boosts quality scores for near-passing skill references
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `boost_near_pass_refs.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/boost_near_pass_refs.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/boost_near_pass_refs.py`

**Type:** Python

**Usage:**
```bash
python boost_near_pass_refs.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python boost_near_pass_refs.py [options]
```

## When to Use

- When you need to perform boost near pass refs as part of your workflow
