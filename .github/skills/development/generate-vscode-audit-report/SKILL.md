---
name: generate-vscode-audit-report
title: Generate Vscode Audit Report
description: Generates vscode audit report
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `generate_vscode_audit_report.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/generate_vscode_audit_report.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/generate_vscode_audit_report.py`

**Type:** Python

**Usage:**
```bash
python generate_vscode_audit_report.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python generate_vscode_audit_report.py [options]
```

## When to Use

- When you need to perform generate vscode audit report as part of your workflow
