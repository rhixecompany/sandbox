---
name: fix-duplicate-hooks
title: Fix Duplicate Hooks
description: Fixes duplicate hooks issues across files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `fix_duplicate_hooks.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/fix_duplicate_hooks.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/fix_duplicate_hooks.py`

**Type:** Python

**Usage:**
```bash
python fix_duplicate_hooks.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python fix_duplicate_hooks.py [options]
```

## When to Use

- When you need to perform fix duplicate hooks as part of your workflow
