...
name: check-handle
title: Check Handle
description: Checks for handle or resource availability
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `check_handle.ps1` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/check_handle.ps1`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/check_handle.ps1`

**Type:** PowerShell

**Usage:**
```bash
powershell -File check_handle.ps1 [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && powershell -File check_handle.ps1 [options]
```

## When to Use

- When you need to perform check handle as part of your workflow
