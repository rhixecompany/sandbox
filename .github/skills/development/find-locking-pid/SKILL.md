---
name: find-locking-pid
title: Find Locking Pid
description: Finds locking pid in the system
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `find_locking_pid.ps1` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/find_locking_pid.ps1`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/find_locking_pid.ps1`

**Type:** PowerShell

**Usage:**
```bash
powershell -File find_locking_pid.ps1 [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && powershell -File find_locking_pid.ps1 [options]
```

## When to Use

- When you need to perform find locking pid as part of your workflow
