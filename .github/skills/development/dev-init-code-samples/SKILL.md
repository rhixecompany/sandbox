---
name: dev-init-code-samples
title: Dev Init Code Samples
description: Initializes development code samples for reference
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `dev-init-code-samples.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/dev-init-code-samples.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/dev-init-code-samples.py`

**Type:** Python

**Usage:**
```bash
python dev-init-code-samples.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python dev-init-code-samples.py [options]
```

## When to Use

- When you need to perform dev init code samples as part of your workflow
