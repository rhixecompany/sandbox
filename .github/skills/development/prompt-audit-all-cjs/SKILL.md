---
name: prompt-audit-all-cjs
title: Prompt Audit All
description: Performs audit-all on prompts
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
The script is located at `~/AppData/Local/hermes/scripts/prompt-audit-all.cjs`.
...
**Usage:**
```bash
node prompt-audit-all.cjs [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && node prompt-audit-all.cjs [options]
```

## When to Use

- When you need to perform prompt audit all as part of your workflow
