---
name: fix-frontmatter-yaml-cjs
title: Fix Frontmatter Yaml
description: Fixes frontmatter-yaml issues
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `fix-frontmatter-yaml.cjs` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/fix-frontmatter-yaml.cjs`.
...
**Usage:**
```bash
node fix-frontmatter-yaml.cjs [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && node fix-frontmatter-yaml.cjs [options]
```

## When to Use

- When you need to perform fix frontmatter yaml as part of your workflow
