...
description: Inventories agents
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `inventory-agents.ps1` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/inventory-agents.ps1`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/inventory-agents.ps1`

**Type:** PowerShell

**Usage:**
```bash
powershell -File inventory-agents.ps1 [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && powershell -File inventory-agents.ps1 [options]
```

## When to Use

- When you need to perform inventory agents as part of your workflow
