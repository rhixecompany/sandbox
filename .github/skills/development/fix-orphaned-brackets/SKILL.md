---
name: fix-orphaned-brackets
title: Fix Orphaned Brackets
description: Fixes orphaned brackets issues across files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `fix_orphaned_brackets.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/fix_orphaned_brackets.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/fix_orphaned_brackets.py`

**Type:** Python

**Usage:**
```bash
python fix_orphaned_brackets.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python fix_orphaned_brackets.py [options]
```

## When to Use

- When you need to perform fix orphaned brackets as part of your workflow
