---
name: validate-prompt-inventory
title: Validate Prompt Inventory
description: Validates prompt inventory for correctness
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
# Validate Prompt Inventory

## Overview

Wrapper skill for the `validate_prompt_inventory.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/validate_prompt_inventory.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/validate_prompt_inventory.py`

**Type:** Python

**Usage:**
```bash
python validate_prompt_inventory.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python validate_prompt_inventory.py [options]
```

## When to Use

- When you need to perform validate prompt inventory as part of your workflow
