...
name: lcs
title: Lcs
description: Longest common subsequence utility for string comparison
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `lcs.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/lcs.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/lcs.py`

**Type:** Python

**Usage:**
```bash
python lcs.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python lcs.py [options]
```

## When to Use

- When you need to perform lcs as part of your workflow
