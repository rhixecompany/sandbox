---
name: prompt-repair-frontmatter
title: Prompt Repair Frontmatter
description: Performs repair-frontmatter on prompts
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `prompt-repair-frontmatter.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/prompt-repair-frontmatter.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/prompt-repair-frontmatter.py`

**Type:** Python

**Usage:**
```bash
python prompt-repair-frontmatter.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python prompt-repair-frontmatter.py [options]
```

## When to Use

- When you need to perform prompt repair frontmatter as part of your workflow
