...
description: Fixes frontmatter issues
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `fix-frontmatter.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/fix-frontmatter.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/fix-frontmatter.py`

**Type:** Python

**Usage:**
```bash
python fix-frontmatter.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python fix-frontmatter.py [options]
```

## When to Use

- When you need to perform fix frontmatter as part of your workflow
