---
name: repair-frontmatter-closes
title: Repair Frontmatter Closes
description: Repairs frontmatter closure issues in YAML
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `repair-frontmatter-closes.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/repair-frontmatter-closes.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/repair-frontmatter-closes.py`

**Type:** Python

**Usage:**
```bash
python repair-frontmatter-closes.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python repair-frontmatter-closes.py [options]
```

## When to Use

- When you need to perform repair frontmatter closes as part of your workflow
