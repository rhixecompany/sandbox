---
name: audit-vscode-config
title: Audit Vscode Config
description: Audits vscode config for issues and quality concerns
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `audit_vscode_config.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/audit_vscode_config.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/audit_vscode_config.py`

**Type:** Python

**Usage:**
```bash
python audit_vscode_config.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python audit_vscode_config.py [options]
```

## When to Use

- When you need to perform audit vscode config as part of your workflow
