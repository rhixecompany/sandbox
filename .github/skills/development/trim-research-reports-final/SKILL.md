---
name: trim-research-reports-final
title: Trim Research Reports Final
description: Trims research reports final content from files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `trim_research_reports_final.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/trim_research_reports_final.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/trim_research_reports_final.py`

**Type:** Python

**Usage:**
```bash
python trim_research_reports_final.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python trim_research_reports_final.py [options]
```

## When to Use

- When you need to perform trim research reports final as part of your workflow
