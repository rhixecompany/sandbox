---
name: fix-copilot-frontmatter
title: Fix Copilot Frontmatter
description: Fixes copilot frontmatter issues across files
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `fix_copilot_frontmatter.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/fix_copilot_frontmatter.py`.
...
**Usage:**
```bash
python fix_copilot_frontmatter.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python fix_copilot_frontmatter.py [options]
```

## When to Use

- When you need to perform fix copilot frontmatter as part of your workflow
