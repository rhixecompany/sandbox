---
name: -agents-fix-report
title: Agents Fix Report
description: Script for Agents Fix Report operations
version: 1.0.0
author: Hermes Agent
license: MIT
tags:
...
Wrapper skill for the `_agents_fix_report.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/_agents_fix_report.py`.

## Script Reference

**Location:** `~/AppData/Local/hermes/scripts/_agents_fix_report.py`

**Type:** Python

**Usage:**
```bash
python _agents_fix_report.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python _agents_fix_report.py [options]
```

## When to Use

- When you need to perform agents fix report as part of your workflow
