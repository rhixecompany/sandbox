...
Wrapper skill for the `benchmark_providers.py` script in `~/AppData/Local/hermes/scripts/`.

The script is located at `~/AppData/Local/hermes/scripts/benchmark_providers.py`.
...
**Usage:**
```bash
python benchmark_providers.py [options]
```

Run from the scripts directory or use the full path:
```bash
cd ~/AppData/Local/hermes/scripts && python benchmark_providers.py [options]
```

## When to Use

- When you need to perform benchmark providers as part of your workflow
