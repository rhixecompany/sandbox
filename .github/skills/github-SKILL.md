...
- When you need to automate or structure workflows for `github`.
...
Automated reasoning and workflow tool for `github`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
- [ ] "GitHub Workflow (auth, PRs, code review, issues)" operations completed successfully
...
Set up required environment, dependencies, and configuration for "GitHub Workflow (auth, PRs, code review, issues)".

### Phase 2: Execution

Run the primary "GitHub Workflow (auth, PRs, code review, issues)" operations according to the defined requirements.
