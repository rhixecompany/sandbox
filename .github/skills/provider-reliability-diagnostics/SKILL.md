---
author: Hermes Agent
description: 'Diagnose and fix provider chain failures in Hermes: trace failures across
  main → fallback → auxiliary → aggregator layers, validate provider health via ACPX
  exec, fix MCP transport mismatches, and configure auxiliary fallbacks.'
license: MIT
...
- mcp
- fallback
title: Provider Reliability Diagnostics — Chain Tracing, ACPX Validation, Auxiliary
  Fallback
trigger: '• User says "provider not working", "API failing", "model not responding"

  • errors.log shows repeated provider failure patterns (ConnectionError, 402, 429,
  401)
...
Automated reasoning and workflow tool for `provider-reliability-diagnostics`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
grep -E "WARNING|ERROR" $LOCALAPPDATA/hermes/logs/errors.log \
...
grep -A2 "auxiliary" $LOCALAPPDATA/hermes/config.yaml

# 2. Re-run provider health check
acpx copilot exec "Reply with exactly: COPILOT_ALIVE"

# 3. Confirm MCP servers load cleanly (no Docker warnings)
tail -20 $LOCALAPPDATA/hermes/logs/gateway.log | grep "MCP"
...
- `references/model-catalog-free-models.md` — Model catalog endpoint, free model list (27 OpenRouter models), benchmark template
- `scripts/test-free-models.py` — Runnable benchmark script for free models via opencode-zen endpoint

## Related Scripts (global)

`~/AppData/Local/hermes/scripts/test_models.py` — Multi-provider benchmark harness with `--list-providers` and `--hermes` flags. Covers all 6 configured providers.

## Workflow

### Phase 1: Preparation

Set up required environment, dependencies, and configuration for Provider Reliability Diagnostics — Chain Tracing, ACPX Validation, Auxiliary.

### Phase 2: Execution

Run the primary Provider Reliability Diagnostics — Chain Tracing, ACPX Validation, Auxiliary operations according to the defined requirements.
