...
- When you need to automate or structure workflows for `windows-package-management`.
...
Automated reasoning and workflow tool for `windows-package-management`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
Use this section when an install or upgrade fails, emits a non-zero exit code, or appears to hang.

- Read the installer log before retrying.
- Identify process blockers before killing or rescheduling anything.
- Verify the installed version after partial-success scenarios.
...
Set up required environment, dependencies, and configuration for "Windows Package Management".

### Phase 2: Execution

Run the primary "Windows Package Management" operations according to the defined requirements.
