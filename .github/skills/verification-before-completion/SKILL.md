...
Automated reasoning and workflow tool for `verification-before-completion`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
### Ad-Hoc Verification (temp `hermes-verify-*.py` recipe)

Use when a code edit has no canonical test/lint/build command and fresh
verification evidence is required. Write a throwaway script to the temp dir,
load the changed module via `importlib.util.spec_from_file_location`, assert
the new behavior + edge cases, run it, then delete it. Summarize as **ad-hoc
verification**, never "suite green".

**Critical Windows/MSYS pitfall:** pass a NATIVE path with forward slashes
(`"C:/Users/.../hermes-verify-x.py"`), NOT an MSYS `/c/Users/...` path — the
mount layer rewrites `/c/...` to `\\c\\...` and `python3` fails with
`[Errno 2] No such file`. Also run write+execute in one statement, because a
stray `rm` in a failed earlier command wipes the script before retry.

**False-failure pitfall:** a failing assert is sometimes the TEST's fault, not
the code's (e.g. hard-coding one valid LCS when several are valid). Fix the
assertion, don't "fix" correct code to match a wrong test.

**Script reference:** See `references/ad-hoc-verify-python-windows.md` for the
full recipe and both pitfalls.
