...
### 8. Substring Path Check False Positive (Archived Templates)

**Session:** 2026-06-29 | **Case:** enhance-markdown on prompts/repo.prompt.md

**Trigger:** Verification check `'templates/repo.prompts' in text` for stale template references.

**False Positive Pattern:** When a stale template path was fixed by changing it to `.hermes/archived-prompt-templates/repo.prompts/`, the substring check `'templates/repo.prompts' in text` returns `True` because `archived-prompt-**templates/repo.prompts**/` contains `templates/repo.prompts` as a substring.

```
# False positive:
check = 'templates/repo.prompts' in text
# text contains: '.hermes/archived-prompt-templates/repo.prompts/'
# check returns True even though the ref was correctly updated!
```

**Fix:** Use exact path matching or regex with start-of-path/word-boundary anchors instead of substring `in`:

```python
import re
# Correct: match 'templates/' as a path component, not substring of 'archived-prompt-templates/'
template_matches = re.findall(r'Templates in `([^`]+)`', text)
stale_template = any(m.startswith('templates/') for m in template_matches)
```

Or for path content checks:
```python
# Detect 'templates/' at start of path (not in 'archived-prompt-templates/')
stale = bool(re.search(r'(?<![a-zA-Z-])templates/', text))
```

**General principle:** Never use substring `in` to check for path or filename presence when the target string could be a substring of a larger valid string. Use regex with `\b` word boundaries or path-component anchors (`startswith`, `endswith`, path-separator-aware splitting).

---

### 9. Non-YAML Prompt Files in Batch Audits
