...
**⚠️ OpenCode Zen Pitfall:** Always include `/chat/completions` in the base URL. The common mistake is using `https://opencode.ai/zen/v1` (returns 404) instead of `https://opencode.ai/zen/v1/chat/completions` (correct). See `references/opencode-zen-provider-config.md` for setup details and verification commands.
...
- When you need to perform Hermes Agent operations or tasks
- When managing Hermes Agent infrastructure or configurations
- When automating or debugging Hermes Agent workflows
- **Triggers**: "hermes agent" required for a project
