...
**Rationale:** If patches fail, `git checkout HEAD -- ~/AppData/Local/hermes/skills/{skill}/SKILL.md` restores any skill.
...
git checkout HEAD -- $LOCALAPPDATA/hermes/skills/{skill-name}/SKILL.md

# Example:
git checkout HEAD -- $LOCALAPPDATA/hermes/skills/dispatching-parallel-agents/SKILL.md
...
git checkout SKILL-FIX-{DATE}-PRE-PATCH -- $LOCALAPPDATA/hermes/skills/
...
grep "skill_view()" $LOCALAPPDATA/hermes/skills/{skill-name}/SKILL.md | grep -v "name="
...
grep -r "skill_view()" $LOCALAPPDATA/hermes/skills/*/SKILL.md | grep -v "name=" | wc -l
# Expected: 0

# 4. Examples are current
grep -i "sandbox\|bash/" $LOCALAPPDATA/hermes/skills/*/SKILL.md | head -5
