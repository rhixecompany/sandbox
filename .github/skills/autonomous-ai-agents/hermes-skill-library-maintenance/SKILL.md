...
- Access to Hermes skills directory: `~/AppData/Local/hermes/skills/` (or `%APPDATA%\Local\hermes\skills\` on Windows)
...
1. **Inventory all skills**
   ...
   skills_dir = Path(os.environ.get("LOCALAPPDATA", os.path.expanduser("~/AppData/Local")) + "/hermes/skills")  # cross-platform
   # OR (legacy Windows absolute):
...
4. **Rollback Plan**
   ...
   # If error: git checkout HEAD -- ~/AppData/Local/hermes/skills/{skill}/SKILL.md
   ```
...
skills_dir = os.environ.get("LOCALAPPDATA", os.path.expanduser("~/AppData/Local")) + "/hermes/skills"
...
python3 -c "
import os, yaml
skills_dir = os.path.join(os.environ.get("LOCALAPPDATA", os.path.expanduser("~/AppData/Local")), "hermes", "skills")
...
3. Always verify with `ls ~/AppData/Local/hermes/skills/<path>/SKILL.md` after any delete to confirm the right target was hit
...
- `references/remediation-patterns-2026-06-04.md` — Concrete patterns discovered during skills-fix sessions
- `references/pipeline-execution-2026-06-28.md` — Full execute-all-prompts pipeline execution record (362 skills, 4 phases, 0 FAIL)
