...
- **[NEW] Check session continuity:**
  ...
  - Verify global `~/AppData/Local/hermes/memories/USER.md` and `~/AppData/Local/hermes/SOUL.md` exist and are current
...
### Phase 1.5: Clarify Blockers Before Proceeding

- If any phase is blocked by missing files, unclear scope, or conflicting instructions, use `clarify` with up to 4 choices immediately.
- Do not fabricate artifacts, stubs, or plausible outputs to replace missing sources or blocked execution.
- When the user releases a quarantine or override, note it explicitly and proceed only with verified non-destructive actions.
...
- Provide response
- Document decisions
- **Never return an empty response after tool calls.** All tool results must be processed and summarized in the same turn before composing the assistant message.
- **Encode completion prefs once:** honor the user’s explicit completion requirement
  - Hooks tasks are not complete until the full lifecycle is verified end-to-end: unconfigure, delete, recreate, re-register, doctor, and tests all pass.
  - If the user says "dont stop until the plans has been created,verified,executed,completed without errors,warnings,issues", treat that as a hard stop condition and keep driving verification, not as a suggestion.- **Encode completion prefs once:** honor the user’s explicit completion requirement
  - Hooks tasks are not complete until the full lifecycle is verified end-to-end: unconfigure, delete, recreate, re-register, doctor, and tests all pass.
  - If the user says "dont stop until the plans has been created,verified,executed,completed without errors,warnings,issues", treat that as a hard stop condition and keep driving verification, not as a suggestion.
## Tools & References
...
- **Skill line-limit audit pattern**: when a skill exceeds its own verification limit (commonly 250 lines), move detailed body content into `references/<topic>.md` and rewrite `SKILL.md` as a compact core (commands + workflow + checklist + pointer to reference). Verify line count drops below the limit and the reference file exists before stopping.
- **Bulk-edit scripts can silently corrupt structured data (YAML, JSON, TOML).** Regex/line-based replacements on multi-line structured formats often collapse arrays into inline strings, duplicate keys, or break nesting. Always validate after bulk edits with a parser (e.g., `yaml.safe_load` on each file) and prefer targeted `patch`/`read_file` edits over scripted sweeps.
- **Corruption spanning many commits is best fixed by restoring from the last clean ancestor.** When bad commits interleave with good ones, `git checkout <clean-commit> -- <paths>` is faster and safer than manual repair. Always verify with a structural check (e.g., `grep -r "corruption_pattern" prompts/`).
...
- **Re-run vs resume prompt pipelines**: When the user invokes a prompt pipeline (e.g., `/execute-all-prompts`) that is already recorded as completed in SESSION_REPORT.md, do NOT ask the user whether to re-run or resume via check. Proceed with fresh re-execution using reasonable defaults (full run from scratch). The user's invocation intent is clear: run it again. Blocking on check for re-runs wastes a round-trip.
- **Authorization gating when blocked**: If the runtime blocks further action, stop before doing anything indefinite or irreversible. Summarize actual verified artifacts vs optional next steps, then state exactly:
  1. what is complete on disk now,
  2. what remains,
  3. what authorization/scope/time-window you need to proceed.
  Do not continue without a clarified explicit authorization.
- **Async delegation batch handling**: When an async delegation batch completes, first verify the stated output files exist on disk before claiming progress or advancing to the next phase. Use the existence check as the success criterion, not the subagent summary alone.
...
- **YAML frontmatter corruption from bulk edits.** Scripted bulk edits (regex, sed, line-replacement) on files with YAML frontmatter frequently collapse multi-line arrays into inline strings (`tags: - foo - bar`), duplicate `metadata:` blocks, and merge adjacent keys into garbage like `tags: - foo.barmetadata:`. The corruption is structural — not a typo — and compounds across commits. When debugging garbled frontmatter, check `git log --oneline` for "updates" or bulk-edit commits; restore from the last clean commit with `git checkout <clean> -- prompts/`.
- **YAML frontmatter corruption from bulk edits**: When scripted edits (regex, sed, line-replacement) target files with YAML frontmatter, they often collapse multi-line arrays into inline strings (`tags: - foo - bar`), duplicate `metadata:` blocks, and merge adjacent keys into garbage like `tags: - foo.barmetadata:`. The corruption is structural — not a typo — and compounds across commits. Fix: restore from the last clean commit (`git log --oneline | grep -v "updates" | head -1`) using `git checkout <clean> -- prompts/`, then re-apply only intentional changes.
...
- **Strict sequential on "and only then"**: treat each chained phase as a hard dependency — spec compliance must be PASS before code quality review begins, and both reviews must be APPROVED before marking any task complete. The `subagent-driven-development` skill is the canonical implementation of this ordering constraint: implementer → spec reviewer → quality reviewer → mark complete → next task, strictly in that order.
