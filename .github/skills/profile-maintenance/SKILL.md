...
Automated reasoning and workflow tool for `profile-maintenance`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
1. Read the 3 canonical files:
   - `~/AppData/Local/hermes/memories/USER.md` — user identity (canonical for default profile)
...
4. If a `SESSION_REPORT.md` exists in the workspace, inspect it before relying on it.
   - If it is only a stub/header or otherwise lacks actionable continuity, treat prior context as absent and overwrite it with a real rolling summary.
   - A useful report should include: session id, timestamp, profile, model, work completed, tools/skills used, current state, and a compact session changelog.
...
5. Prefer a compact session report over long prose.
   - The report is a durable handoff artifact, not a narrative.
   - Use tables/bullets only; keep the changelog explicit so the next session can see exactly what changed.
...
11. Verify `memories/USER.md` is the canonical file for each profile (root USER.md no longer exists)
...
- **memories/USER.md** → canonical user identity file (root USER.md deleted; `memories/USER.md` is now authoritative for each profile) → does NOT duplicate SOUL.md content

## Strict Rules Checklist (SOUL.md Updates)

When updating SOUL.md, verify these 4 non-negotiable rules are present and current:
...
**Cross-profile sync:** When updating SOUL.md rules, apply to ALL profiles (`code-architect`, `research-analyst`, `creative-director`, `exec-assistant`, `patient-tutor`, `adminbot`, `default`). Each profile keeps unique personality + shared strict rules.

## Profile Config Deduplication (DRY)

When auditing profile configs:

1. Compare all `config.yaml` files across profiles
2. Keep only profiles with **genuinely unique** settings (custom model, different MCP servers, unique plugins)
3. Remove duplicates — profiles without unique config inherit from global
4. Clean all `.bak.*`, `.before-mcp.merge.*` backup artifacts
5. Verify hooks/plugins/skills paths are consistent where shared

See `references/profile-sync-procedure.md` for the full sync workflow.
...
- `references/profile-sync-procedure.md` — Full profile sync & deduplication workflow with verification steps
...
- **Don't miss the memories/ copies.** Each profile has its own `memories/USER.md` and `memories/MEMORY.md`. The default profile uses root `~/AppData/Local/hermes/memories/` directly.
...
- **Fix validator stale schema instead of re-duplicating profile data.** When a validator still enforces sections you intentionally deduped away, patch the validator script rather than bloating compact memory files.
- **Cross-profile edit guard.** Writes under another profile's `memories/` are blocked by default. Use `write_file(..., cross_profile=True)` for authorized cross-profile updates.
- **memories/USER.md size**: Keep `memories/USER.md` within the standard size limit (<1375 bytes). Root USER.md no longer exists — `memories/USER.md` is the canonical file.
- **System identity belongs in root profile files only.** Non-default profile memory files should cross-reference root for system-wide identity such as model/provider, not restate it.
- **`~/AppData/Local/hermes/` is usually not a git repository.** Favor recording profile/work updates in a workspace git repo. Confirm repo root before committing or reporting git state.
- **Validator reads injected prompt text, not raw files:** `memory(action='replace')` matches the injected system prompt's whitespace-collapsed text. If `replace` fails twice, switch to `write_file` for a clean rewrite and stop retrying the same pattern.

### Two Memory Systems: Native `memory` Tool vs MCP Knowledge Graph

Hermes has **two distinct persistent memory systems** — they are NOT interchangeable:

| Aspect | Native `memory` Tool | MCP Memory Server |
|--------|---------------------|-------------------|
| **Backend** | Flat files (`MEMORY.md`, `USER.md`) | Knowledge graph (entities + observations + relations) |
| **Data model** | Line-based flat text with `§` delimiters | Graph: Entity(observations[]) + Relation(from→to) |
| **Write approval** | May have `memory.write_approval` — stages writes for TUI approval | Writes directly, no approval step |
| **Approval method** | `/memory pending` or `/memory approve <id>` in TUI | N/A — writes are immediate |
| **Char limit** | Yes (MEMORY.md 2200, USER.md 1375) | No per-entity limit |
| **Tool name in session** | `memory` | `mcp_mcp_docker_mcp_exec` (via Docker gateway) + memory server's graph tools |
| **Adding at runtime** | Always available | Add via `mcp_mcp_docker_mcp_find` + `mcp_mcp_docker_mcp_add(name='memory')` |
| **Persistence** | Cross-session (injected into every turn) | Cross-session (knowledge graph DB) |

**When to use which:**
- **Native `memory` tool**: Compact user preferences, environment facts, corrections — things that need to be injected into every system prompt (the char-limited injection).
- **MCP memory server**: Rich structured knowledge (entity graphs, multi-observation profiles, relational data) that benefits from queries and traversal rather than flat injection.

**Workflow for populating MCP memory from session knowledge:**
1. Add the server: `mcp_mcp_docker_mcp_find(query='memory')` → then `mcp_mcp_docker_mcp_add(name='memory')`
2. Verify with `mcp_mcp_docker_mcp_exec(name='read_graph')`
3. Create entities with observations: `mcp_mcp_docker_mcp_exec(name='create_entities', arguments={entities: [...]})`
4. Create relations between entities
5. Verify with `read_graph` again

**Recommended entity model for user profile knowledge:**
- `User` (type: User) — identity facts as observations (name, OS, shell, editor, workspace, active model)
- `Communication-Preferences` (type: Preference) — style rules as observations
- `Execution-Rules` (type: Workflow) — workflow constraints as observations
- `Safety-Rules` (type: Protocol) — safety conventions as observations
- `Session-Protocol` (type: Workflow) — session startup requirements
- `Tech-Stack` (type: Environment) — tools and versions
- `Config-Management` (type: Workflow) — config edit rules
- `Documentation-Tips` (type: Knowledge) — environment-specific tips

Connect with relations like `Alexa → prefers → Communication-Preferences`, `Execution-Rules → enforces → Safety-Rules`.

### Pitfall: `memory.write_approval` Staging — No CLI Approve

**Problem:** When `memory.write_approval` is enabled in config.yaml, the native `memory` tool stages every write with a `pending_id` instead of writing directly. The CLI (`hermes memory`) does NOT expose an approve command — only `setup`, `status`, `off`, and `reset`. There is no way to approve a pending memory write from the terminal.

**Symptom:** `memory(action='add'/'replace')` returns `staged: true, pending_id: "xxxxx", message: "Staged for approval (memory.write_approval is on). Not yet saved — review with /memory pending."` The write appears to succeed but the file is not updated on disk.

**Prevention:**
1. When you need to write immediately and irrefutably, either:
   - Ask the user to type `/memory approve <id>` or `/memory pending` in the TUI
   - Write the target file directly with `write_file` (only for USER.md/MEMORY.md under `memories/` — bypasses the native tool entirely)
2. The MCP memory server does NOT have this constraint — writes are immediate.

**Recovery:** Batch all pending memory updates into a single `operations` array call. The whole batch stages as one pending_id, so the user only needs to approve once. If you have multiple staged writes pending, each must be approved separately.
...
- [ ] No placeholder text
- [ ] All USER.md references point to `memories/USER.md` (root USER.md deleted)
- [ ] All USER.md references point to `memories/USER.md` (root USER.md deleted)
