...
cat > $HOME/AppData/Local/hermes/scripts/sync-profiles.py << 'EOF'
#!/usr/bin/env python3
...
if __name__ == "__main__":
    main()
EOF
chmod +x $HOME/AppData/Local/hermes/scripts/sync-profiles.py
```
...
cd $HOME/AppData/Local/hermes/scripts/
python3 sync-profiles.py

# Or with full path
python3 $HOME/AppData/Local/hermes/scripts/sync-profiles.py
```
...
HERMES_HOME = Path(os.environ.get("LOCALAPPDATA", os.environ.get("HERMES_HOME", os.path.expanduser("~/AppData/Local"))) + "/hermes")
...
2. Verify `scripts/` directory exists: `ls $HOME/AppData/Local/hermes/scripts/`
