...
**Per-subproject naming:** When generating per-subproject docs in a monorepo, use the exact directory name (preserving hyphens, underscores, case) for the filename — e.g., `docs/Project_Architecture/university-libary-jsm_techstack.md` for subproject `university-libary-jsm`. Do NOT normalize underscores — mismatches create duplicate files.
