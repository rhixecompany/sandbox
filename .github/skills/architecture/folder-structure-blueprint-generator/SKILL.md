...
**Per-subproject naming:** When generating per-subproject docs in a monorepo, use the exact directory name (preserving hyphens, underscores, case) for the filename — e.g., `docs/Project_Architecture/cookiecutter-django-tailwind_folders.md` for subproject `cookiecutter-django-tailwind`. Do NOT normalize underscores — mismatches create duplicate files.
