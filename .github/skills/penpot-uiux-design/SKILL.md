---
author: Alexa
description: 'Comprehensive guide for creating professional UI/UX designs in Penpot
  using MCP tools. Use this skill when: (1) Creating new UI/UX designs for web, mobile,
  or desktop applications, (2) Building design systems with components and tokens,
  (3) Designing dashboards, forms, navigation, or landing pages, (4) Applying accessibility
  standards and best practices, (5) Following platform guidelines (iOS, Android, Material
  Design), (6) Reviewing or improving existing Penpot designs for usability. Triggers:
  ''design a UI'', ''create interface'', ''build layout'', ''design dashboard'', ''create
  form'', ''design landing page'', ''make it accessible'', ''design system'', ''component
  library''.'
license: MIT
...
Automated reasoning and workflow tool for `penpot-uiux-design`. Execute multi-step tasks with deterministic quality controls and structured outputs.

## When to Use

- Use this skill when working with penpot uiux design tasks
- Triggered by: `penpot-uiux-design` related operations
...
const boards = penpotUtils.findShapes(
  s => s.type === "board",
  penpot.root
);
let nextX = 0;
...
Set up required environment, dependencies, and configuration for Penpot Uiux Design.

### Phase 2: Execution

Run the primary Penpot Uiux Design operations according to the defined requirements.
