...
Automated reasoning and workflow tool for `plans-and-specs`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
- **Cross-Platform Asset Sync (Skills/Plugins/Hooks)**: See `references/cross-platform-asset-sync.md` — companion to agent inventory; sync workflows for Hermes → Copilot → Codex
...
- **Plan search scope drift**: When asked to locate plans under `docs/`, `thoughts/`, or `plan/`, do not treat every file containing `plan` as a plan artifact. Use content cues against path context: real planning docs include phases, tasks, status sections, or roadmap intent; skip resource/reference/prompt/library doc folders that only match by filename. If ambiguous, inspect before migrating.
- **Duplicate migration marking**: Before adding another `status: not_started` block, check whether the file was already marked (`status`, `not_started`, `<!-- migrated from:`) so one pass does not stack multiple frontmatter/marker sections.
