...
## Open-Source / Local Generation (HeartMuLa)

> The former standalone `heartmula` skill is consolidated here as `references/heartmula.md`.
> Use it when the user wants an open-source, local, or offline alternative to Suno-style generation,
> or asks specifically about HeartMuLa / heartlib.

HeartMuLa is a family of Apache-2.0 music foundation models that generate full songs from lyrics +
tags (comparable to Suno, but self-hostable). Key facts: min 8GB VRAM (`--lazy_load true`), Python
3.10 venv, manual dependency upgrades + two source patches required as of Feb 2026, CUDA/Linux for
GPU. The full install, patching, CLI flags, and pitfalls are in `references/heartmula.md`.

Suno (§6) is the fastest path for most users; HeartMuLa is the choice when you need local/offline
control or want to avoid a hosted service.

## 10. Audio Analysis (Songsee)

> The former standalone `songsee` skill is consolidated here as `references/songsee.md`.
> Use it when the user wants to *inspect* audio (spectrograms, MFCC, chroma, tempo) rather than
> generate it — comparing AI-music outputs, debugging synthesis, or documenting an audio pipeline.

Songsee (Go CLI) renders multi-panel visualizations from any audio file:
```bash
go install github.com/steipete/songsee/cmd/songsee@latest
songsee track.mp3 --viz spectrogram,mel,chroma,hpss,selfsim,loudness,tempogram,mfcc,flux -o grid.png
```
Full flag reference and visualization types are in `references/songsee.md`. Output PNGs can be
read with `vision_analyze` for automated audio analysis.

## When to Use

- When you need to use songwriting and ai music for creative or design tasks
- When a project requires songwriting and ai music output or integration
- When automating or generating songwriting and ai music-style content
- **Triggers**: "Songwriting", "AI music", "Suno", "HeartMuLa", "generate a song"

## Workflow

### Phase 1: Setup

Load required dependencies and verify environment is configured correctly.

### Phase 2: Configure

Set parameters, styles, and options for the desired songwriting and ai music output.

### Phase 3: Generate

Execute the songwriting and ai music process to produce the final result.

### Phase 4: Review

Verify the output meets quality standards and adjust parameters if needed.

## Overview

The Skills\creative\songwriting And Ai Music\skill.md skill provides tools and workflows for managing skills\creative\songwriting and ai music\skill.md operations efficiently.
...
- [ ] Songwriting And Ai Music output was generated successfully
