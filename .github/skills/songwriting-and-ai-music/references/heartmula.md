# HeartMuLa - Open-Source Music Generation (reference)

> Demoted from the standalone `heartmula` skill during umbrella consolidation.
> Use this when the user wants an open-source / local / offline alternative to Suno-style
> generation, or asks specifically about HeartMuLa / heartlib.

## Overview
HeartMuLa is a family of open-source music foundation models (Apache-2.0) that generates music
conditioned on lyrics and tags, with multilingual support. Generates full songs from lyrics + tags.
Comparable to Suno for open-source. Includes:
...
In the `setup_caches` method of the `HeartMuLa` class, add RoPE reinitialization after the
`reset_caches` try/except block and before the `with device:` block:
```python
...
for module in self.modules():
    if isinstance(module, Llama3ScaledRoPE) and not module.is_cache_built:
        ...
        module.to(device)
```
**Why**: `from_pretrained` creates model on meta device first; `Llama3ScaledRoPE.rope_init()`
skips cache building on meta tensors, then never rebuilds after weights are loaded to real device.

**Patch 2 - HeartCodec loading fix** in `src/heartlib/pipelines/music_generation.py`:
Add `ignore_mismatched_sizes=True` to ALL `HeartCodec.from_pretrained()` calls (there are 2: the
eager load in `__init__` and the lazy load in the `codec` property).
**Why**: VQ codebook `initted` buffers have shape `[1]` in checkpoint vs `[]` in model. Same data,
just scalar vs 0-d tensor. Safe to ignore.
...
## GPU / CUDA
HeartMuLa uses CUDA by default (`--mula_device cuda --codec_device cuda`). No extra setup needed if
the user has an NVIDIA GPU with PyTorch CUDA support installed.
...
- **No GPU?** Run on CPU with `--mula_device cpu --codec_device cpu`, but expect generation to be
  **extremely slow** (30-60+ minutes per song vs ~4 min on GPU). CPU mode also requires ~12GB+ RAM.
  If no NVIDIA GPU, recommend a cloud GPU service (Google Colab free tier with T4, Lambda Labs) or
  the online demo at https://heartmula.github.io/ instead.
...
or `rock,energetic,guitar,drums,male-vocal`
