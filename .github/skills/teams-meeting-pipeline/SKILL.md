...
Automated reasoning and workflow tool for `teams-meeting-pipeline`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
Before using the pipeline, verify these are set in `~/AppData/Local/hermes/.env`:
...
- When working with teams meeting pipeline tools or integrations
- When automating or managing teams meeting pipeline tasks
- When troubleshooting teams meeting pipeline configurations
- **Triggers**: "Teams Meeting Pipeline", "teams meeting pipeline" related operations
...
Perform the teams meeting pipeline operations following the skill instructions.
