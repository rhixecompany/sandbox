---
author: Alexa
description: Wire external services (GitHub, Stripe, Sentry, Cal.com, TypeForm, Helius,
  Beehiv, Home Assistant) to Hermes webhooks for event-driven automation.
license: MIT
metadata:
  hermes:
    tags:
...
Automated reasoning and workflow tool for `service-integrations`. Execute multi-step tasks with deterministic quality controls and structured outputs.
...
```bash
grep webhook $LOCALAPPDATA/hermes/logs/gateway.log | tail -20
```
