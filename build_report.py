...
lines.append("")
lines.append(
    "**Scope:** Read-only audit of all 211 `*.prompt.md` files in "
    "`C:\\Users\\Alexa\\AppData\\Local\\hermes\\prompts\\`."
)
lines.append("")
lines.append("**Date:** 2026-07-16  ")
lines.append(
    "**Method:** Python regex scan of each prompt body (text after the closing frontmatter `---`). "
...
lines.append("")
lines.append(
    f"**Overall library safety verdict:** ✅ **SAFE — no CRITICAL or HIGH findings.** "
...
lines.append("")
lines.append(
    "A prompt is considered to have *recognizable structure* if its body contains at least one "
    "H2 (`##`) or H3 (`###`) heading whose text includes one of the canonical keywords: "
    "**Goal, Context, Workflow, Phases, Rules**."
)
lines.append("")
lines.append(
    f"**{len(flagged)} of {s['total']} prompts ({(len(flagged) / s['total'] * 100):.1f}%) do not meet the "
...
lines.append("")
lines.append(
    "These contain **no H2/H3 headings at all** — only prose and/or bullet lists. This is the "
...
lines.append("")
lines.append(
    "These DO have H2/H3 section headings (so they are not blobs), but the headings do not use "
...
lines.append("")
lines.append(
    "- **CRITICAL — Injection / jailbreak:** `ignore previous instructions`, `pretend you are`, "
    "`DAN`, `do anything now`, `reveal your system prompt`, `exfiltrate`, `bypass (safety/guard)`."
)
lines.append(
    "- **HIGH — Destructive ops without approval gate:** `rm -rf`, `sudo rm`, `drop table/database`, "
...
lines.append(
    "- **HIGH — Secrets / credentials exposure:** instructions to show/print/echo/output/reveal/"
...
if not high:
    lines.append(
        "- **HIGH findings: 0** — no un-gated destructive-operation instructions and no "
...
lines.append("")
lines.append(
    "**Library safety status: PASS.** No CRITICAL or HIGH severity safety issues were found "
...
lines.append("")
lines.append(
    "**Structural note (non-blocking):** 47/211 prompts do not use canonical section headings. "
...
lines.append("")
lines.append(
    "*Generated by automated read-only audit (`audit_prompts.py`). Source patterns: "
...
print(
    f"  total={s['total']} flagged={len(flagged)} (blob={s['structure_less_count']}, noncanonical={s['noncanonical_count']}) critical={s['critical_count']} high={s['high_count']}"
)
