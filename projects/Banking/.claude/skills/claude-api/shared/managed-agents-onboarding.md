...
| **Custom tools** | The user's own app handles these tool calls — agent fires `agent.custom_tool_use`, the app sends a result message back. | Ask for each tool: name, description, input schema. The app code that handles the event is _their_ code — don't generate it. Full detail: `shared/managed-agents-tools.md` → Custom Tools. |

**Round B — Skills, files, and repos.** What the agent has on hand when it starts.

_Skills_ — two types; both work the same way — Claude auto-uses them when relevant. Max 20 per agent.
...
_GitHub repositories_ — any repos the agent needs on-disk? For each:
...
_Files_ — any local files to seed the session with? For each:
