...
This document describes the YAML schema for the banking registry, a data-driven catalog of OpenCode extensions. It covers extension types, configuration locations, schema fields, and downstream compatibility.

---

## 1. Overview

The banking registry is a curated collection of extensions that enhance the OpenCode editor. The registry follows a data-driven approach where YAML files in `.opencode/{category}/` directories serve as the source of truth.
...
Extensions in banking fall into distinct categories, each serving a specific purpose in the OpenCode ecosystem.
...
**Resolution Example:** If `scope` is set to `[global, project]` and both global and project configs define the same extension, the project-level configuration takes precedence for that project.

---

## 4. YAML Schema

The banking registry uses a flexible schema with four required fields and several optional fields for enhanced discoverability.
...
| `.opencode/themes/`    | `theme`    |
| `.opencode/agents/`    | `agent`    |
| `.opencode/projects/`  | `project`  |
| `.opencode/resources/` | `resource` |
| `.opencode/forks/`     | `fork`     |
...
The banking schema is designed for seamless conversion to downstream platforms like opencode.cafe.

### Field Mapping

| banking YAML   | opencode.cafe JSON | Notes                       |
| -------------- | ------------------ | --------------------------- |
...
**YAML Input (`.opencode/plugins/codegpt.yaml`):**
...
Note: `type` is derived from `.opencode/plugins/` → `plugin`, and `productId` from filename `codegpt.yaml` → `codegpt`.

---

## 7. Future Vision

This section outlines planned features and improvements for the banking ecosystem.
...
| `theme`    | `.opencode/themes/`    |
| `agent`    | `.opencode/agents/`    |
| `project`  | `.opencode/projects/`  |
| `resource` | `.opencode/resources/` |
| `fork`     | `.opencode/forks/`     |
...
See `.opencode/examples/` directory for full exemplar files.
...
Remaining tasks for the banking registry.

### 9.1 Update Contributing Guide

Update `contributing.md` to:

- Reference `.opencode/examples/` directory for full field documentation
...
```javascript
// Pseudocode
for each file in .opencode/{category}/*.yaml:
