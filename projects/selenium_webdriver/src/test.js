...
async function parsePageDetail(driver) {
  let title = await driver
    .findElement(
      By.xpath(
        "//div[@class='text-center sm:text-left']/span[@class='text-xl font-bold']",
      ),
    )
    .getText();
  let image = await driver
    .findElement(
      By.xpath(
        "//div[@class='relative col-span-full sm:col-span-3 space-y-3 px-6 sm:px-0']/img[@class='rounded mx-auto md:mx-0']",
      ),
    )
    .getAttribute("src");
  ...
  try {
    ...
    while (retries > 0) {
      try {
        ...
        await clickElement(
          By.xpath(
            "//*[@class='flex items-center justify-center py-[15px] bg-[#222222] ']/a[2]",
          ),
          driver,
        );
