...
export function DataTable({ data: initialData }: { data: z.infer<typeof schema>[] }) {
  ...
  // react-hooks/incompatible-library: Narrow disable to the specific call site where
  // `useReactTable()` returns non-memoized callbacks known to trigger the React
  // compiler warning. This keeps the rest of the file linted normally. See docs/proposedFixes.MD
  // for rationale.
  ...
  return (
    <Drawer direction={isMobile ? "bottom" : "right"}>
      <DrawerTrigger asChild>
        <Button className="w-fit px-0 text-start text-foreground" variant="link">
          {item.header}
        </Button>
      </DrawerTrigger>
      <DrawerContent>
