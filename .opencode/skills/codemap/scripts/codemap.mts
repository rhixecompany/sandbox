...
export function computeFolderHash(folder, fileHashes) {
	const folderFiles = Object.entries(fileHashes)
		.filter(([filePath]) => filePath.startsWith(`${folder}/`) || (folder === "." && !filePath.includes("/")))
...
export function cmdInit({ root, include = [], exclude = [], exception = [] }) {
	...
	const selectedFiles = selectFiles(resolvedRoot, includePatterns, excludePatterns, exceptions, gitignore);

	console.log(`Selected ${selectedFiles.length} files`);

	const { state, folders } = buildState(resolvedRoot, includePatterns, excludePatterns, exceptions, selectedFiles);
...
export function cmdChanges({ root }) {
	...
	const currentFiles = selectFiles(resolvedRoot, includePatterns, excludePatterns, exceptions, gitignore);
...
export function cmdUpdate({ root }) {
	...
	const { state: nextState } = buildState(resolvedRoot, includePatterns, excludePatterns, exceptions, selectedFiles);
