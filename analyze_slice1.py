...
ACCEPT_PAT = re.compile(r'\b(acceptance criteria|acceptance|criterion|done when|definition of done|success criteria)\b', re.I)
VERIFY_PAT = re.compile(r'\b(verify|verification|checklist|self[- ]?check|before (you )?finish|confirm)\b', re.I)
FILLER_PAT = re.compile(r'\b(hi|hello|hey|sure|great|let me know|feel free|as an ai|i can help|please note that|just a|okay|ok)\b', re.I)
...
for fn in FILES:
    ...
    tags_empty = (tags is None) or (isinstance(tags, list) and len(tags)==0) or (isinstance(tags,str) and not tags.strip())
    ...
    def hdr_has(pat):
        return any(re.search(pat, h, re.I) for h in headers)
    has_goal = hdr_has(r'goal')
    has_context = hdr_has(r'context')
    has_phases = hdr_has(r'phase|workflow|step|process|procedure')
    has_rules = hdr_has(r'rule|constraint|requirement|do(?:n.?t)?|guideline|policy')
    has_verify = hdr_has(r'verif|checklist|check|acceptance|done|validat')
    ...
    issues = []
    if not (desc and str(desc).strip()): issues.append("missing description")
    if tags_empty: issues.append("empty/missing tags")
    if not plan: issues.append("missing plan")
    if not formatter: issues.append("missing formatter")
    if not has_acc_or_verify: issues.append("no acceptance/verification criteria")
    if not has_rules: issues.append("no explicit rules section")
    if not has_phases: issues.append("no workflow/phases section")
    if refs: issues.append("dead references")
    # candidate value: more issues = higher value
    value = len(issues)
    results.append((fn, fm, {
        "n_lines": n_lines, "n_headers": len(headers),
        "desc": bool(desc and str(desc).strip()),
        "tags_empty": tags_empty, "tags": tags,
        "plan": bool(plan), "formatter": bool(formatter),
        "has_goal": has_goal, "has_context": has_context,
        "has_phases": has_phases, "has_rules": has_rules, "has_verify": has_verify,
        "has_acc_or_verify": has_acc_or_verify,
        "bullets": bullets, "do_dont": do_dont, "accept": accept, "verify_kw": verify_kw,
        "filler": filler, "instruction_ratio": round(instruction_ratio,2),
        "refs": refs, "issues": issues, "value": value,
        "size": len(text),
    }))
...
# Print summary table
print(f"{'FILE':45} {'val':>3} {'desc':>4} {'tags':>4} {'plan':>4} {'fmt':>3} {'acc/v':>5} {'phs':>3} {'rul':>3} {'refs':>4} issues")
for fn, fm, m in sorted(results, key=lambda x: -x[2].get('value',0)):
    if m.get('missing'):
        print(f"{fn:45} MISSING"); continue
    print(f"{fn[:44]:45} {m['value']:>3} {str(m['desc'])[0]:>4} {('E' if m['tags_empty'] else '+'):>4} {str(m['plan'])[0]:>4} {str(m['formatter'])[0]:>3} {(m['accept'] or m['has_verify']):>5} {m['has_phases']:>3} {m['has_rules']:>3} {len(m['refs']):>4}  {','.join(m['issues'])}")
