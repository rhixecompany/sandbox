...
## 1. Identity Lock (Verified — From `.hermes.md` + SOUL.md + USER.md + MEMORY.md)
...
- **Workspace:** `~/Desktop/SandBox` (verified `pwd` = `/c/Users/Alexa/Desktop/SandBox`; `.hermes.md` verified 2859 B).
...
- **DRY enforcement:** Never duplicate identity rules in descriptions/aliases; cross-reference `.hermes.md` + `references/hooks-contract.md`.
...
|---|---|
| DRY — no duplicate facts | `grep -R` for duplicate identity sentences across profiles = 0; cross-reference `.hermes.md` |
...
| Profile identity DRY | SOUL.md owns identity rules; USER.md points to SOUL.md + MEMORY.md; MEMORY.md owns durable facts; descriptions + aliases point to `.hermes.md` + best practices |
...
find ~/AppData/Local/hermes/profiles/ -maxdepth 1 -type d | grep -v '^~/AppData/Local/hermes/profiles/$' | while read prof_dir; do
  ...
  # Read → patch (targeted) → verify (grep for .hermes.md reference)
done
```
Gate per profile: `grep -q '.hermes.md' ~/AppData/Local/hermes/profiles/$profile_name/SOUL.md` → 1.
...
- [x] Verify DRY enforced (no duplicate identity phrases across profiles; cross-references to `.hermes.md` verified)
...
---
*Prompt verified: `verified:true` from `write_file()`; all cross-references point to verified real files; no synthetic session IDs / capabilities / quality / ranking inserted; DRY enforced; best practices referenced via `.hermes.md` + skill cross-links (not duplicated).*
